...
public abstract class BaseContainer extends HashMap<String, String> implements Constants{
	...
	private static final String TAG = RP_TAG + BaseContainer.class.getSimpleName();
    ...
    protected void parse() {
        ...
        for (Node child : children) {
            ...
            if (LOG_VERBOSE) Log.d(TAG, "Node name is " + name);
    ...
    protected final boolean hasCDataChildNodes(Node node) {
        ...
        if (node.hasChildNodes()) {
            ...
            for (int j=0; j < list.getLength(); j++) {
                ...
                {
                    if (LOG_VERBOSE) Log.d(TAG, "subnode is " + subnode.getNodeName());
                    found = true;
                    break;
                }
            }
