...
/** IntentHandler class for handling intents for PAF (Publisher Availability Finder).
 * 
 *<code><pre>
 * CLASS:
 *     PafIntentHandler Extends IntentHandler
 * Interface:
 * 		PafConstants
 *
 * RESPONSIBILITIES:
...
public class PafIntentHandler extends IntentHandler  implements PafConstants {
    ...
    private static final String PERMISSION_RECEIVE_PUBLISHER_UPDATES = "com.motorola.smartactions.permission.RECEIVE_PUBLISHER_UPDATES";

    private static String TAG = PsfConstants.PSF_PREFIX + PafIntentHandler.class.getSimpleName();

    private String publisherUpdateReason;

    public PafIntentHandler(Context context, final Intent intent) {
    ...
    /**
     * A package data clear event occurs when a user selects a "Clear Data" on a package (Android app) from their application list.
     * If that package is a Smart Actions publisher (rule, condition or action publisher) then we need to handle that 
     * condition because that means their states are cleared, that is.
     * 
     * ACTION_PACKAGE_DATA_CLEARED intent is received for the package containing Publishers
        ...
        if(whiteList.contains(pkgName)) {
            ...
            if(!ppPubList.isEmpty()) {
                ...
                if(actionPubList.size() > 0) {
                    ...
                    mContext.sendBroadcast(actPubIntent, PERMISSION_RECEIVE_PUBLISHER_UPDATES);
                }
                ...
                if(condPubList.size() > 0) {
                    ...
                    mContext.sendBroadcast(condPubIntent, PERMISSION_RECEIVE_PUBLISHER_UPDATES);
                }
                ...
                if(rulePubList.size() > 0) {
                    ...
                    mContext.sendBroadcast(rulePubIntent, PERMISSION_RECEIVE_PUBLISHER_UPDATES);
                }
            }
        }
    }
    ...
    private void processDiffPublisherListPokeRv(List<LocalPublisherTuple> modifiedPubList,
        ...
        if(actPubUpdIntent.hasExtra(EXTRA_PUBLISHER_MODIFIED_LIST) ||
            ...
            mContext.sendBroadcast(actPubUpdIntent, PERMISSION_RECEIVE_PUBLISHER_UPDATES);
        }
        ...
        if(condPubUpdIntent.hasExtra(EXTRA_PUBLISHER_MODIFIED_LIST) ||
            ...
            mContext.sendBroadcast(condPubUpdIntent, PERMISSION_RECEIVE_PUBLISHER_UPDATES);
        }
        ...
        if(rulePubUpdIntent.hasExtra(EXTRA_PUBLISHER_MODIFIED_LIST) ||
            ...
            mContext.sendBroadcast(rulePubUpdIntent, PERMISSION_RECEIVE_PUBLISHER_UPDATES);
        }
    }
}
