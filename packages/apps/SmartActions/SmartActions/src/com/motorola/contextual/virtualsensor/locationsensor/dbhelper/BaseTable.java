 ...
 *</pre></code>
 */
public abstract class BaseTable implements DbSyntax {
    ...
    public long insert(Context context, ContentValues values) {
        ...
        try {
            ...
            LSAppLog.e(TAG, ".insert failed, values="+values.toString());
            e.printStackTrace();
     ...
     */
    public static Cursor fetchAll(			SQLiteDatabase mDb,
                                     String tableName,
                                     boolean orderById) {
        ...
        if (LOG_INFO) Log.i(TAG, "fetchAll sql="+ sql+",  getCount()="+cursor.getCount());
        return cursor;
    }
    ...
    public Cursor fetchWhere(SQLiteDatabase mDb,
        ...
        String[] selectionArgs = null;
        String sql = SELECT_ALL+FROM+getTableName();
        ...
        if (cursor != null) {
            ...
            if (LOG_INFO) Log.i(TAG, "fetchWhere sql="+ sql+",  getCount()="+cursor.getCount());
        }
        return cursor;
    }
    ...
    public <T extends BaseTuple> T fetch1(
        ...
        T result = null;
        try {
            String matchType = (matchValue.indexOf(LIKE_WILD) > -1? LIKE : EQUALS);
            cursor = fetchWhere(mDb, lookupColumn +matchType+ Q+matchValue+Q, null);
    ...
    public <T extends BaseTuple> int update(SQLiteDatabase mDb, T tuple) {
        ...
        if (LOG_DEBUG) Log.i(TAG, "DB update result = "+result);
        return result;
    }
     ...
     *</pre></code>
     */
    public static class BaseTuple extends com.motorola.contextual.smartrules.util.ParcelableBase {
         ...
         */
        protected BaseTuple(Parcel parcel) {
            super(parcel);
        }
