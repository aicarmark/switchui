 ...
 *</pre></code>
 */
public class AddRuleListActivity extends ListActivity implements Constants, DbSyntax, 
											ListRowInterface,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mActivity = this;
 		...
 		if(savedInstanceState != null) {
			...
			if(getIntent() != null) {
				if(getIntent().hasExtra(NUM_OF_VISIBLE_RULES)) {
    	...
    	if(resultCode == RESULT_OK) {
    		if(clickedListRow != null) { 
    	        ...
    	        if(requestCode == RULE_PRESET || requestCode == RULE_SUGGESTED) {
	    			...
	    			if(intent != null)
	    				insertedRuleId = intent.getLongExtra(LandingPageIntentExtras.RULE_ID_INSERTED,
	    														DEFAULT_RULE_ID);
		            ...
		            boolean isManualRule = intent.getBooleanExtra(LandingPageIntentExtras.IS_RULE_MANUAL_RULE, true);
		            if(! isManualRule)
		            	RulePersistence.markRuleAsEnabled(mContext, insertedRuleId);
    	        }
        	}
		    LandingPageActivity.startLandingPageActivityWithClearTask(mContext);
            finish();
    	}
	...
	public void setViewValue(int position, View view, final Object data) {
		...
		if(ruleIconString != null)
			ruleIconResId = getResources().getIdentifier(ruleIconString, "drawable", getPackageName());
		...
		if(ruleIcon != null) {
			...
			if(listRowType == LIST_ROW_TYPE_BLANK)
			    ...
			    Drawable iconDrawable = IconPersistence.getIconDrawableFromBlob(this,
    ...
    /** Launch the rule builder activity
    */
    public static void startRuleBuilder(Context ctx) {
        Intent ruleIntent = new Intent(ctx, AddRuleListActivity.class);
        ruleIntent.putExtra(NUM_OF_VISIBLE_RULES, 0);
