...
import com.motorola.providers.contacts.MotContactsDatabaseHelper.MotoCNAPandCVColumns;
...
public class CallLogProvider extends ContentProvider {
    ...
    public static final String SIMTYPE = "simtype";//xukai
...
/* 2012-11-05, NWQ834 merged from Ironprime_TD */
        //MOT MOD BEGIN calling CNAP and CV feature
        sCallsProjectionMap.put(MotoCNAPandCVColumns.CNAP_NAME, MotoCNAPandCVColumns.CNAP_NAME); //add cnap_name colume for 34425 Calling Name Presentation
        //convert number is a phone number,for 33860 feature, some special number should be converted to international number
        //when network is not in VZW CDMA network. For example #3282 should be converted to +18887829687 in non-VZW CDMA nw.
        //As the requierment is : in RC List, should display converted number, but in the current db, only original number is stored,
        //we cann't simply overwrite converted number to original number since we need use original number to do query from contacts,
        //so add this field for RC List display
        sCallsProjectionMap.put(MotoCNAPandCVColumns.CACHED_CONVERT_NUMBER, MotoCNAPandCVColumns.CACHED_CONVERT_NUMBER); // 33860 convert number
        //MOT MOD END
        sCallsProjectionMap.put(NETWORK, NETWORK); // MOT CHINA DMDS
        sCallsProjectionMap.put(SIMTYPE, SIMTYPE);
/* NWQ834 End */
    }
