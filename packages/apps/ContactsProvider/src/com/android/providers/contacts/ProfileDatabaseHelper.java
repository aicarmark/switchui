...
//MOT MOD BEGIN - IKPIM-977
import android.util.Log;

//import com.motorola.android.internal.encryption.EncryptionManager;
//import com.motorola.android.internal.encryption.EncryptionException;
//import com.motorola.android.internal.encryption.EncryptionKeyException;
//MOT MOD END
...
public class ProfileDatabaseHelper extends ContactsDatabaseHelper {
    ...
    public static synchronized ProfileDatabaseHelper getInstance(Context context) {
        if (sSingleton == null) {
            ...
            //MOT MOD BEGIN - IKPIM-977, for feature 35109: Contacts data encryption.
            /*EncryptionManager encManager = (EncryptionManager) context.getSystemService(Context.ENCRYPTION_SERVICE);
            if (encManager != null) {
                    try {
                        encManager.startDbConn(sSingleton);
                        Log.d(TAG, "Got encryptionManager");
                    } catch (EncryptionException e) {
                        Log.e(TAG, "EncryptionException caught starting DbConn with EncryptionManager" + e);
                    } catch (EncryptionKeyException e) {
                        Log.e(TAG, "EncryptionKeyException caught starting DbConn with EncryptionManager" + e);
                    }
            } else {
                    Log.w(TAG, "Failed to get encryption manager");
