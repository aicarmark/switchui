...
/* 2012-11-05, NWQ834 merged from Ironprime_TD */
            if (mContext.getResources().getBoolean(R.bool.cmcc_ot)
                    && ContactsProvider2.isLocalContact(rawContactId)) {
                    String cmccnumber = buildCMCCNumber(number);
                    values.put(Phone.NUMBER, cmccnumber);
...
/* 2012-11-05, NWQ834 merged from Ironprime_TD */
    private String buildCMCCNumber(String number) {
        // Log.v("DataRowHandlerForPhoneNumber", "buildCMCCNumber for CMCC OT");
        if (number == null)
            return "";

        String ret = number.replaceAll("[^0-9()*#PpWw+,;]*", "");
