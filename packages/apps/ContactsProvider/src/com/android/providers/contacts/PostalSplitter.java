...
public class PostalSplitter {
    ...
    // BEGIN Motorola, w21667  07/25/2011, IKCBS-1976 / 35411 Contacts Localization
    private static final String KOREAN_LANGUAGE = Locale.KOREAN.getLanguage().toLowerCase();
    // END IKCBS-1976 / 35411 Contacts Localization
    ...
    public String join(Postal postal) {
        ...
        // BEGIN Motorola, w21667  07/25/2011, IKCBS-1976 / 35411 Contacts Localization
                   ...
                   KOREAN_LANGUAGE.equals(mLocale.getLanguage())) {
            return joinKoKr(postal);
        // END IKCBS-1976 / 35411 Contacts Localization
        } else {
    ...
    // BEGIN Motorola, w21667  07/25/2011, IKCBS-1976 / 35411 Contacts Localization
    private String joinKoKr(final Postal postal) {
        final boolean hasStreet = !TextUtils.isEmpty(postal.street);
        final boolean hasPobox = !TextUtils.isEmpty(postal.pobox);
        final boolean hasNeighborhood = !TextUtils.isEmpty(postal.neighborhood);
        final boolean hasCity = !TextUtils.isEmpty(postal.city);
        ...
        // First block: [postcode][, ][country]
        // Second block: [region][ ][city][ ][street]
        // Third block: [neighborhood][ ][pobox]

        final StringBuilder builder = new StringBuilder();

        final boolean hasFirstBlock = hasPostcode || hasCountry;
        final boolean hasSecondBlock = hasRegion || hasCity || hasStreet;
        final boolean hasThirdBlock = hasNeighborhood || hasPobox;
        ...
        if (hasFirstBlock) {
            ...
            if (hasCountry) {
                if (hasPostcode) builder.append(COMMA + SPACE);
                    builder.append(postal.country);
            }
        }
        ...
        if (hasSecondBlock) {
            ...
            if (hasStreet) {
                if (hasCity) builder.append(SPACE);
                    builder.append(postal.street);
            }
        }
        ...
        if (hasThirdBlock) {
            ...
            if (hasPobox) {
                if (hasNeighborhood) builder.append(SPACE);
                    builder.append(postal.pobox);
            }
        }
