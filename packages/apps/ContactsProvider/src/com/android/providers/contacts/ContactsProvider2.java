...
//MOT MOD BEGIN
import com.motorola.providers.contacts.DataRowHandlerForLocalGroup;
import com.motorola.providers.contacts.MotContactsExtensionsProvider;
...
public class ContactsProvider2 extends AbstractContactsProvider
	...
	//MOT MOD BEGIN
    private static final String LOCAL_GROUP_MIMETYPE = "vnd.android.cursor.item/local_group_membership";
    //MOT MOD END
	...
	//MOT MOD BEGIN
    private static final int RAW_CONTACTS_FILTER = 2009;
	//MOT MOD END
	...
	//MOT MOD BEGIN
    private static final int PHONES_NAME_ONLY_FILTER = 3011;
    private static final int EMAILS_NAME_ONLY_FILTER = 3012;    
	//MOT MOD END
    private static final int CALLABLES = 3013;
    private static final int CALLABLES_ID = 3014;
    private static final int CALLABLES_FILTER = 3015;
    ...
    //MOT MOD BEGIN mot partial field vcard feature
    private static final int PROFILE_PARTIAL_FIELD_AS_VCARD = 19650;
    //MOT MOD END
    ...
    public static final int TYPE_GROUP_LOCAL = 1;
    public static final int TYPE_GROUP_FRIEND = 2;
    public static final int TYPE_GROUP_FAMILY = 3;
    public static final int TYPE_GROUP_COWORKER = 4;
	//MOT MOD END
    ...
    //MOT MOD BEGIN make it public and can be used by MotContactsExtensionsProvider
    public static final ProjectionMap sContactsVCardProjectionMap = ProjectionMap.builder()
    //MOT MOD END
            .add(Contacts._ID)
    ...
    private static final ProjectionMap sRawEntityProjectionMap = ProjectionMap.builder()
            ...
            .addAll(sRawContactSyncColumns)
            .addAll(sDataColumns)
            .add(Constants.RawContacts_IS_RESTRICTED) //MOT MOD - IKPIM-955
            .build();
    ...
    private static final ProjectionMap sEntityProjectionMap = ProjectionMap.builder()
            ...
            .add(Constants.Contacts_Entity_IS_RESTRICTED) //MOT MOD - IKPIM-955
            .build();
    ...
    static {
        ...
        matcher.addURI(ContactsContract.AUTHORITY, "raw_contacts/#/stream_items/#",
                RAW_CONTACTS_ID_STREAM_ITEMS_ID);
		//MOT MOD BEGIN
        matcher.addURI(ContactsContract.AUTHORITY, "raw_contacts/filter/*", RAW_CONTACTS_FILTER);
		//MOT MOD END                
        ...
        matcher.addURI(ContactsContract.AUTHORITY, "data/phones/filter/*", PHONES_FILTER);
		//MOT MOD BEGIN
        matcher.addURI(ContactsContract.AUTHORITY, "data/phones/name_only/filter/*", PHONES_NAME_ONLY_FILTER);
		//MOT MOD END
        ...
        matcher.addURI(ContactsContract.AUTHORITY, "data/emails/filter/*", EMAILS_FILTER);
		//MOT MOD BEGIN
        matcher.addURI(ContactsContract.AUTHORITY, "data/emails/name_only/filter/*", EMAILS_NAME_ONLY_FILTER);
		//MOT MOD END
...
//MOT MOD BEGIN mot partial field vcard feature
        matcher.addURI(ContactsContract.AUTHORITY, "profile/as_partial_field_vcard", PROFILE_PARTIAL_FIELD_AS_VCARD);
//MOT MOD END
    }
    ...
    //MOT MOD BEGIN - IKHSS7-2985
    /**
     * whether feature 35412 phone# middle search is on
     */
    private boolean mFID35412On;
    //MOT MOD END - IKHSS7-2985
	...
	//MOT MOD BEGIN
    MotContactsExtensionsProvider mMotExtensions;
	//MOT MOD END
    ...
    private boolean initialize() {
        ...
        mFID35412On = resources.getBoolean(
                R.bool.config_fid35412_on);
        //MOT MOD END - IKHSS7-2985
        ...
        try{
            ...
            Log.e(TAG, "get writeable Database faile = " + e);
        }
        ...
        //MOT MOD BEGIN
        mMotExtensions = new MotContactsExtensionsProvider(this);
        mMotExtensions.onCreate( mContactsHelper ); //IKHSS7-2898
        //MOT MOD END
        return true;
    }
	//MOT MOD BEGIN
    private void createLocalGroups() {
        // per CT request, the preload local group can be deleted by user.
        // so it is user gruop, we only create it for user one time.
        // if user delete it later, we don't create it again for user.
        if(mDb == null){
            try{
                mDb = mContactsHelper.getWritableDatabase();
            } catch (Exception e){
        ...
        final long accountId = mDbHelper.get().getOrCreateAccountIdInTransaction(
                AccountWithDataSet.get(ACCOUNT_LOCAL_DEVICE, ACCOUNT_TYPE_LOCAL, null));
        Cursor c = mDb.query(Tables.GROUPS, new String[]{Groups._ID},
                             GroupsColumns.ACCOUNT_ID + " = " + accountId,
                             null, null, null, null);
        try {
        	//modify by txbv34 for IKCBSMMCPPRC-1312&IKCBSMMCPPRC-1481
            if (c != null && c.moveToFirst()) {
                // the group table has been ready, the preload table must have been set before
            } else {
                createSystemGroup(TYPE_GROUP_FRIEND);
                createSystemGroup(TYPE_GROUP_FAMILY);
                createSystemGroup(TYPE_GROUP_COWORKER);
            }
        } finally {
        	if (c != null)c.close();
            mActiveDb.set(null);
    ...
    private void createSystemGroup(int type) {
        final Context context = getContext();
        final String title = context.getString(getTitleResource(type));
        final ContentValues newGroup = new ContentValues();
        newGroup.put(Groups.TITLE, title);
        newGroup.put(Groups.GROUP_VISIBLE, "1");
        newGroup.put(RawContacts.ACCOUNT_NAME, ACCOUNT_LOCAL_DEVICE);
        newGroup.put(RawContacts.ACCOUNT_TYPE, ACCOUNT_TYPE_LOCAL);
        long groupId = insertGroup(Groups.CONTENT_URI, newGroup, true);
    }

    protected int getTitleResource(int type) {
        switch (type) {
            case TYPE_GROUP_LOCAL: return R.string.local_system_group;
            case TYPE_GROUP_FRIEND: return R.string.friend_system_group;
            case TYPE_GROUP_FAMILY: return R.string.family_system_group;
            case TYPE_GROUP_COWORKER: return R.string.coworker_system_group;
            default: return R.string.local_system_group;
        }
    }

    private void  createCTCustomerServiceNumber(){
        if(mDb == null){
            try{
                mDb = mContactsHelper.getWritableDatabase();
            } catch (Exception e){
        ...
        if (mDb == null) return;
        mActiveDb.set(mDb);
        Cursor c = mDb.query(Tables.CONTACTS,new String[]{Contacts._ID},null,null,null,null,null);
        try {
            if (c != null && c.moveToFirst()) {
                // the contacts table is not empty, so we do not set the ct number
                return;
            } else {
                ContentValues values = new ContentValues();
                values.put(RawContacts.ACCOUNT_NAME, ACCOUNT_LOCAL_DEVICE);
                values.put(RawContacts.ACCOUNT_TYPE, ACCOUNT_TYPE_LOCAL);
                long rawContactId= insertRawContact(RawContacts.CONTENT_URI, values, false);
                values.clear();
                final Context context = getContext();
                values.put(StructuredName.DISPLAY_NAME,  context.getString(R.string.ct_customer_service_number));
                ...
                values.put(Phone.NUMBER, "18918910000");
                ...
                Log.d(TAG, "CT Customer Service Number is created");
                return;
            }
        } finally{
            c.close();
            // Clear out the active database because it is empty before we set.
            mActiveDb.set(null);
        }
    }
    //MOT MOD END
    ...
    private void initForDefaultLocale() {
        ...
        switchToContactMode();
        //MOT MOD BEGIN
        // Preload local groups for China products
        createLocalGroups();
        // Preload China Carrier service numbers
        if (getContext().getResources().getText(R.string.carrier_name).toString().equals("CT")) {
            createCTCustomerServiceNumber();
        }
        //MOT MOD END
    }
    ...
    private void initDataRowHandlers(Map<String, DataRowHandler> handlerMap,
        ...
        // MOT MOD BEGIN -- Local group mime type
        handlerMap.put(LOCAL_GROUP_MIMETYPE,
                new DataRowHandlerForLocalGroup(context, dbHelper, contactAggregator,
                        mGroupIdCache));
        // MOT MOD END
        ...
        switch (task) {
            case BACKGROUND_TASK_INITIALIZE: {
                try{
                    ...
                    Log.e(TAG, "performBackgroundTask  INITIALIZE failed = " + e);
                }
                break;
            }
    ...
    protected void cleanupPhotoStore() {
        ...
        Cursor c = db.query(mDbHelper.get().getDataView(), new String[]{Data._ID, Photo.PHOTO_FILE_ID}, //MOT MOD - IKPIM-955
    ...
    protected Uri insertInTransaction(Uri uri, ContentValues values) {
        ...
        //MOT MOD BEGIN
        if  (mMotExtensions != null && mMotExtensions.isUriMatched(uri)) {
            return mMotExtensions.insertInTransaction(mActiveDb.get(), uri, values);
        }
        //MOT MOD END
    ...
    private Account resolveAccount(Uri uri, ContentValues values) throws IllegalArgumentException {
        ...
        if (validValues && validUri) {
            ...
            values.put(RawContacts.ACCOUNT_NAME, ACCOUNT_LOCAL_DEVICE);
            values.put(RawContacts.ACCOUNT_TYPE, ACCOUNT_TYPE_LOCAL);
            accountName = ACCOUNT_LOCAL_DEVICE;
    ...
    private int deleteData(String selection, String[] selectionArgs, boolean callerIsSyncAdapter) {
        ...
        try {
            while(c.moveToNext()) {
                ...
                // MOTO MOD BEGIN -- IKPIM-935 group redesign to avoid local group to be deleted.
                if (callerIsSyncAdapter) {
                    if (mimeType
                            .equals(LOCAL_GROUP_MIMETYPE)) {
                        continue;
                    }
                }
                // MOT MOD END
    ...
    public long insertStatusUpdate(ContentValues values) {
        ...
        mSb.append(" AND ").append(getContactsRestrictions()); //MOT MOD - IKPIM-955
    ...
    protected int deleteInTransaction(Uri uri, String selection, String[] selectionArgs) {
        ...
        //MOT MOD BEGIN
        if  (mMotExtensions != null && mMotExtensions.isUriMatched(uri)) {
            return mMotExtensions.deleteInTransaction(mActiveDb.get(), uri, selection, selectionArgs);
        }
        //MOT MOD END
        ...
        switch (match) {
            ...
            case PROFILE_RAW_CONTACTS: {
                invalidateFastScrollingIndexCache();
                int numDeletes = 0;
                Cursor c = mActiveDb.get().query(Views.RAW_CONTACTS_ALL,
            ...
            case GROUPS: {
                int numDeletes = 0;
                Cursor c = mActiveDb.get().query(Views.GROUPS_ALL, Projections.ID,
                        appendAccountIdToSelection(uri, selection), selectionArgs,
                        null, null, null);
                try {
                    while (c.moveToNext()) {
    ...
    public int deleteGroup(Uri uri, long groupId, boolean callerIsSyncAdapter) {
        ...
        // MOTO MOD BEGIN -- use new mime type string for local group
        final long LocalgroupMembershipMimetypeId = mDbHelper.get()
                .getMimeTypeId(LOCAL_GROUP_MIMETYPE);

        mActiveDb.get().delete(Tables.DATA, "(" + DataColumns.MIMETYPE_ID + "="
                + groupMembershipMimetypeId + " OR " + DataColumns.MIMETYPE_ID + "="
                + LocalgroupMembershipMimetypeId + ") " + " AND " + GroupMembership.GROUP_ROW_ID + "="
                + groupId, null);
        // MOTO MOD END
    ...
    private int deleteStreamItems(Uri uri, ContentValues values, String selection,
        ...
        int count = 0;
        final Cursor c = mActiveDb.get().query(Views.STREAM_ITEMS_ALL, Projections.ID,
    ...
    protected int updateInTransaction(Uri uri, ContentValues values, String selection,
        ...
        //MOT MOD BEGIN
        if  (mMotExtensions != null && mMotExtensions.isUriMatched(uri)) {
            return mMotExtensions.updateInTransaction(mActiveDb.get(), uri, values, selection, selectionArgs);
        }
        //MOT MOD END
    ...
    private int updateRawContacts(ContentValues values, String selection, String[] selectionArgs,
        ...
        int count = 0;
        Cursor cursor = mActiveDb.get().query(mDbHelper.get().getRawContactView(), //MOT MOD - IKPIM-955
    ...
    private int updateContactOptions(ContentValues values, String selection,
            String[] selectionArgs, boolean callerIsSyncAdapter) {
        int count = 0;
        Cursor cursor = mActiveDb.get().query(mDbHelper.get().getContactView(), //MOT MOD - IKPIM-955
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
            String sortOrder) {
    	if (VERBOSE_LOGGING) {
    		Log.v(TAG, "contacts provider query2");
    	}
    ...
    protected Cursor queryLocal(final Uri uri, final String[] projection, String selection,
    	...
    	if(VERBOSE_LOGGING) {
    		Log.v(TAG, "contacts provider queryLocal");
    	}
        ...
        //MOT MOD BEGIN
        if  (mMotExtensions != null && mMotExtensions.isUriMatched(uri)) {
            return mMotExtensions.query(mActiveDb.get(), uri, projection, selection, selectionArgs, sortOrder);
        }
        //MOT MOD END
        ...
        switch (match) {
            ...
            case CONTACTS_AS_VCARD: {
                ...
                long contactId = lookupContactIdByLookupKey(mActiveDb.get(), lookupKey);
                qb.setTables(mDbHelper.get().getContactView(true /* require restricted */)); //MOT MOD - IKPIM-955
            ...
            case CONTACTS_STREQUENT: {
                ...
                if (phoneOnly) {
                    ...
                    tableBuilder.append(Tables.DATA_USAGE_STAT
                            + " INNER JOIN " + mDbHelper.get().getDataView() + " " + Tables.DATA //MOT MOD - IKPIM-955
                    ...
                    if (searchPhoneNumber) {
                        ...
                        if (!TextUtils.isEmpty(number)) {
                            ...
                            sb.append(Data._ID +
                                ...
                                + " WHERE " + PhoneLookupColumns.NORMALIZED_NUMBER + " LIKE '%"); // Motorola, w21624 08/05/2011, IKCBS-2112 / FID 35412 Korean Contacts Search
                            sb.append(number);
                            sb.append("%')");
                            hasCondition = true;
                        }
            ...
            case PHONES_NAME_ONLY_FILTER: {
                ...
                if (uri.getPathSegments().size() > 3) {
                    ...
                    if (!hasCondition) {
                        // If it is not a name, the query should return
                        // an empty cursor.  Let's ensure that.
                        sb.append("0");
                    }
                    sb.append(")");
            ...
            case RAW_CONTACTS_FILTER: {
                setTablesAndProjectionMapForRawContacts(qb, uri); //MOTO CONTACTS
                if (uri.getPathSegments().size() > 2) {
                    String filterParam = uri.getLastPathSegment();
                    StringBuilder sb = new StringBuilder();
                    //Log.v(TAG, "query(), case- RAW_CONTACTS_FILTER:  filterParam="+filterParam+", uri="+uri+", sb="+sb.toString()+", RawContacts._ID="+RawContacts._ID+", Contacts._ID="+Contacts._ID+".");
                    sb.append(RawContacts._ID + " IN ");
                    sb.append("(SELECT DISTINCT " + NameLookupColumns.RAW_CONTACT_ID + " FROM " + Tables.NAME_LOOKUP +
...
//MOT MOD END
            ...
            case PROFILE_DATA: {                
            	//MOT MOD BEGIN
                groupBy = getQueryParameter(uri, "GROUP_BY_PRIMARY_KEY");   // MOT CHINA

                if (!TextUtils.isEmpty(groupBy)) {                	
                    List<String> projectiondList = new ArrayList<String>();
                    for (String member : projection) {
                        projectiondList.add(member);
                    }
                    String projectionListStr = TextUtils.join(",", projectiondList);
                	//Log.v(TAG, "projectionListStr: "+ projectionListStr);
                	                	
                	String sortedDataQuery = "SELECT " + projectionListStr 
                	                         + " FROM " + mDbHelper.get().getDataView(shouldExcludeRestrictedData(uri))
                	                         + " WHERE " + selection 
                	                         + " ORDER BY IS_SUPER_PRIMARY ASC, IS_PRIMARY ASC ";
                	
                	String select = "SELECT * FROM (" + sortedDataQuery + ") GROUP BY " + groupBy;
                	
                	//Log.v(TAG, "rawQuery(DAT), select: "+select);
                    // query only the default ids (is_super_primary, is_primary)
                    return mActiveDb.get().rawQuery(select, selectionArgs);                       
                } else {
                    //Mot begin IKHSS7-8399
                    groupBy = getQueryParameter(uri, "GROUP_BY_KEY");  
                    //Mot end IKHSS7-8399
                }
                //MOT MOD END
                setTablesAndProjectionMapForData(qb, uri, projection, false);                
            ...
            case GROUPS: {
                qb.setTables(Views.GROUPS_ALL); //MOT MOD - IKPIM-955
            ...
            case GROUPS_SUMMARY: {
                String tables = Views.GROUPS_ALL + " AS " + Tables.GROUPS; //MOT MOD - IKPIM-955
    ...
    //MOT MOD BEGIN make query public and can be called by MotContactsExtensionsProvider
    ...
    //MOT MOD END
    	...
    	if(VERBOSE_LOGGING) {
    		Log.v(TAG, "contacts provider query1");
    	}
...
/*
                //MOT MOD BEGIN - IKSTABLE6-17513
                if (TextUtils.equals(title, "\u0141") &&
                    (TextUtils.equals(locale, "pl_PL") || TextUtils.equals(locale, "el_GR"))) {
                    title = "L";
                }
                //MOT MOD END - IKSTABLE6-17513
*/
                if (indexCount == 0 || !TextUtils.equals(title, currentTitle)) {
/*
                    //MOT MOD BEGIN - IKHSS6UPGR-4524
                    if (indexCount > 0 &&
                            locale.compareToIgnoreCase("el_GR") == 0 &&
                            !TextUtils.isEmpty(title) &&
                            !TextUtils.isPrintableAscii(title.charAt(0))) {
                        counts[indexCount - 1] += count;
                    } else {
                    //MOT MOD END - IKHSS6UPGR-4524
*/
                        titles[indexCount] = currentTitle = title;
                        counts[indexCount] = count;
                        indexCount++;
//                    } //MOT MOD - IKHSS6UPGR-4524
                } else {
                    counts[indexCount - 1] += count;
                }
            }
    ...
    private interface LookupBySourceIdQuery {
        //String TABLE = Views.RAW_CONTACTS;  //MOT MOD - IKPIM-955
    ...
    private long lookupContactIdBySourceIds(SQLiteDatabase db,
        ...
        Cursor c = db.query(mDbHelper.get().getRawContactView(), LookupBySourceIdQuery.COLUMNS, //MOT MOD - IKPIM-955
    ...
    private long lookupContactIdByRawContactIds(SQLiteDatabase db,
        ...
        Cursor c = db.query(mDbHelper.get().getRawContactView(), LookupByRawContactIdQuery.COLUMNS, //MOT MOD - IKPIM-955
                 sb.toString(), null, null, null, null);
        try {
            while (c.moveToNext()) {
    ...
    private void setTablesAndProjectionMapForContacts(SQLiteQueryBuilder qb, Uri uri,
        ...
        if (includeDataUsageStat) {
            sb.append(mDbHelper.get().getDataUsageStatView() + " AS " + Tables.DATA_USAGE_STAT); //MOT MOD - IKPIM-955
        ...
        if (includeDataUsageStat) {
            sb.append(" ON (" +
                    DbQueryUtils.concatenateClauses(
                            ...
                            RawContacts.CONTACT_ID + "=" + viewName + "." + Contacts._ID) +  //MOT MOD - IKPIM-955
                    ")");
        }
    ...
    private void setTablesAndProjectionMapForContactsWithSnippet(SQLiteQueryBuilder qb, Uri uri,
        ...
        StringBuilder sb = new StringBuilder();
        sb.append(mDbHelper.get().getContactView(shouldExcludeRestrictedData(uri))); //MOT MOD - IKPIM-955
    ...
    public void appendSearchIndexJoin(StringBuilder sb, String filter,
        ...
        // validate the filter string to avoid crash
        Log.v(TAG, "Validate FILTER: orig_filter = "+filter+", snippetNeeded = "+snippetNeeded+", isEmailAddress = "+isEmailAddress);
        filter = filter.replaceAll("[\\[]", "\\\\[");   // [ --> \[
        filter = filter.replaceAll("[\\]]", "\\\\]");   // ] --> \]
        Log.v(TAG, "Validated FILTER  : after_filter = "+filter);

        //MOT MOD BEGIN - IKHSS7-2985
        if (!(mFID35412On && isPhoneNumber)) {
        ...
        if (snippetNeeded) {
            ...
            if (isEmailAddress) {
                ...
                //MOT MOD BEGIN - IKHSS7-2985
                if (mFID35412On) {
                    sb.append(" JOIN (SELECT distinct " + RawContactsColumns.CONCRETE_CONTACT_ID + " AS " + SNIPPET_CONTACT_ID);
                    sb.append(", ");
                    DatabaseUtils.appendEscapedSQLString(sb, startMatch);
                    sb.append("||");
        ...
        if (!(mFID35412On && isPhoneNumber)) sb.append("'"); //MOT MOD- IKHSS7-2985

        sb.append(" AND " + SNIPPET_CONTACT_ID + " IN " + Tables.DEFAULT_DIRECTORY + ")"); //MOT MOD- IKHSS7-2985
    ...
    private void setTablesAndProjectionMapForRawContacts(SQLiteQueryBuilder qb, Uri uri) {
        StringBuilder sb = new StringBuilder();
        sb.append(mDbHelper.get().getRawContactView(shouldExcludeRestrictedData(uri))); //MOT MOD - IKPIM-955
        qb.setTables(sb.toString());
    ...
    private void setTablesAndProjectionMapForRawEntities(SQLiteQueryBuilder qb, Uri uri) {
        qb.setTables(mDbHelper.get().getRawEntitiesView(shouldExcludeRestrictedData(uri))); //MOT MOD - IKPIM-955
     ...
     */
    //MOT MOD BEGIN make it public and can be used by MotContactsExtensionsProvider
    public void setTablesAndProjectionMapForData(SQLiteQueryBuilder qb, Uri uri,
    //MOT MOD END
    ...
    private void setTablesAndProjectionMapForData(SQLiteQueryBuilder qb, Uri uri,
        ...
        StringBuilder sb = new StringBuilder();
        sb.append(mDbHelper.get().getDataView(shouldExcludeRestrictedData(uri))); //MOT MOD - IKPIM-955
        sb.append(" data");
    ...
    private void setTableAndProjectionMapForStatusUpdates(SQLiteQueryBuilder qb,
            String[] projection) {
        StringBuilder sb = new StringBuilder();
        sb.append(mDbHelper.get().getDataView()); //MOT MOD - IKPIM-955
        sb.append(" data");
        appendDataPresenceJoin(sb, projection, DataColumns.CONCRETE_ID);
        appendDataStatusUpdateJoin(sb, projection, DataColumns.CONCRETE_ID);
    ...
    private void setTablesAndProjectionMapForStreamItems(SQLiteQueryBuilder qb) {
        qb.setTables(mDbHelper.get().getStreamItemsView()); //MOT MOD - IKPIM-955
    ...
    private void setTablesAndProjectionMapForEntities(SQLiteQueryBuilder qb, Uri uri,
            String[] projection) {
        StringBuilder sb = new StringBuilder();
        sb.append(mDbHelper.get().getEntitiesView(shouldExcludeRestrictedData(uri))); //MOT MOD - IKPIM-955
        sb.append(" data");
    ...
    private boolean shouldExcludeRestrictedData(Uri uri) {
        ...
        boolean excludeRestrictedData = readBooleanQueryParameter(uri,
                Constants.Data_FOR_EXPORT_ONLY, false);
        if (excludeRestrictedData) {
            return true;
        }

        String requestingPackage = getQueryParameter(uri,
                Constants.ContactsContract_REQUESTING_PACKAGE_PARAM_KEY);
    ...
    String getContactsRestrictions() {
        if (mDbHelper.get().hasAccessToRestrictedData()) {
            ...
            return Constants.RawContactsColumns_CONCRETE_IS_RESTRICTED + "=0";
        }
    }
    ...
    public String getContactsRestrictionExceptionAsNestedQuery(String contactIdColumn) {
        if (mDbHelper.get().hasAccessToRestrictedData()) {
            ...
            return "(SELECT " + Constants.RawContacts_IS_RESTRICTED + " FROM " + Tables.RAW_CONTACTS
    ...
    public AssetFileDescriptor openAssetFileLocal(Uri uri, String mode)
        ...
        //MOT MOD BEGIN
        if  (mMotExtensions != null && mMotExtensions.isUriMatched(uri)) {
            return mMotExtensions.openAssetFile(mActiveDb.get(), uri, mode);
        }
        //MOT MOD END
        ...
        switch (match) {
            ...
            case CONTACTS_AS_MULTI_VCARD: {
                //MOT MOD BEGIN - IKHSS6-1328, when too many contacts be selected in multi-picker, session id will be sent
                //back as result, here is how to handle the session id, porting this from TABLET-MAIN
                //google original code
                /*
                final String lookupKeys = uri.getPathSegments().get(2);
                final String[] loopupKeyList = lookupKeys.split(":");*/
                final String[] loopupKeyList;

                String sessionIdStr = uri.getQueryParameter("multiple_picker_session_id");
                if(sessionIdStr != null){//need to read the lookup key from the multiple picker provider
                    final long sessionId = Long.valueOf(sessionIdStr);
                    ...
                    final Cursor cursor = getContext().getContentResolver().query(sessionUri, LOOKUP_URI_PROJECTION, null, null, null);
                    ArrayList<String> lookupkeyArrayList = new ArrayList<String>();
                    try{
                        if (cursor == null || !cursor.moveToFirst()) {
                            Log.e(TAG, "error in reading the multiplepicker uri: " + sessionUri);
                            throw new FileNotFoundException(mDbHelper.get().exceptionMessage("File does not exist",
                                    uri));
                        }
                        ...
                        for(;!cursor.isAfterLast(); cursor.moveToNext()) {
                            ...
                            if(segmentCount >= 4){
                                String lookupKey = pathSegments.get(2);
                                lookupKey = Uri.encode(lookupKey); // encode to get the raw lookup key because getPathSegments will decode uri
                                lookupkeyArrayList.add(lookupKey);
                            }else{
                    ...
                    loopupKeyList = lookupkeyArrayList.toArray(new String[lookupkeyArrayList.size()]);
                }else{//get the lookup keys from the uri directly
                    final String lookupKeys = uri.getPathSegments().get(2);
                    loopupKeyList = lookupKeys.split(":");
                }
                //MOTO MOD END - IKHSS6-1328
    ...
    private AssetFileDescriptor openPhotoAssetFile(SQLiteDatabase db, Uri uri, String mode,
        ...
        String sql =
                "SELECT " + Photo.PHOTO + " FROM " + mDbHelper.get().getDataView() + //MOT MOD - IKPIM-955
                " WHERE " + selection;
        try {
     ...
     */
     //MOT MOD BEGIN make it public and can be used by MotContactsExtensionsProvider
    public AssetFileDescriptor buildAssetFileDescriptor(ByteArrayOutputStream stream) {
    //MOT MOD END
        try {
            stream.flush();
    ...
    private void outputRawContactsAsVCard(Uri uri, OutputStream stream,
        ...
        Writer writer = null;
        //MOTO MOD BEGIN ,IKHSS6UPGR-8989
        Uri rawContactsUri;
        //MOTO MOD END
        if (mapsToProfileDb(uri)) {
        ...
        //MOTO MOD BEGIN ,IKHSS6UPGR-8989
        rawContactsUri = rawContactsUri.buildUpon().appendQueryParameter("for_export_only", "1").build();
        //MOTO MOD END
        try {
    ...
    public String getType(Uri uri) {
        Log.v(TAG, "contacts provider getType");
        //MOT MOD BEGIN
        String sType = mMotExtensions.getType(uri);
        if (sType!=null) {
            return sType;
        }
        //MOT MOD END
     ...
     */
    //MOT MOD BEGIN
    public String[] insertSelectionArg(String[] selectionArgs, String arg) {
        //MOTO CONTACTS IKSTABLETWO-2802
        if (arg == null) {
            arg = "";
        }
        //MOT MOD END
    ...
    protected Account getDefaultAccount() {
        ...
        try {
            // Moto CHINA default to LOCAL
            Account[] accounts = accountManager.getAccountsByType(ACCOUNT_TYPE_LOCAL);
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        Log.v(TAG, "contacts provider dump");
	...
	//MOT MOD BEGIN
    public static boolean isLocalContact(long rawContactId) {
        if (mDb == null) return false;

        boolean isLocal = false;
        Cursor c = mDb.query(RawContactsQuery.TABLE, RawContactsQuery.COLUMNS,
                RawContacts._ID + "=" + rawContactId, null, null, null, null);
        try {
            if (c.moveToNext()) {
                final String accountType = c.getString(RawContactsQuery.ACCOUNT_TYPE);
	...
	//MOT MOD END
    //MOT MOD BEGIN - IKPIM-955
    public ContactsDatabaseHelper getDelegateDatabaseHelper() {
        return mDbHelper.get();
    }
    //MOT MOD END - IKPIM-955
}
