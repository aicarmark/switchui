...
public class VCardComposer {
    ...
    public VCardComposer(final Context context, ContentResolver resolver,
        ...
        // BEGIN Motorola, w21765, 22/09/2011, IKCBS-2373 / FID37602-Support SHIFT_JIS in vCard
        /*
          * In case of au mobile device (KDDI as carrier of Japan),
          * CHARSET should be defined to each property even to use Vcard 3.0 format
          * If vCard version is 3.0 and Japanese locale is supported and
          * Operator is KDDI, Charset will be set as Shift-JIS
          */
        if (VCardConfig.isVersion30(vcardType) &&
                TextUtils.isEmpty(charset) &&
                Arrays.asList(Locale.getAvailableLocales()).contains(Locale.JAPAN) &&
    ...
    private String createOneEntryInternal(final String contactId,
        ...
        Cursor cursor = mContentResolver.query(Contacts.CONTENT_URI, null, "_id=" + contactId, null, null);      
        int fdId = -1;
        if (cursor != null) {
    		if (cursor.getCount() > 0 && cursor.moveToFirst()) {
    			 fdId = cursor.getInt(cursor.getColumnIndexOrThrow(Contacts.PHOTO_FILE_ID));
    		}
    	 	try {
    	 		cursor.close();
        ...
        if(mGroups != null){
        	builder.setGroups(mGroups);
        }
        
        String displayName = getNamebyRawID(getRawIDbyContactID(contactId));
        Log.w(LOG_TAG, "createOneEntryInternal displayName: " + displayName);
        builder.appendNameProperties(contentValuesListMap.get(StructuredName.CONTENT_ITEM_TYPE),displayName)
                .appendNickNames(contentValuesListMap.get(Nickname.CONTENT_ITEM_TYPE))
                .appendPhones(contentValuesListMap.get(Phone.CONTENT_ITEM_TYPE), mPhoneTranslationCallback)
                .appendEmails(contentValuesListMap.get(Email.CONTENT_ITEM_TYPE))
                .appendPostals(contentValuesListMap.get(StructuredPostal.CONTENT_ITEM_TYPE))
                .appendOrganizations(contentValuesListMap.get(Organization.CONTENT_ITEM_TYPE))
                .appendWebsites(contentValuesListMap.get(Website.CONTENT_ITEM_TYPE));
        //builder.appendPhotos(contentValuesListMap.get(Photo.CONTENT_ITEM_TYPE));  remove for SWITCHUITWOV-74 or IKCBSMMCPPRC-1676
        builder.appendNotes(contentValuesListMap.get(Note.CONTENT_ITEM_TYPE))
                .appendEvents(contentValuesListMap.get(Event.CONTENT_ITEM_TYPE))
                .appendIms(contentValuesListMap.get(Im.CONTENT_ITEM_TYPE))
                .appendRelation(contentValuesListMap.get(Relation.CONTENT_ITEM_TYPE))
                .appendCategories(contentValuesListMap.get(GroupMembership.CONTENT_ITEM_TYPE));
        if((mPhotoPath != null) && (fdId > 0)){
        	builder.appendPhotoFileId(mContentResolver,Integer.toString(fdId), mPhotoPath);
        }
        return builder.toString();
    }

	public int getRawIDbyContactID(String contactId){
        Log.e(LOG_TAG, "getRawIDbyContactID() contactID: " + contactId);

	    String project[] = new String[]{Contacts.NAME_RAW_CONTACT_ID};
	    Cursor cursor = mContentResolver.query(Contacts.CONTENT_URI, project, "_id=" + contactId, null, null);
	    if (cursor == null) {
		return -1;
	    }
	    ...
	    int rawId = cursor.getInt(cursor.getColumnIndexOrThrow(Contacts.NAME_RAW_CONTACT_ID));
        Log.e(LOG_TAG, "getRawIDbyContactID() rawId: " + rawId);
	...
	public String getNamebyRawID(int rawID){
        Log.e(LOG_TAG, "getNamebyRawID() rawID: " + rawID);

	    String project[] = new String[]{RawContacts.DISPLAY_NAME_PRIMARY};
	    Cursor cursor = mContentResolver.query(RawContacts.CONTENT_URI, project, RawContacts._ID+ "=" + rawID, null, null);
	    if (cursor == null) {
		return "";
	    }
	    ...
	    String name = cursor.getString(cursor.getColumnIndexOrThrow(RawContacts.DISPLAY_NAME_PRIMARY));
        Log.e(LOG_TAG, "getNamebyRawID() name: " + name);
    ...
    public String buildVCard(final Map<String, List<ContentValues>> contentValuesListMap) {
        if (contentValuesListMap == null) {
            ...
            builder.appendNameProperties(contentValuesListMap.get(StructuredName.CONTENT_ITEM_TYPE),null)
                    .appendNickNames(contentValuesListMap.get(Nickname.CONTENT_ITEM_TYPE))
            ...
            builder.appendNotes(contentValuesListMap.get(Note.CONTENT_ITEM_TYPE))
                    ...
                    .appendGeo(contentValuesListMap.get(Geo.CONTENT_ITEM_TYPE)) //Feature 36508
                    .appendRelation(contentValuesListMap.get(Relation.CONTENT_ITEM_TYPE))
            		.appendCategories(contentValuesListMap.get(GroupMembership.CONTENT_ITEM_TYPE));
            // END MOT ICS UPMERGE
            return builder.toString();
