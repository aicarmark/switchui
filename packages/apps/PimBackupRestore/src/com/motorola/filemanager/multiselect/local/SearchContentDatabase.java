...
public class SearchContentDatabase {
  ...
  private Cursor query(String selection, String[] selectionArgs, String[] columns) {
    ...
    Cursor cursor =
        builder.query(mDatabaseOpenHelper.getReadableDatabase(), columns, selection, selectionArgs,
            null, null, null);
    ...
    FileManagerApp.log(TAG + " query() returned cursor with size=" + cursor.getCount());
    return cursor;
  }
  ...
  private /*static*/ class DictionaryOpenHelper extends SQLiteOpenHelper {
    ...
    private static final String FTS_TABLE_DELETE = "DELETE FROM " + FTS_VIRTUAL_TABLE;
    ...
    private class loadAllPaths extends Thread {
    public loadAllPaths() {
      super();
    }   
    @Override
//    private void loadAllPaths() {
    public void run(){
      File rootDirectory = new File(FileManagerApp.getInstance(mContext).SD_CARD_DIR);
      FileManagerApp.log(TAG + "a new thread to loadAllPaths() " + rootDirectory);
       addObserver(FileManagerApp.getInstance(mContext).SD_CARD_DIR);
      scanDirectory(rootDirectory);
      FileManagerApp.log(TAG + ":loadAllPaths(), after scan rootDirectory mFileList.size= " +
          mFileList.size());
      if (SdcardManager.hasInternalSdcard(mContext)) {
        File rootDirectory_emmc = new File(FileManagerApp.getInstance(mContext).SD_CARD_EXT_DIR);
        FileManagerApp.log(TAG + ":loadAllPaths() /mnt/sdcard-ext " + rootDirectory_emmc);
         addObserver(FileManagerApp.getInstance(mContext).SD_CARD_EXT_DIR);
        scanDirectory(rootDirectory_emmc);
        FileManagerApp.log(TAG + ":loadAllPaths(), after scan rootDirectory_emmc mFileList.size= " +
            mFileList.size());
      }
      /*
       * for (IconifiedText currentFile : mFileList) { long id =
       * addRecord(currentFile.getText().trim(),
       * currentFile.getPathInfo().trim()); if (id < 0) { FileManagerApp.log(TAG
       * +":unable to add path: " + currentFile.getText().trim()); }
       * FileManagerApp
    ...
    private void scanDirectory(File directory) {
      ...
      if (files != null) {
        for (File currentFile : files) {
          IconifiedText item = IconifiedText.buildIconItem(mHelperContext, currentFile);
          long id = addRecord(item.getText().trim(), item.getPathInfo().trim());
          if (id < 0) {
        ...
        FileManagerApp.log(TAG + ":scanDirectory(),files is null, " + directory + "is empty");
      }
    }
    public long addRecord(String folder, String full_path) {
      ...
      public void deleteRecord( String full_path) {
        if ( mDatabase == null){
          FileManagerApp.log(TAG+"database is not created or opened yet.");
          return ;
        }
        ...
        if (isDirectory == true){
          ...
          int numDeleted = mDatabase.delete(FTS_VIRTUAL_TABLE, KEY_FULL_PATH + "=?", deleteArgs);
          FileManagerApp.log(TAG+"called delete to delete directory="+full_path
            +", number of deleted="+numDeleted);
        }
        else{
          String[] deleteArgs = new String[] {full_path};
          int numDeleted = mDatabase.delete(FTS_VIRTUAL_TABLE, KEY_FULL_PATH + "=?", deleteArgs);
          FileManagerApp.log(TAG+"called delete to delete file="+full_path
            +", number of deleted="+numDeleted);
        }
      }
      ...
      public void addObserver( String parent_path) {
        synchronized(this){
          if ( !m_directoryObservers.containsKey(parent_path)){
            int eventMask = FileObserver.CREATE
              | FileObserver.DELETE
              | FileObserver.MOVED_FROM
              | FileObserver.MOVED_TO
              | FileObserver.DELETE_SELF
              | FileObserver.MOVE_SELF;
            SearchContentDatabase.DirectoryObserver directoryObserver =
              new SearchContentDatabase.DirectoryObserver(this, parent_path, eventMask);
            m_directoryObservers.put(parent_path,directoryObserver);
            directoryObserver.startWatching();
              FileManagerApp.log(TAG + " Added FileObserver for "+parent_path);
          }
        }
      }
        ...
        synchronized(this){
          if ( m_directoryObservers.containsKey(parent_path)){
            int eventMask = FileObserver.CREATE
              ...
              | FileObserver.MOVE_SELF;
            SearchContentDatabase.DirectoryObserver directoryObserver =
            m_directoryObservers.remove(parent_path);
            if (directoryObserver!=null){
              directoryObserver.stopWatching();
            }
            FileManagerApp.log(TAG + " Removed FileObserver for "+parent_path);
          }
        }
      }
 ...
 private class DirectoryObserver extends FileObserver  {
      ...
      public DirectoryObserver(DictionaryOpenHelper _database, String path) {
        ...
        m_parentPath = path;
      //FileManagerApp.log(TAG + " " + "created observer for parent="+path);
    }
    public DirectoryObserver(DictionaryOpenHelper _database, String path, int mask) {
      super(path, mask);
      m_database = _database;
      m_parentPath = path;
      //FileManagerApp.log(TAG + " " + "created observer for parent="+path);
      ...
      if (( createEvent != 0) || ( moved_toEvent != 0)){
        ...
        File currentFile = new File(m_parentPath+"/"+path);
          if ( currentFile.exists() == false ){
            //FileManagerApp.log(TAG + " onEvent(): cant get file from path="+m_parentPath+"/"+path);
            return;
          }
          IconifiedText item =
            IconifiedText.buildIconItem(m_database.getContext(), currentFile);
            m_database.addRecord(item.getText().trim(), item.getPathInfo().trim());
            if ( currentFile.isDirectory()){
        ...
        if (( deleteEvent != 0) || ( moved_fromEvent != 0) ){
          //file deleted
          if ( deleteEvent != 0){
            FileManagerApp.log(TAG + " on " + m_parentPath + " onEvent(): event=DELETE"+", path="+path);
          }
          else{
            FileManagerApp.log(TAG + " on " + m_parentPath + " onEvent(): event=MOVED_FROM"+", path="+path);
          }
          m_database.deleteRecord( m_parentPath+"/"+path);
          m_database.removeObserver(m_parentPath+"/"+path);
        }
        if ( delete_selfEvent != 0){
          FileManagerApp.log(TAG + " on " + m_parentPath + " onEvent(): event=DELETE_SELF"+", path="+path);
        }
        if ( move_selfEvent != 0){
          FileManagerApp.log(TAG + " on " + m_parentPath + " onEvent(): event=MOVE_SELF"+", path="+path);
        }
      }
    }
}
