...
public class FileManagerContent extends ActivityGroup {
  ...
  private static final String PREFIX_MOTOCONNECT_FILE = "mc://";
  ...
  private View mMoveNCancelview = null;
    ...
    mPasteButton.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        if (mCurrentContent == null) {
          ...
          mPasteIntent.setClass(mContext, com.motorola.filemanager.multiselect.local.FileManagerActivity.class);
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if (intentAction == null) {
        ...
        if (destination.equals(FileManager.ID_LOCAL)) {
          intent.setClass(mContext, com.motorola.filemanager.multiselect.local.FileManagerActivity.class);
  ...
  private String findDestAct(Intent intent) {
    ...
    if (destFileName != null) {
      if (destFileName.startsWith(FileManagerApp.getInstance(mContext).SD_CARD_EXT_DIR)) {
        return FileManager.ID_EXTSDCARD;
      } else if (destFileName.startsWith(FileManagerApp.getInstance(mContext).SD_CARD_DIR)) {
        return FileManager.ID_LOCAL;
      } else if (destFileName.startsWith(PREFIX_SMB_FILE)) {
        return FileManager.ID_REMOTE;
      } else if (destFileName.startsWith(PREFIX_MOTOCONNECT_FILE)) {
        return FileManager.ID_MOTOCONNECT;
      }
    }
...
/*  private String findSourceAct(Intent intent) {
    ...
    try {
      ...
      if (sourceFirst.startsWith(FileManagerApp.SD_CARD_EXT_DIR)) {
        ...
        return FileManager.ID_REMOTE;
      } else if (sourceFirst.startsWith(PREFIX_MOTOCONNECT_FILE)) {
        // TBD
        return FileManager.ID_MOTOCONNECT;
      }
    } catch (Exception e) {
  ...
  public void displayPasteButton() {
    mMoveNCancelview.setVisibility(View.VISIBLE);
  }
