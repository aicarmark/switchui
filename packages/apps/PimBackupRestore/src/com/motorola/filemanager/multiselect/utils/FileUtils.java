  ...
  public static File getFile(String filepath) {
    ...
    public static String getMediaMimeTypeByDataBase(Context context, String path) {
        ...
        try {
            ...
            cursor = query(context, MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, cols, selection, selectArgs, null);

            if (cursor != null && cursor.getCount() != 0) {
                mimeType = "audio/*";
            }

            String[] cols1 = new String[] { BaseColumns._ID, };
            String selection1 = MediaColumns.DATA + " like ?";
    ...
    public static int getMimeId(Context context, File fi) {
        ...
        if (isMediaFile(context , fi.getName()) && needQueryDB(ext.toLowerCase())) {
    ...
    public static void setAs(Activity activity, File file,int ringtone_type) {
        if (!file.exists()) {
            Toast.makeText(activity, activity.getString(R.string.file_not_exist, file.getName()),
        ...
        String mimeType = MimeTypeUtil.getMimeType(activity , getExtension(file.getName()));

        if (FileUtils.getMimeId(mimeType) == FileUtils.MIME_AUDIO ) {
            if (NULL_RINGTONE!=ringtone_type && setRingtone(activity, file.getAbsolutePath(),ringtone_type)) {
                if ((CDMA_CALL_RINGTONE == (ringtone_type & CDMA_CALL_RINGTONE))||(GSM_CALL_RINGTONE == (ringtone_type & GSM_CALL_RINGTONE)))
                    Toast.makeText(activity, activity.getString(R.string.success_set_call_ringstone,file.getName()), Toast.LENGTH_SHORT).show();
                if(FileManagerActivity.mSupportDualMode){
                    if ( CDMA_MESSAGE_RINGTONE == (ringtone_type & CDMA_MESSAGE_RINGTONE))
                        Toast.makeText(activity, activity.getString(R.string.success_set_cdma_message_ringstone,file.getName()), Toast.LENGTH_SHORT).show();
                    if (GSM_MESSAGE_RINGTONE == (ringtone_type & GSM_MESSAGE_RINGTONE))
                        Toast.makeText(activity, activity.getString(R.string.success_set_gsm_message_ringstone,file.getName()), Toast.LENGTH_SHORT).show();
                }else{
                    if (GSM_MESSAGE_RINGTONE == (ringtone_type & GSM_MESSAGE_RINGTONE))
                        Toast.makeText(activity, activity.getString(R.string.success_set_message_ringstone,file.getName()), Toast.LENGTH_SHORT).show();
                } 
            }
            return;
        }
    ...
    public static Cursor query(Context context, Uri uri, String[] projection, String selection,String[] selectionArgs, String
sortOrder) {
        ...
        try {
            ...
            if (cursor == null || cursor.getCount() == 0) {
                ...
                if (GSM_CALL_RINGTONE == (ringtone_type &GSM_CALL_RINGTONE)){
                    ...
                    resolver.update(ringUri, values, null, null);
                    if(FileManagerActivity.mSupportDualMode){
                        Settings.System.putString(resolver, "gsm_ringtone", ringUri.toString());
                    } else {
                ...
                if (CDMA_MESSAGE_RINGTONE == (ringtone_type &CDMA_MESSAGE_RINGTONE)){
                    ...
                    resolver.update(ringUri, values, null, null);
                    Settings.System.putString(resolver, "cdma_message_ringtone", ringUri.toString());
                }
                if (GSM_MESSAGE_RINGTONE == (ringtone_type &GSM_MESSAGE_RINGTONE)){
                    ContentValues values = new ContentValues(1);
                    values.put(AudioColumns.IS_NOTIFICATION, "1");                 
                    resolver.update(ringUri, values, null, null);
                    Settings.System.putString(resolver, "gsm_message_ringtone", ringUri.toString());
                }
            }
        }catch (Exception e) {
            e.printStackTrace();
