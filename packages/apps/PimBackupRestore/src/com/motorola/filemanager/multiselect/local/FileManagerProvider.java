...
public class FileManagerProvider extends ContentProvider {
  ...
  public String getType(Uri uri) {
    ...
    return MimeTypeUtil.getMimeType(FileManager.getCurContext(), uri.toString());
  }
  ...
  public Cursor query(Uri uri, String[] as, String s, String[] as1, String s1) {
    if (uri.toString().startsWith(MIME_TYPE_PREFIX)) {
      MatrixCursor c = new MatrixCursor(new String[]{MediaColumns.DATA, MediaColumns.MIME_TYPE});
      // data = absolute path = uri - content://authority/mimetype
      String data = uri.toString().substring(20 + AUTHORITY.length());
      String mimeType = MimeTypeUtil.getMimeType(FileManager.getCurContext(), data);
