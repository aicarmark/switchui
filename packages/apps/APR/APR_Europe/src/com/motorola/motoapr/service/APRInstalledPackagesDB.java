...
* @author wlsd10
*/
public class APRInstalledPackagesDB extends SQLiteOpenHelper {
	
	 static String TAG = "APRInstalledPackagesDB";
     ...
     static Context mContext = null;
		...
		public enum APRFields {
		 ...
		 static int    APR_IP_DB_VERSION = 1;
		 static String AP_IP_DB_NAME = "APRInstalledPackagesDB" + APR_IP_DB_VERSION;
		 static String APR_IP_TABLE_NAME = "InstalledPackages" + APR_IP_DB_VERSION;
		...
		// Constructor...
		 APRInstalledPackagesDB( Context context ) {
				
			super( context, AP_IP_DB_NAME, null, APR_IP_DB_VERSION );
				
			mContext = context;
			
			startDatabase();

        Cursor cursor = getGenericCursor(APR_IP_TABLE_NAME);
		...
		private static void startDatabase() {
			...
			try {
				if ( sqlite_database == null )
				{
					...
					sqlite_database = 
						context.openOrCreateDatabase( AP_IP_DB_NAME, 
				...
				APRDebug.APRLog ( TAG, "Exception during Package DB Creation:" + AP_IP_DB_NAME );
...
//					sqlite_database = null;
		...
		protected static void deleteEntry(int id) {
			deleteRow( APR_IP_TABLE_NAME, APR_ID_FIELD, id );
		}
		
		protected static void deleteAll() {
			deleteDB( APR_IP_TABLE_NAME );
		}
		...
		public Cursor GetDBCursor() {
			...
			Cursor return_cursor = getGenericCursor( APR_IP_TABLE_NAME );
		...
		public Cursor GetMatchingPackageName( String package_name ) {
			...
			try {
				...
				String myQuery = "SELECT * FROM " + APR_IP_TABLE_NAME + 
		...
		private static void createTable( SQLiteDatabase db) {
			...
			String dbBuild_str = "CREATE TABLE " + APR_IP_TABLE_NAME + " ("
				 ...
				 ) 
		{
			...
			String tString = new String( "insert into " + APR_IP_TABLE_NAME + " (" 
					+ APR_ID_FIELD + "," 
