	...
	@Override
   	public void onCreate() {
   		...
   		try {
			...
			boolean flex_init = APRPreferences.APRPrefsGetFlexInit();
			APRDebug.APRLog(TAG, "flex_init = "  + flex_init);
			if(false == flex_init)
			{
   		      		aprflex   = new APRFLEX(mContext);
   		      		if(APRPreferences.APRGetReadFlexSuccess()){
   		      			APRDebug.APRLog(TAG, "ReadFlexSuccess will set flex_init to be true!");
   		      			APRPreferences.APRPrefsSetFlexInit(true);
   		      		}
			}
   			boolean run_fast = APRPreferences.APRPrefsGetRunFast();
