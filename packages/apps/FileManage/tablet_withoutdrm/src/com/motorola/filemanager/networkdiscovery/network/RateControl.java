...
public class RateControl {
    ...
    private String mPath = "/system/bin/ping";
    ...
    public void adaptRate() {
        ...
        if ((response_time = getAvgResponseTime(mIndicatorHost, 3)) > 0) {
            ...
            SambaExplorer.log(TAG + " adaptRate rate=" + rate, false);
        }
        // }
    }
        ...
        try {
            if ((new File(mPath)).exists() == true) {
                ...
                while ((line = r.readLine()) != null) {
                    ...
                    if (matcher.matches()) {
                        SambaExplorer.log(
                                TAG + " getAvgResponseTime matches line: " + matcher.group(1),
                                false);
                        long time = Long.valueOf(matcher.group(1));
                        SambaExplorer.log(TAG + " getAvgResponseTime ping time: " + time, false);
                        return time;
                    }
                }
            }
        } catch (Exception e) {
            SambaExplorer.log(TAG + " Can't use native ping: " + e, true);
        } finally {
            try {
                if (r != null) {
                    r.close();
