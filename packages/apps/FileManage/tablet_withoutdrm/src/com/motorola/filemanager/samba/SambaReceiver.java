...
public class SambaReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        SambaExplorer.log("SambaReceiver onReceive" + intent, false);
        String action = intent.getAction();
        if ((action != null) && (action.equals(DownloadServer.ACTION_FILE_PICKER))) {
            SambaExplorer.log("SambaReceiver ACTION_FILE_PICKER", false);
            startFilePickerMode(context, intent);
        } else {
            SambaExplorer.log("SambaReceiver undefined intent", false);
        }
    }
    ...
    private void startFilePickerMode(Context context, Intent intent) {
        ...
        SambaExplorer.log("startService startFilePickerMode", false);
        // args.putInt(DownloadServer.DOWNLOAD_MODE,
        // DownloadServer.SAMBA_FILE_PICKER);

        context.startService(new Intent(context, DownloadServer.class).putExtras(extras));
    }
}
