...
class NameServiceClient implements Runnable {
    ...
    private static final int SND_BUF_SIZE = Config.getInt(
	    "jcifs.netbios.snd_buf_size", DEFAULT_SND_BUF_SIZE);
    private static final int RCV_BUF_SIZE = Config.getInt(
	    "jcifs.netbios.rcv_buf_size", DEFAULT_RCV_BUF_SIZE);
    private static final int SO_TIMEOUT = Config.getInt(
	    "jcifs.netbios.soTimeout", DEFAULT_SO_TIMEOUT);
    private static final int RETRY_COUNT = Config.getInt(
	    "jcifs.netbios.retryCount", DEFAULT_RETRY_COUNT);
    private static final int RETRY_TIMEOUT = Config.getInt(
	    "jcifs.netbios.retryTimeout", DEFAULT_RETRY_TIMEOUT);
    private static final int LPORT = Config.getInt("jcifs.netbios.lport", 0);
    private static final InetAddress LADDR = Config.getInetAddress(
	    "jcifs.netbios.laddr", null);
	...
	out = new DatagramPacket(snd_buf, SND_BUF_SIZE, baddr,
	...
	if (RO == null || RO.length() == 0) {
	     ...
	     * LMHOSTS,DNS,WINS,BCAST LMHOSTS,BCAST,DNS if jcifs.netbios.wins
	...
	try {
		...
		response = (NameServicePacket) responseTable.get(new Integer(
			nameTrnId));
	...
	synchronized (response) {
		...
		try {
			...
			if (response.received
				&& request.questionType == response.recordType)
			    return;
	...
	if (addr != null) { /*
		...
		if (response.received && response.resultCode == 0) {
		    ...
		    response.addrEntry[last].hostName.srcHashCode = addr
			    .hashCode();
	...
	for (int i = 0; i < resolveOrder.length; i++) {
		...
		case RESOLVER_BCAST:
			...
			if (response.received && response.resultCode == 0) {
			     ...
			     * being cached we must augment the addresses name's
			     * hashCode to distinguish those resolved by
			     * Lmhosts, WINS, or BCAST. Otherwise a failed query
			     * from say WINS would get pulled out of the cache
			     * for a BCAST on the same name.
			     */
	...
	request = new NodeStatusRequest(new Name(NbtAddress.ANY_HOSTS_NAME,
		0x00, null));
