...
public class Name {
    ...
    private static final String DEFAULT_SCOPE = Config
	    .getProperty("jcifs.netbios.scope");
	...
	this.scope = scope != null && scope.length() > 0 ? scope
		: DEFAULT_SCOPE;
	this.srcHashCode = 0;
    }
	...
	return SCOPE_OFFSET
		+ writeScopeWireFormat(dst, dstIndex + SCOPE_OFFSET);
    }
	...
	dst[dstIndex++] = (byte) '.';
	try {
	    System.arraycopy(scope.getBytes(Name.OEM_ENCODING), 0, dst,
	...
	try {
		...
		sb.append('.').append(
			new String(src, srcIndex, n, Name.OEM_ENCODING));
		srcIndex += n;
	    }
	    scope = sb.toString();
	} catch (UnsupportedEncodingException uee) {
	...
	sb.append(n).append("<").append(Hexdump.toHexString(hexCode, 2))
		.append(">");
	if (scope != null) {
