...
public class Lmhosts {
    ...
    private static final String FILENAME = Config
	    .getProperty("jcifs.netbios.lmhosts");
	...
	while ((line = br.readLine()) != null) {
		...
		if (line.startsWith("#INCLUDE ")) {
		    ...
		    if (alt > 0) {
			try {
			    populate(new InputStreamReader(
				    new SmbFileInputStream(url)));
			} catch (IOException ioe) {
			...
			populate(new InputStreamReader(new SmbFileInputStream(
				url)));
		    }
		    ...
		    throw new IOException(
			    "no lmhosts alternate includes loaded");
		}
		...
		j = i;
		while (j < data.length
			&& Character.isWhitespace(data[j]) == false) {
		    j++;
		}
