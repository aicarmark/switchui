...
public final class NbtAddress {
    ...
    static final InetAddress[] NBNS = Config.getInetAddressArray(
	    "jcifs.netbios.wins", ",", new InetAddress[0]);
    ...
    private static final int CACHE_POLICY = Config.getInt(
	    "jcifs.netbios.cachePolicy", DEFAULT_CACHE_POLICY);
    ...
    static final NbtAddress UNKNOWN_ADDRESS = new NbtAddress(UNKNOWN_NAME, 0,
	    false, B_NODE);
    static final byte[] UNKNOWN_MAC_ADDRESS = new byte[] { (byte) 0x00,
	...
	ADDRESS_CACHE.put(UNKNOWN_NAME, new CacheEntry(UNKNOWN_NAME,
		UNKNOWN_ADDRESS, FOREVER));
	 ...
	 */
	localName = new Name(localHostname, 0x00, Config.getProperty(
	...
	synchronized (ADDRESS_CACHE) {
		...
		CacheEntry entry = (CacheEntry) ADDRESS_CACHE
			.get(addrs[i].hostName);
		if (entry == null) {
		    entry = new CacheEntry(addrs[i].hostName, addrs[i],
			    expiration);
    ...
    public static NbtAddress getByName(String host, int type, String scope,
	    InetAddress svr) throws UnknownHostException {
	...
	if (!Character.isDigit(host.charAt(0))) {
		...
		char c = data[i];
		if (c < 48 || c > 57) {
		    return (NbtAddress) doNameQuery(
			    new Name(host, type, scope), svr);
		}
		int b = 0x00;
		while (c != '.') {
		    if (c < 48 || c > 57) {
			return (NbtAddress) doNameQuery(new Name(host, type,
				scope), svr);
		    }
		    b = b * 10 + c - '0';
    ...
    public static NbtAddress[] getAllByName(String host, int type,
	    String scope, InetAddress svr) throws UnknownHostException {
    ...
    public static NbtAddress[] getAllByAddress(String host, int type,
	    String scope) throws UnknownHostException {
	...
	try {
	    ...
	    throw new UnknownHostException(
		    "no name with type 0x"
			    ...
			    + (((addr.hostName.scope == null) || (addr.hostName.scope
				    .length() == 0)) ? " with no scope"
    ...
    boolean groupName, isBeingDeleted, isInConflict, isActive, isPermanent,
    ...
    NbtAddress(Name hostName, int address, boolean groupName, int nodeType,
	    boolean isBeingDeleted, boolean isInConflict, boolean isActive,
