...
public abstract class SessionServicePacket {
	...
	return ((src[srcIndex++] & 0x01) << 16)
		+ ((src[srcIndex++] & 0xFF) << 8) + (src[srcIndex++] & 0xFF);
    }

    static int readn(InputStream in, byte[] b, int off, int len)
	    throws IOException {
	int i = 0, n;
    ...
    static int readPacketType(InputStream in, byte[] buffer, int bufferIndex)
	    throws IOException {
	int n;
	if ((n = readn(in, buffer, bufferIndex, HEADER_LENGTH)) != HEADER_LENGTH) {
	    ...
	    throw new IOException(
		    "unexpected EOF reading netbios session header");
	}
    ...
    int readWireFormat(InputStream in, byte[] buffer, int bufferIndex)
	    throws IOException {
	...
	length = ((buffer[bufferIndex] & 0x01) << 16)
    ...
    abstract int readTrailerWireFormat(InputStream in, byte[] buffer,
