...
public class srvsvc {
	...
	public void decode(NdrBuffer _src) throws NdrException {
		...
		if (security_descriptor == null) {
		    if (_security_descriptors < 0
			    || _security_descriptors > 0xFFFF)
			throw new NdrException(NdrException.INVALID_CONFORMANCE);
	...
	public ShareEnumAll(String servername, int level, NdrObject info,
		int prefmaxlen, int totalentries, int resume_handle) {
	...
	public ShareGetInfo(String servername, String sharename, int level,
		NdrObject info) {
	    this.servername = servername;
