...
public class UUID extends rpc.uuid_t {
	...
	for (ai = offset; ai < arr.length && count < length; ai++) {
		...
		break;
	    default:
		throw new IllegalArgumentException(new String(arr, offset,
			length));
	    }
	    count++;
	}
	...
	return bin_to_hex(time_low, 8) + '-' + bin_to_hex(time_mid, 4) + '-'
		...
		+ bin_to_hex(clock_seq_low, 2) + '-' + bin_to_hex(node[0], 2)
		+ bin_to_hex(node[1], 2) + bin_to_hex(node[2], 2)
		+ bin_to_hex(node[3], 2) + bin_to_hex(node[4], 2)
		+ bin_to_hex(node[5], 2);
    }
}
