...
public class Type2Message extends NtlmMessage {
	...
	this(getDefaultFlags(type1), challenge,
		(type1 != null && target == null && type1
			.getFlag(NTLMSSP_REQUEST_TARGET)) ? getDefaultDomain()
			: target);
    }
	...
	try {
	    ...
	    if ((flags & (NTLMSSP_TARGET_TYPE_DOMAIN
		    | NTLMSSP_TARGET_TYPE_SERVER | NTLMSSP_TARGET_TYPE_SHARE)) != 0) {
		if (targetName != null && targetName.length() != 0) {
		    target = (flags & NTLMSSP_NEGOTIATE_UNICODE) != 0 ? targetName
			    .getBytes(UNI_ENCODING) : targetName.toUpperCase()
			    .getBytes(getOEMEncoding());
		} else {
		    flags &= (0xffffffff ^ (NTLMSSP_TARGET_TYPE_DOMAIN
			    | NTLMSSP_TARGET_TYPE_SERVER | NTLMSSP_TARGET_TYPE_SHARE));
		}
	    }
	...
	if (targetInformation != null) {
		...
		buffer.append(Integer
			.toHexString((targetInformation[i] >> 4) & 0x0f));
	...
	if (bytes.length != 0) {
	    target = new String(bytes,
		    ((flags & NTLMSSP_NEGOTIATE_UNICODE) != 0) ? UNI_ENCODING
			    : getOEMEncoding());
	}
	setTarget(target);
	for (int i = 24; i < 32; i++) {
