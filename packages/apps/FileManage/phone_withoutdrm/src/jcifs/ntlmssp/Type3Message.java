...
public class Type3Message extends NtlmMessage {
	...
	DEFAULT_PASSWORD = Config
		.getProperty("jcifs.smb.client.password", null);
	...
	case 1:
		...
		// NTLMv1 w/ NTLM2 session sec and key exch all been verified
		// with a debug build of smbclient

		byte[] responseKeyNT = NtlmPasswordAuthentication
			.nTOWFv1(password);
		byte[] ntlm2Response = NtlmPasswordAuthentication
			.getNTLM2Response(responseKeyNT, type2.getChallenge(),
				clientChallenge);
		...
		if ((getFlags() & NTLMSSP_NEGOTIATE_SIGN) == NTLMSSP_NEGOTIATE_SIGN) {
		    ...
		    System.arraycopy(type2.getChallenge(), 0, sessionNonce, 0,
			    8);
			...
			/*
			 * RC4 was not added to Java until 1.5u7 so let's use
			 * our own for a little while longer ... try { Cipher
			 * rc4 = Cipher.getInstance("RC4");
			 * rc4.init(Cipher.ENCRYPT_MODE, new
			 * SecretKeySpec(ntlm2SessionKey, "RC4"));
			 * rc4.update(masterKey, 0, 16, exchangedKey, 0); }
			 * catch (GeneralSecurityException gse) { throw new
			 * RuntimeException("", gse); }
			 */
	...
	case 4:
	case 5:
	    byte[] responseKeyNT = NtlmPasswordAuthentication.nTOWFv2(domain,
		    user, password);
	    ...
	    setNTResponse(getNTLMv2Response(type2, responseKeyNT,
		    clientChallenge2));

	    if ((getFlags() & NTLMSSP_NEGOTIATE_SIGN) == NTLMSSP_NEGOTIATE_SIGN) {
		HMACT64 hmac = new HMACT64(responseKeyNT);
		hmac.update(ntResponse, 0, 16); // only first 16 bytes of
						// ntResponse
		...
		if ((getFlags() & NTLMSSP_NEGOTIATE_KEY_EXCH) != 0) {
		    ...
		    rc4.update(masterKey, 0, 16, exchangedKey, 0);
		    /*
		     * RC4 was not added to Java until 1.5u7 so let's use our
		     * own for a little while longer ... try { Cipher rc4 =
		     * Cipher.getInstance("RC4"); rc4.init(Cipher.ENCRYPT_MODE,
		     * new SecretKeySpec(userSessionKey, "RC4"));
		     * rc4.update(masterKey, 0, 16, exchangedKey, 0); } catch
    ...
    /**
     * The real session key if the regular session key is actually the encrypted
     * version used for key exchange.
     * 
	...
	try {
		...
		domain = unicode ? domainName.getBytes(UNI_ENCODING)
		...
		user = unicode ? userName.getBytes(UNI_ENCODING) : userName
		...
		workstation = unicode ? workstationName.getBytes(UNI_ENCODING)
			: workstationName.toUpperCase().getBytes(oem);
	    }
	    int workstationLength = (workstation != null) ? workstation.length
		    : 0;
    ...
    public static byte[] getLMv2Response(Type2Message type2, String domain,
    ...
    public static byte[] getNTLMv2Response(Type2Message type2,
	    byte[] responseKeyNT, byte[] clientChallenge) {
	...
	return NtlmPasswordAuthentication.getNTLMv2Response(responseKeyNT,
		type2.getChallenge(), clientChallenge, nanos1601,
	...
	if (lmResponseOffset == 52 || ntResponseOffset == 52
	    ...
	    charset = ((flags & NTLMSSP_NEGOTIATE_UNICODE) != 0) ? UNI_ENCODING
		    : getOEMEncoding();
	}
