...
public class SmbTransport extends Transport implements SmbConstants {
    ...
    static synchronized SmbTransport getSmbTransport(UniAddress address,
	    int port, InetAddress localAddr, int localPort, String hostName) {
	SmbTransport conn;
    ...
    SmbTransport(UniAddress address, int port, InetAddress localAddr,
	    int localPort) {
	...
	/* logoff old sessions */
	if (SO_TIMEOUT > 0
		&& sessionExpiration < (now = System.currentTimeMillis())) {
    ...
    boolean matches(UniAddress address, int port, InetAddress localAddr,
	    int localPort, String hostName) {
	if (hostName == null)
	    hostName = address.getHostName();
	return (this.tconHostName == null || hostName
		.equalsIgnoreCase(this.tconHostName))
	...
	return (flags2 & ServerMessageBlock.FLAGS2_SECURITY_SIGNATURES) != 0
		&& digest == null && auth != NtlmPasswordAuthentication.NULL
	...
	do {
		...
		socket = new Socket(address.getHostAddress(), 139, localAddr,
			localPort);
	    }
	...
	if ((server.capabilities & CAP_EXTENDED_SECURITY) != CAP_EXTENDED_SECURITY
		&& server.encryptionKeyLength != 8 && LM_COMPATIBILITY == 0) {
	...
	for (;;) {
	    ...
	    if (sbuf[0] == (byte) 0x00 && sbuf[1] == (byte) 0x00
		    && sbuf[4] == (byte) 0xFF && sbuf[5] == (byte) 'S'
		    && sbuf[6] == (byte) 'M' && sbuf[7] == (byte) 'B') {
		break; /* all good */
	    }
	    /* out of phase maybe? */
    ...
    void checkStatus(ServerMessageBlock req, ServerMessageBlock resp)
	    throws SmbException {
	...
	try {
		...
		try {
			...
			try {
			    ...
			    resp.expiration = System.currentTimeMillis()
				    + timeout;
			    while (resp.hasMoreElements()) {
				response_map.wait(timeout);
				timeout = resp.expiration
    ...
    DfsReferral getDfsReferrals(NtlmPasswordAuthentication auth, String path,
    ...
    DfsReferral[] __getDfsReferrals(NtlmPasswordAuthentication auth,
	    String path, int rn) throws SmbException {
	SmbTree ipc = getSmbSession(auth).getSmbTree("IPC$", null);
	Trans2GetDfsReferralResponse resp = new Trans2GetDfsReferralResponse();
	ipc.send(new Trans2GetDfsReferral(path), resp);
    ...
    // FileEntry[] getDfsRoots(String domainName, NtlmPasswordAuthentication
    // auth) throws IOException {
