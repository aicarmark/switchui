...
class Trans2FindFirst2 extends SmbComTransaction {
    ...
    static final int LIST_SIZE = Config.getInt("jcifs.smb.client.listSize",
	    DEFAULT_LIST_SIZE);
    static final int LIST_COUNT = Config.getInt("jcifs.smb.client.listCount",
	    DEFAULT_LIST_COUNT);
	...
	return new String("Trans2FindFirst2[" + super.toString()
		+ ",searchAttributes=0x"
		+ Hexdump.toHexString(searchAttributes, 2) + ",searchCount="
		+ LIST_COUNT + ",flags=0x" + Hexdump.toHexString(flags, 2)
		+ ",informationLevel=0x"
		+ Hexdump.toHexString(informationLevel, 3)
		+ ",searchStorageType=" + searchStorageType + ",filename="
		+ path + "]");
    }
}
