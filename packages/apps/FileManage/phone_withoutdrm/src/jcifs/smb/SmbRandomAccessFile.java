...
public class SmbRandomAccessFile implements DataOutput, DataInput {
	...
	if (mode.equals("r")) {
	    ...
	    this.openFlags = SmbFile.O_CREAT | SmbFile.O_RDWR
		    | SmbFile.O_APPEND;
	...
	do {
	    ...
	    file.send(new SmbComWriteAndX(file.fid, fp, len - w, b, off, w,
		    null), write_andx_resp);
	...
	file.send(new SmbComWrite(file.fid, (int) (newLength & 0xFFFFFFFFL), 0,
		tmp, 0, 0), rsp);
    }
