...
class SmbComSessionSetupAndX extends AndXServerMessageBlock {
    ...
    SmbComSessionSetupAndX(SmbSession session, ServerMessageBlock andx,
	    Object cred) throws SmbException {
	super(andx);
	...
	if (session.transport.server.security == SECURITY_USER) {
		...
		if (auth == NtlmPasswordAuthentication.ANONYMOUS) {
		    ...
		    ntHash = auth
			    .getUnicodeHash(session.transport.server.encryptionKey);
	...
	String result = new String("SmbComSessionSetupAndX[" + super.toString()
		+ ",snd_buf_size=" + session.transport.snd_buf_size
		+ ",maxMpxCount=" + session.transport.maxMpxCount
		+ ",VC_NUMBER=" + session.transport.VC_NUMBER + ",sessionKey="
		+ sessionKey + ",lmHash.length="
		+ (lmHash == null ? 0 : lmHash.length) + ",ntHash.length="
		+ (ntHash == null ? 0 : ntHash.length) + ",capabilities="
		+ capabilities + ",accountName=" + accountName
		+ ",primaryDomain=" + primaryDomain + ",NATIVE_OS="
		+ session.transport.NATIVE_OS + ",NATIVE_LANMAN="
		+ session.transport.NATIVE_LANMAN + "]");
	return result;
    }
}
