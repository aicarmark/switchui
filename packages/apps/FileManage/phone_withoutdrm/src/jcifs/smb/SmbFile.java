...
public class SmbFile extends URLConnection implements SmbConstants {
	...
	attrExpirationPeriod = Config.getLong(
    ...
    public SmbFile(SmbFile context, String name) throws MalformedURLException,
	    UnknownHostException {
	this(context.isWorkgroup0() ? new URL(null, "smb://" + name,
		Handler.SMB_HANDLER) : new URL(context.url, name,
	...
	this(new URL(new URL(null, context, Handler.SMB_HANDLER), name,
		Handler.SMB_HANDLER));
    }
	...
	super(url);
	this.auth = auth == null ? new NtlmPasswordAuthentication(
		url.getUserInfo()) : auth;

	getUncPath0();
    }

    SmbFile(SmbFile context, String name, int type, int attributes,
	...
	attrExpiration = sizeExpiration = System.currentTimeMillis()
	...
	DfsReferral dr = dfs.resolve(tree.session.transport.tconHostName,
		tree.share, unc, auth);
	if (dr != null) {
	    String service = null;
		...
		try {
		    ...
		    SmbTransport trans = SmbTransport.getSmbTransport(addr,
			    url.getPort());
		    /*
		     ...
		     */
		    trans.connect();
		    tree = trans.getSmbSession(auth).getSmbTree(dr.share,
			    service);
	...
	if (host.length() == 0) {
		...
		addresses = UniAddress.getAllByName(
			NtlmPasswordAuthentication.DEFAULT_DOMAIN, true);
	    }
	...
	try {
		...
		tree.inDomainDfs = dfs
			.resolve(hostName, tree.share, null, auth) != null;
		if (tree.inDomainDfs) {
		    tree.treeConnected = true;
		}
		tree.treeConnect(null, null);
	...
	if (tree.session.transport
		.hasCapability(ServerMessageBlock.CAP_NT_SMBS)) {
	    SmbComNTCreateAndXResponse response = new SmbComNTCreateAndXResponse();
	    SmbComNTCreateAndX request = new SmbComNTCreateAndX(unc, flags,
		    access, shareAccess, attrs, options, null);
	...
	String path = "smb:/" + dfsReferral.server + "/" + dfsReferral.share
		+ unc;
	path = path.replace('\\', '/');
	if (isDirectory()) {
	    path += '/';
	...
	return list("*", ATTR_DIRECTORY | ATTR_HIDDEN | ATTR_SYSTEM, filter,
		null);
    }
	...
	return listFiles("*", ATTR_DIRECTORY | ATTR_HIDDEN | ATTR_SYSTEM, null,
		null);
    }
	...
	return listFiles(wildcard, ATTR_DIRECTORY | ATTR_HIDDEN | ATTR_SYSTEM,
		null, null);
    }
	...
	return listFiles("*", ATTR_DIRECTORY | ATTR_HIDDEN | ATTR_SYSTEM,
		filter, null);
    }
    ...
    String[] list(String wildcard, int searchAttributes, SmbFilenameFilter fnf,
    ...
    void doEnum(ArrayList list, boolean files, String wildcard,
	    int searchAttributes, SmbFilenameFilter fnf, SmbFileFilter ff)
	    throws SmbException {
	...
	try {
		...
		doNetServerEnum(list, files, wildcard, searchAttributes, fnf,
			ff);
	    } else if (share == null) {
		doShareEnum(list, files, wildcard, searchAttributes, fnf, ff);
	    } else {
		doFindFirstNext(list, files, wildcard, searchAttributes, fnf,
			ff);
	    }
	} catch (UnknownHostException uhe) {
	    throw new SmbException(url.toString(), uhe);
    ...
    void doShareEnum(ArrayList list, boolean files, String wildcard,
	    int searchAttributes, SmbFilenameFilter fnf, SmbFileFilter ff)
	...
	if (p.lastIndexOf('/') != (p.length() - 1))
	    throw new SmbException(url.toString()
		    + " directory must end with '/'");
	if (getType() != TYPE_SERVER)
	    throw new SmbException("The requested list operations is invalid: "
		    + url.toString());
	...
	while (iter.hasNext()) {
		...
		SmbFile f = new SmbFile(this, name, e.getType(), ATTR_READONLY
			| ATTR_DIRECTORY, 0L, 0L, 0L);
	...
	handle = DcerpcHandle.getHandle("ncacn_np:"
		+ getAddress().getHostAddress() + "[\\PIPE\\srvsvc]", auth);
    ...
    void doNetServerEnum(ArrayList list, boolean files, String wildcard,
	    int searchAttributes, SmbFilenameFilter fnf, SmbFileFilter ff)
	    throws SmbException, UnknownHostException, MalformedURLException {
    ...
    void doFindFirstNext(ArrayList list, boolean files, String wildcard,
	    int searchAttributes, SmbFilenameFilter fnf, SmbFileFilter ff)
	    throws SmbException, UnknownHostException, MalformedURLException {
	SmbComTransaction req;
	...
	WriterThread() throws SmbException {
	    ...
	    useNTSmbs = tree.session.transport
		    .hasCapability(ServerMessageBlock.CAP_NT_SMBS);
	    if (useNTSmbs) {
	...
	if (isDirectory()) {
		...
		try {
		    dest.mkdir();
		    dest.setPathInformation(attributes, createTime,
			    lastModified);
		} catch (SmbException se) {
	    ...
	    files = listFiles("*", ATTR_DIRECTORY | ATTR_HIDDEN | ATTR_SYSTEM,
		    null, null);
	    try {
		for (i = 0; i < files.length; i++) {
		    ndest = new SmbFile(dest, files[i].getName(),
			    files[i].type, files[i].attributes,
			    files[i].createTime, files[i].lastModified,
			    files[i].size);
		...
		try {
		    dest.open(SmbFile.O_CREAT | SmbFile.O_WRONLY
			    | SmbFile.O_TRUNC, FILE_WRITE_DATA
			 ...
			 */
			dest.setPathInformation(dest.attributes
				& ~ATTR_READONLY, 0L, 0L);
			dest.open(SmbFile.O_CREAT | SmbFile.O_WRONLY
				| SmbFile.O_TRUNC, FILE_WRITE_DATA
				| FILE_WRITE_ATTRIBUTES, attributes, 0);
		...
		dest.send(new Trans2SetFileInformation(dest.fid, attributes,
	...
	if ((attributes & ATTR_DIRECTORY) != 0) {
	    ...
	    try {
		SmbFile[] l = listFiles("*", ATTR_DIRECTORY | ATTR_HIDDEN
			| ATTR_SYSTEM, null, null);
	...
	f = open0(O_RDONLY, FILE_WRITE_ATTRIBUTES, dir, dir != 0 ? 0x0001
		: 0x0040);
    ...
    private void processAces(ACE[] aces, boolean resolveSids)
	    throws IOException {
