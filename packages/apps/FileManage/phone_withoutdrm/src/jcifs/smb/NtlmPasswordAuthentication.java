...
public final class NtlmPasswordAuthentication implements Principal,
	...
	DEFAULT_USERNAME = Config.getProperty("jcifs.smb.client.username",
		"GUEST");
	...
	try {
	    ...
	    // The next 2-1/2 lines of this should be placed with nTOWFv1 in
	    // place of password
	    MD4 md4 = new MD4();
	    ...
	    hmac.update(domain.toUpperCase()
		    .getBytes(SmbConstants.UNI_ENCODING));
    ...
    static byte[] computeResponse(byte[] responseKey, byte[] serverChallenge,
    ...
    public static byte[] getNTLMv2Response(byte[] responseKeyNT,
	    byte[] serverChallenge, byte[] clientChallenge, long nanos1601,
	    byte[] targetInfo) {
	...
	Encdec.enc_uint32le(0x00000000, temp, 28 + targetInfoLength); // mystery
								      // bytes!
	...
	case 5:
	    /*
	     * This code is only called if extended security is not on. This
	     * will all be cleaned up an normalized in JCIFS 2.x.
	     */
