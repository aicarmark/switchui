...
abstract class SmbComTransaction extends ServerMessageBlock implements
	...
	if (isPrimary) {
	    ...
	    totalParameterCount = writeParametersWireFormat(txn_buf,
		    bufParameterOffset);
	    ...
	    parameterCount = Math.min(totalParameterCount
		    - parameterDisplacement, available);
	    available -= parameterCount;
	...
	if (parameterCount > 0) {
	    ...
	    System.arraycopy(txn_buf, bufParameterOffset, dst, dstIndex,
		    parameterCount);
    ...
    abstract int readParametersWireFormat(byte[] buffer, int bufferIndex,
	    int len);

    abstract int readDataWireFormat(byte[] buffer, int bufferIndex, int len);

    public String toString() {
	return new String(super.toString() + ",totalParameterCount="
		+ totalParameterCount + ",totalDataCount=" + totalDataCount
		+ ",maxParameterCount=" + maxParameterCount + ",maxDataCount="
		+ maxDataCount + ",maxSetupCount=" + (int) maxSetupCount
		+ ",flags=0x" + Hexdump.toHexString(flags, 2) + ",timeout="
		+ timeout + ",parameterCount=" + parameterCount
		+ ",parameterOffset=" + parameterOffset
		+ ",parameterDisplacement=" + parameterDisplacement
		+ ",dataCount=" + dataCount + ",dataOffset=" + dataOffset
		+ ",dataDisplacement=" + dataDisplacement + ",setupCount="
		+ setupCount + ",pad=" + pad + ",pad1=" + pad1);
    }
}
