...
class SmbTree {
	...
	if (service.equals("A:") == false) {
		...
		case SmbComTransaction.TRANS2_GET_DFS_REFERRAL:
		    break;
		default:
		    throw new SmbException("Invalid operation for " + service
			    + " service");
		}
		break;
	    default:
	...
	request.tid = tid;
	if (inDfs && !service.equals("IPC") && request.path != null
    ...
    void treeConnect(ServerMessageBlock andx, ServerMessageBlock andxResponse)
	    throws SmbException {
	String unc;
	...
	synchronized (transport.setupDiscoLock) {
		...
		if (session.transport.log.level >= 4)
		    session.transport.log.println("treeConnect: unc=" + unc
			    + ",service=" + service);

		SmbComTreeConnectAndXResponse response = new SmbComTreeConnectAndXResponse(
			andxResponse);
		SmbComTreeConnectAndX request = new SmbComTreeConnectAndX(
			session, unc, service, andx);
		session.send(request, response);
	...
	return "SmbTree[share=" + share + ",service=" + service + ",tid=" + tid
		+ ",inDfs=" + inDfs + ",inDomainDfs=" + inDomainDfs
