...
class SmbComTreeConnectAndX extends AndXServerMessageBlock {
    ...
    private static final boolean DISABLE_PLAIN_TEXT_PASSWORDS = Config
	    .getBoolean("jcifs.smb.client.disablePlainTextPasswords", true);
	...
	if ((s = Config
		.getProperty("jcifs.smb.client.TreeConnectAndX.CheckDirectory")) != null) {
	    batchLimits[0] = Byte.parseByte(s);
	}
	if ((s = Config
		.getProperty("jcifs.smb.client.TreeConnectAndX.CreateDirectory")) != null) {
	...
	if ((s = Config
		.getProperty("jcifs.smb.client.TreeConnectAndX.DeleteDirectory")) != null) {
	    batchLimits[4] = Byte.parseByte(s);
	}
	if ((s = Config
		.getProperty("jcifs.smb.client.TreeConnectAndX.OpenAndX")) != null) {
	...
	if ((s = Config
		.getProperty("jcifs.smb.client.TreeConnectAndX.Transaction")) != null) {
	    batchLimits[7] = Byte.parseByte(s);
	}
	if ((s = Config
		.getProperty("jcifs.smb.client.TreeConnectAndX.QueryInformation")) != null) {
	    batchLimits[8] = Byte.parseByte(s);
	}
    }

    SmbComTreeConnectAndX(SmbSession session, String path, String service,
	...
	if (session.transport.server.security == SECURITY_SHARE
		&& (session.auth.hashesExternal || session.auth.password
			.length() > 0)) {

	    if (session.transport.server.encryptedPasswords) {
		// encrypted
		password = session.auth
			.getAnsiHash(session.transport.server.encryptionKey);
	...
	try {
	    System.arraycopy(service.getBytes("ASCII"), 0, dst, dstIndex,
		    service.length());
	...
	String result = new String("SmbComTreeConnectAndX[" + super.toString()
		+ ",disconnectTid=" + disconnectTid + ",passwordLength="
		+ passwordLength + ",password="
		+ Hexdump.toHexString(password, passwordLength, 0) + ",path="
