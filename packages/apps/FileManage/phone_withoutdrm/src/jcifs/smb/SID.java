...
public class SID extends rpc.sid_t {
    ...
    static final String[] SID_TYPE_NAMES = { "0", "User", "Domain group",
	    "Domain", "Local group", "Builtin group", "Deleted", "Invalid",
	    "Unknown" };
    ...
    static void resolveSids(DcerpcHandle handle, LsaPolicyHandle policyHandle,
	...
	for (int si = 0; si < sids.length; si++) {
		...
		sids[si].domainName = (new UnicodeString(ustr, false))
			.toString();
		break;
	    }

	    sids[si].acctName = (new UnicodeString(rpc.names.names[si].name,
		    false)).toString();
    ...
    static void resolveSids0(String authorityServerName,
	    NtlmPasswordAuthentication auth, SID[] sids) throws IOException {
	...
	try {
	    ...
	    policyHandle = new LsaPolicyHandle(handle, "\\\\" + server,
		    0x00000800);
    ...
    public SID(rpc.sid_t sid, int type, String domainName, String acctName,
	...
	return new SID(this, SID_TYPE_DOMAIN, this.domainName, null,
	...
	if (type == SID_TYPE_UNKNOWN) {
	    ...
	    return full.substring(0, full.length() - getAccountName().length()
		    - 1);
	}
	return domainName;
    }
	...
	if (identifier_authority[0] != (byte) 0
		|| identifier_authority[1] != (byte) 0) {
	    ret += "0x";
    ...
    static SID[] getGroupMemberSids0(DcerpcHandle handle,
	    SamrDomainHandle domainHandle, SID domsid, int rid, int flags)
	    throws IOException {
	...
	try {
	    aliasHandle = new SamrAliasHandle(handle, domainHandle, 0x0002000c,
		    rid);
	...
	try {
	    handle = DcerpcHandle.getHandle("ncacn_np:" + authorityServerName
		    + "[\\PIPE\\samr]", auth);
	    policyHandle = new SamrPolicyHandle(handle, authorityServerName,
		    0x00000030);
	    domainHandle = new SamrDomainHandle(handle, policyHandle,
		    0x00000200, domsid);
	    return SID.getGroupMemberSids0(handle, domainHandle, domsid,
		    getRid(), flags);
	} finally {
	    if (handle != null) {
		if (policyHandle != null) {
	...
	try {
		...
		SID[] mems = SID.getGroupMemberSids0(handle, domainHandle,
			domsid, entry.idx, flags);
		SID groupSid = new SID(domsid, entry.idx);
		groupSid.type = SID_TYPE_ALIAS;
		groupSid.domainName = domsid.getDomainName();
		groupSid.acctName = (new UnicodeString(entry.name, false))
			.toString();
