...
public class SmbFileOutputStream extends OutputStream {
    ...
    public SmbFileOutputStream(String url) throws SmbException,
    ...
    public SmbFileOutputStream(SmbFile file, boolean append)
	    throws SmbException, MalformedURLException, UnknownHostException {
	this(file, append, append ? SmbFile.O_CREAT | SmbFile.O_WRONLY
		| SmbFile.O_APPEND : SmbFile.O_CREAT | SmbFile.O_WRONLY
		| SmbFile.O_TRUNC);
    }
	...
	file.open(openFlags, access | SmbConstants.FILE_WRITE_DATA,
	...
	useNTSmbs = file.tree.session.transport
     ...
     */
    public void writeDirect(byte[] b, int off, int len, int flags)
	    throws IOException {
	if (len <= 0) {
	    return;
	}
	...
	if (file.log.level >= 4)
	    file.log.println("write: fid=" + file.fid + ",off=" + off + ",len="
		    + len);
