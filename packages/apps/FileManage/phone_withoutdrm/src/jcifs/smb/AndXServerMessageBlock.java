...
abstract class AndXServerMessageBlock extends ServerMessageBlock {
	...
	wordCount = writeParameterWordsWireFormat(dst, start
		+ ANDX_OFFSET_OFFSET + 2);
	...
	if (andx instanceof AndXServerMessageBlock) {
	    ...
	    andx.uid = uid;
	    dstIndex += ((AndXServerMessageBlock) andx).writeAndXWireFormat(
		    dst, dstIndex);
	} else {
	...
	if (wordCount != 0) {
		...
		/*
		 * The SMB_COM_NT_CREATE_ANDX response wordCount is wrong.
		 * There's an extra 16 bytes for some
		 * "Offline Files (CSC or Client Side Caching)" junk. We need to
		 * bump up the wordCount here so that this method returns the
		 * correct number of bytes for signing purposes. Otherwise we
		 * get a signing verification failure.
		 */
		if (command == SMB_COM_NT_CREATE_ANDX
			&& ((SmbComNTCreateAndXResponse) this).isExtended)
		    wordCount += 8;
	    }
	...
	if (errorCode != 0 || andxCommand == (byte) 0xFF) {
	    ...
	    if (andx instanceof AndXServerMessageBlock) {
		bufferIndex += ((AndXServerMessageBlock) andx)
			.readAndXWireFormat(buffer, bufferIndex);
	    } else {
		...
		if (andx.wordCount != 0) {
		    ...
		    if (andx.wordCount > 2) {
			bufferIndex += andx.readParameterWordsWireFormat(
				buffer, bufferIndex);
		    }
		}
	...
	return new String(super.toString() + ",andxCommand=0x"
		+ Hexdump.toHexString(andxCommand, 2) + ",andxOffset="
		+ andxOffset);
    }
}
