...
abstract class SmbComTransactionResponse extends ServerMessageBlock implements
	...
	if (dataCount > 0) {
	    ...
	    System.arraycopy(buffer, bufferIndex, txn_buf, bufDataStart
	...
	if (parametersDone && dataDone) {
	    hasMore = false;
	    readParametersWireFormat(txn_buf, bufParameterStart,
		    totalParameterCount);
	...
	return new String(super.toString() + ",totalParameterCount="
		...
		+ ",parameterCount=" + parameterCount + ",parameterOffset="
		+ parameterOffset + ",parameterDisplacement="
		+ parameterDisplacement + ",dataCount=" + dataCount
		+ ",dataOffset=" + dataOffset + ",dataDisplacement="
		+ dataDisplacement + ",setupCount=" + setupCount + ",pad="
		+ pad + ",pad1=" + pad1);
    }
}
