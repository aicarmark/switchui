...
class Trans2GetDfsReferralResponse extends SmbComTransactionResponse {
	...
	int readWireFormat(byte[] buffer, int bufferIndex, int len) {
		...
		path = readString(buffer, start + pathOffset, len,
			(flags2 & FLAGS2_UNICODE) != 0);
		if (nodeOffset > 0)
		    node = readString(buffer, start + nodeOffset, len,
			    (flags2 & FLAGS2_UNICODE) != 0);
	    } else if (version == 1) {
		node = readString(buffer, bufferIndex, len,
			(flags2 & FLAGS2_UNICODE) != 0);
	    }
	...
	public String toString() {
	    return new String("Referral[" + "version=" + version + ",size="
		    + size + ",serverType=" + serverType + ",flags=" + flags
		    + ",proximity=" + proximity + ",ttl=" + ttl
		    + ",pathOffset=" + pathOffset + ",altPathOffset="
		    + altPathOffset + ",nodeOffset=" + nodeOffset + ",path="
		    + path + ",altPath=" + altPath + ",node=" + node + "]");
	}
    }
	...
	for (int ri = 0; ri < numReferrals; ri++) {
	    ...
	    bufferIndex += referrals[ri].readWireFormat(buffer, bufferIndex,
		    len);
	}
    ...
    public String toString() {
	return new String("Trans2GetDfsReferralResponse[" + super.toString()
		+ ",pathConsumed=" + pathConsumed + ",numReferrals="
		+ numReferrals + ",flags=" + flags + "]");
    }
}
