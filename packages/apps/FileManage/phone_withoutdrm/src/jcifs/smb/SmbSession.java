...
public final class SmbSession {
    ...
    private static final String LOGON_SHARE = Config.getProperty(
	    "jcifs.smb.client.logonShare", null);
    private static final int LOOKUP_RESP_LIMIT = Config.getInt(
	    "jcifs.netbios.lookupRespLimit", 3);
    private static final String DOMAIN = Config.getProperty(
	    "jcifs.smb.client.domain", null);
    private static final String USERNAME = Config.getProperty(
	    "jcifs.smb.client.username", null);
    private static final int CACHE_POLICY = Config.getInt(
	    "jcifs.netbios.cachePolicy", 60 * 10) * 60; /* 10 hours */
	...
	if (USERNAME == null) {
		...
		SmbTransport.log
			.println("Default credentials (jcifs.smb.client.username/password)"
				+ " not specified. SMB signing may not work propertly."
				+ "  Skipping DC interrogation.");
	} else {
	    SmbSession ssn = trans
		    .getSmbSession(NtlmPasswordAuthentication.DEFAULT);
	    ssn.getSmbTree(LOGON_SHARE, null).treeConnect(null, null);
	}
	return new NtlmChallenge(trans.server.encryptionKey, dc);
    }

    public static NtlmChallenge getChallengeForDomain() throws SmbException,
	...
	synchronized (DOMAIN) {
		...
		if (dc_list_expiration < now) {
		    NbtAddress[] list = NbtAddress.getAllByName(DOMAIN, 0x1C,
			    null, null);
			...
			if (SmbTransport.log.level >= 2) {
			    SmbTransport.log
				    .println("Failed to retrieve DC list from WINS");
			}
		    }
		}
		...
		for (int j = 0; j < max; j++) {
			...
			try {
			    ...
			    if (SmbTransport.log.level >= 2) {
				SmbTransport.log.println("Failed validate DC: "
					+ dc_list[i]);
				if (SmbTransport.log.level > 2)
	...
	throw new UnknownHostException(
		"Failed to negotiate with a suitable domain controller for "
			+ DOMAIN);
    }

    public static byte[] getChallenge(UniAddress dc) throws SmbException,
	    UnknownHostException {
     ...
     * <p>
     * Update June 2009: This method does not support NTLMv2. JCIFS does not
     * implement the acceptor side of NTLM authentication. It can only initiate
     * NTLM authentication as a client.
     * <p>
	...
	SmbTree tree = SmbTransport.getSmbTransport(dc, port)
		.getSmbSession(auth).getSmbTree(LOGON_SHARE, null);
	if (LOGON_SHARE == null) {
    ...
    SmbSession(UniAddress address, int port, InetAddress localAddr,
	    int localPort, NtlmPasswordAuthentication auth) {
	...
	if (transport == null) {
	    transport = SmbTransport.getSmbTransport(address, port, localAddr,
		    localPort, null);
	}
	return transport;
    }
	...
	synchronized (transport()) {
		...
		transport.log.println("sessionSetup: accountName="
			+ auth.username + ",primaryDomain=" + auth.domain);

	    /*
	     * We explicitly set uid to 0 here to prevent a new
	     * SMB_COM_SESSION_SETUP_ANDX from having it's uid set to an old
	     * value when the session is re-established. Otherwise a
		...
		case 10: /* NTLM */
		    if (auth != NtlmPasswordAuthentication.ANONYMOUS
			    && transport
				    .hasCapability(SmbConstants.CAP_EXTENDED_SECURITY)) {
			state = 20; /* NTLMSSP */
			break;
		    }
			...
			if (auth.hashesExternal
			    ...
			    transport
				    .getSmbSession(
					    NtlmPasswordAuthentication.DEFAULT)
				    .getSmbTree(LOGON_SHARE, null)
				    .treeConnect(null, null);
			} else {
			    byte[] signingKey = auth
				    .getSigningKey(transport.server.encryptionKey);
		...
		case 20:
			...
			/*
			 * We must close the transport or the server will be
			 * expecting a Type3Message. Otherwise, when we send a
			 * Type1Message it will return "Invalid parameter".
			 */
			try {
			...
			try {
			    ...
			    ex = se;
			    /*
			     * Apparently once a successfull NTLMSSP login
			     * occurs, the server will return "Access denied"
			     * even if a logoff is sent. Unfortunately calling
			     * disconnect() doesn't always actually shutdown the
			     * connection before other threads have committed
			     * themselves (e.g. InterruptTest example).
			     */
			    try {
				transport.disconnect(true);
			    } catch (Exception e) {
		    ...
		    break;
		default:
		    throw new SmbException("Unexpected session setup state: "
			    + state);
		}
	    } while (state != 0);
	}
	...
	synchronized (transport()) {
	    ...
	    if (!inError
		    && transport.server.security != ServerMessageBlock.SECURITY_SHARE) {
		/*
	...
	return "SmbSession[accountName=" + auth.username + ",primaryDomain="
		+ auth.domain + ",uid=" + uid + ",sessionSetup=" + sessionSetup
		+ "]";
    }
}
