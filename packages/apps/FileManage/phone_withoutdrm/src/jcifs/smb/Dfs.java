...
public class Dfs {
	...
	if (_domains != null
		&& System.currentTimeMillis() > _domains.expiration) {
	    _domains = null;
	}
	if (_domains != null)
	    return _domains.map;
    ...
    public DfsReferral getReferral(SmbTransport trans, String domain,
	    String root, String path, NtlmPasswordAuthentication auth)
	    throws SmbAuthException {
	if (DISABLED)
	    return null;
    ...
    public synchronized DfsReferral resolve(String domain, String root,
	    String path, NtlmPasswordAuthentication auth)
	    throws SmbAuthException {
	DfsReferral dr = null;
	...
	if (domains != null) {
		...
		/*
		 * The link entries contain maps of referrals by path
		 * representing DFS links. Note that paths are relative to the
		 * root like "\" and not "\example.com\root".
		 */
		...
		if (links == null) {
			...
			do {
				 ...
				 * SmbFile.resolveDfs can re-insert the dr list
				 * with the dr that was successful so that
				 * subsequent attempts to resolve DFS use the
				 * last successful referral first.
				 */
				tmp.map = links.map;
				tmp.key = "\\";
			    }
		...
		if (links != null) {
		    ...
		    /*
		     * Lookup the domain based DFS root target referral. Note
		     * the path is just "\" and not "\example.com\root".
		     */
			...
			dr = getReferral(trans, domain, root, path, auth);
			if (dr != null) {
			    dr.pathConsumed -= 1 + domain.length() + 1
				    + root.length();
			    dr.link = link;
	...
	if (dr == null && path != null) {
		...
		if (_klen == key.length()) {
		    ...
		    match = _key.regionMatches(0, key, 0, _klen)
			    && key.charAt(_klen) == '\\';
		}
	 ...
	 * causes problems matching keys in resolve() where an extra slash
	 * causes a mismatch. This strips trailing slashes from all keys to
	...
	if (referrals != null
		&& (System.currentTimeMillis() + 10000) > referrals.expiration) {
	    referrals = null;
	}
	if (referrals == null) {
	    referrals = new CacheEntry(0);
