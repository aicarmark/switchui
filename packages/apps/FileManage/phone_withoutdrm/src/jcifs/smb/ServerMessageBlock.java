...
abstract class ServerMessageBlock extends Response implements Request,
    ...
    static final byte[] header = { (byte) 0xFF, (byte) 'S', (byte) 'M',
    ...
    byte command, flags;
    int headerStart, length, batchLevel, errorCode, flags2, tid, pid, uid, mid,
	    wordCount, byteCount;
	...
	try {
		...
		System.arraycopy(str.getBytes(UNI_ENCODING), 0, dst, dstIndex,
			str.length() * 2);
	...
	try {
		...
		while (src[srcIndex + len] != (byte) 0x00
		    ...
		    if (len > maxLen) {
			if (log.level > 0)
			    Hexdump.hexdump(System.err, src, srcIndex,
				    maxLen < 128 ? maxLen + 8 : 128);
    ...
    String readString(byte[] src, int srcIndex, int srcEnd, int maxLen,
	    boolean useUnicode) {
	int len = 0;
	String str = null;
	try {
	...
	return obj instanceof ServerMessageBlock
		&& ((ServerMessageBlock) obj).mid == mid;
    }
	...
	String str = errorCode == 0 ? "0" : SmbException
		.getMessageByCode(errorCode);
	return new String("command=" + c + ",received=" + received
		+ ",errorCode=" + str + ",flags=0x"
		+ Hexdump.toHexString(flags & 0xFF, 4) + ",flags2=0x"
		+ Hexdump.toHexString(flags2, 4) + ",signSeq=" + signSeq
		+ ",tid=" + tid + ",pid=" + pid + ",uid=" + uid + ",mid=" + mid
		+ ",wordCount=" + wordCount + ",byteCount=" + byteCount);
    }
}
