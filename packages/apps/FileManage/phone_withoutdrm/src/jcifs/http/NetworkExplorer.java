...
/**
 * This servlet may be used to "browse" the entire hierarchy of resources on an
 * SMB network like one might with Network Neighborhood or Windows Explorer. The
 * users credentials with be negotiated using NTLM SSP if the client is
...
public class NetworkExplorer extends HttpServlet {
    ...
    protected void doFile(HttpServletRequest req, HttpServletResponse resp,
	...
	if ((n = url.lastIndexOf('.')) > 0
		&& (type = url.substring(n + 1)) != null && type.length() > 1
		&& type.length() < 6) {
    ...
    protected void doDirectory(HttpServletRequest req,
	    HttpServletResponse resp, SmbFile dir) throws IOException {
	PrintWriter out;
	SmbFile[] dirents;
	SmbFile f;
	...
	out.print("<a class=\"sort\" style=\"width: " + maxLen
		+ ";\" href=\"?fmt=detail&sort=name\">Name</a>");
	...
	out.println((dirCount + fileCount) + " objects (" + dirCount
		+ " directories, " + fileCount + " files)<br>");
	...
	try {
	    ...
	    redir = new StringBuffer(redir.substring(0, redir.length()
