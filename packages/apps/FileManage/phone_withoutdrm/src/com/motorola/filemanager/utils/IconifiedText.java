...
public class IconifiedText {
	...
	if (mMiMeType != null) {
		...
		} else if (preSubtype.equals("msword")
			|| preSubtype.equals("rtf") || preSubtype.equals("csv")
			|| preSubtype.equals("vnd") || preSubtype.equals("pdf")) {
	...
	if (filename.endsWith("/")) {
	    fileIcon = context.getResources().getDrawable(
		    R.drawable.ic_thb_generic_folder);
		...
		if (filename.endsWith(".zip")) { // Check if it's .zip file.
		    fileIcon = context.getResources().getDrawable(
			    R.drawable.ic_thb_mimetype_zip);
		} else if (filename.endsWith(".dcf")) { // Check if it's drm
							// file
		    fileIcon = context.getResources().getDrawable(
			    R.drawable.ic_launcher_drm_file);
	...
	if (file.isDirectory()) {
	    ...
	    info = context.getResources().getString(R.string.size_kb,
		    Long.toString(size));
	    pathInfo = file.getAbsolutePath();
	    if (fileIcon == null) {
	...
	if (tag == 0) {
	    fileIcon = context.getResources().getDrawable(
		    R.drawable.ic_thb_computer);
	} else if (tag == 1) {
	    fileIcon = context.getResources()
		    .getDrawable(R.drawable.ic_thb_add);
	} else if (tag == 2) {
	    fileIcon = context.getResources().getDrawable(
		    R.drawable.ic_thb_generic_folder);
	} else if (tag == 3) {
	    fileIcon = context.getResources().getDrawable(
		    R.drawable.ic_thb_motoconnect);
	}
	...
	public int compare(IconifiedText object1, IconifiedText object2) {
		...
		return object1.getText().toLowerCase()
			.compareTo(object2.getText().toLowerCase());
	    }
