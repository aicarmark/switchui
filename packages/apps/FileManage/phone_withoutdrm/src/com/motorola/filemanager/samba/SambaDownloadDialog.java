...
public class SambaDownloadDialog extends Activity {
	...
	public void onReceive(Context arg0, Intent intent) {
		...
		if (action.equals(COMPLETE_INTENT)) {
		    SambaExplorer.log(TAG + " refresh to complete dialog");
		    showResultView();
		    ...
		    SambaExplorer.log(TAG + " Prg view intent is received");
		    showProgressView();
		    getProgressParameter(intent);
		}
	    }
	...
	if (action != null) {
	    if (action.equals(COMPLETE_INTENT)) {
		SambaExplorer.log(TAG + " Create complete result dialog");
		showResultView();
		getResultParameter(getIntent());
	    } else if (action.equals(PRG_VIEW_INTENT)) {
		SambaExplorer.log(TAG + " Create progress dialog");
		showProgressView();
	...
	int dialogSize = FileManagerApp.getDialogSize(this);
	View myView = findViewById(Window.ID_ANDROID_CONTENT);
	myView.setMinimumWidth(dialogSize);
    }
	...
	setContentView(R.layout.pending_download_result);
	...
	try {
		...
		if (!toFolder.equals(mToFolder)) {
		    mToFolder = toFolder;
		    int index = mToFolder.lastIndexOf('/',
			    mToFolder.length() - 3) + 1;
		    String toString = this.getResources()
			    .getString(R.string.to)
			    + " "
			    + mToFolder.substring(index);
		    ((TextView) findViewById(R.id.destination_folder))
			    .setText(toString);
		}
	    }
		...
		if (!filename.equals(mFileName)) {
		    ...
		    try {
			fileIcon = this.getPackageManager().getActivityIcon(
				tempIntent);
		...
		size = size < 1 ? 1 : size;
		String sizeString = this.getResources().getString(
			R.string.file_size)
			+ " "
			+ String.valueOf(size)
			+ " "
		...
		String percent = this.getResources().getString(
			R.string.percent_sign);
		mPercentage.setText(String.valueOf(step) + percent);
	    }
	...
	try {
		...
		String outText = String.format(
			getResources().getString(R.string.transfer_result),
			transFiles);
