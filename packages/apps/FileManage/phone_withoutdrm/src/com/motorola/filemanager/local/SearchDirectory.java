...
public class SearchDirectory extends Thread {
    ...
    SearchDirectory(File directory, Context context, Handler handler,
	    String filterString) {
	...
	if (files != null) {
		...
		if (currentFile.isDirectory()) {
			...
			mDirList.add(IconifiedText.buildIconItem(mContext,
				currentFile));
		    }
		    scanDirectory(currentFile);
		} else {
		    if (currentFile.getName().indexOf(mFilterString) >= 0) {
			mFileList.add(IconifiedText.buildIconItem(mContext,
				currentFile));
		    }
		}
	    }
