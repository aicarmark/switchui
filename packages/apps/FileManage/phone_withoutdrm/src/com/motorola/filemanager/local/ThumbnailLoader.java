...
public class ThumbnailLoader extends Thread {
    ...
    /*
     * We need at least 2 MB of available memory to continue thumbnail
     * generation
     */
	...
	if (runningAppProcesses == null) {
	    FileManagerApp.log(TAG
		    + "runningAppProcess null, exit to avoid low memory");
	    listFile = null;
	    mContext = null;
	    // handler.obtainMessage(FileManagerActivity.MESSAGE_LOAD_THUMBNAIL_END).sendToTarget();
	    return;
	...
	for (int x = 0; x < count; x++) {
		...
		try {
			...
			options.inTempStorage = tempStorage; // Use tempstorage
							     // to decode
			bitmap = BitmapFactory.decodeFile(fileNameWithPath,
				options);
		    }
		} catch (Exception e) {
		...
		try {
		    bitmap = ThumbnailUtils.createVideoThumbnail(FileUtils
			    .getFile(file, text.getText()).getPath(),
			    MediaStore.Images.Thumbnails.MICRO_KIND);
		} catch (Exception ex) {
		    FileManagerApp
			    .log(TAG + "Video thumbnail is not retrieved");
		} catch (OutOfMemoryError e) {
		    FileManagerApp.log(TAG
			    + "Video Thumbnail loader out of memory");
		    cancel = true;
		    return;
		}
	    } else {
		...
		Message msg = handler.obtainMessage(
			FileManagerActivity.MESSAGE_ICON_CHANGED, x,
			(int) (this.getId()));
		if (msg != null) {
		    msg.obj = drawable;
		    if (!cancel) {
