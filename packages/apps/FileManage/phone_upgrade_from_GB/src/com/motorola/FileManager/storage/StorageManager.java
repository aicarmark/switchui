...
public class StorageManager {
	...
	public void onStorageStateChanged(String path, String oldState,
	    ...
	    final int size = mListeners.size();
	    for (int i = 0; i < size; i++) {
		mListeners.get(i).sendStorageStateChanged(path, oldState,
			newState);
	    }
	}
    }
	...
	public int addListener(OnObbStateChangeListener listener) {
	    final ObbListenerDelegate delegate = new ObbListenerDelegate(
		    listener);
	...
	void sendObbStateChanged(String path, int state) {
	    ObbStateChangedStorageEvent e = new ObbStateChangedStorageEvent(
		    path, state);
    ...
    /**
     * Private base class for messages sent between the callback thread and the
     * target looper handler.
     */
	...
	public StorageStateChangedStorageEvent(String p, String oldS,
		String newS) {
	...
	ListenerDelegate(StorageEventListener listener) {
		...
		public void handleMessage(Message msg) {
			...
			mStorageEventListener
				.onUsbMassStorageConnectionChanged(ev.available);
			...
			mStorageEventListener.onStorageStateChanged(ev.path,
	...
	void sendShareAvailabilityChanged(boolean available) {
	    UmsConnectionChangedStorageEvent e = new UmsConnectionChangedStorageEvent(
		    available);
	    mHandler.sendMessage(e.getMessage());
	}

	void sendStorageStateChanged(String path, String oldState,
		String newState) {
	    StorageStateChangedStorageEvent e = new StorageStateChangedStorageEvent(
     ...
     * 
     * @param tgtLooper
     *            The {@android.os.Looper} which events will be received on.
     * 
     *            <p>
     *            Applications can get instance of this class by calling
     *            {@link android.content.Context#getSystemService(java.lang.String)}
     *            with an argument of
     *            {@link android.content.Context#STORAGE_SERVICE}.
     * 
     * @hide
     */
	...
	if (mMountService == null) {
	    Log.e(TAG,
		    "Unable to connect to mount service! - is it running yet?");
	    return;
	}
	...
	try {
	    final int nonce = mObbActionListener.addListener(listener);
	    mMountService
		    .unmountObb(filename, force, mObbActionListener, nonce);
	    return true;
	} catch (RemoteException e) {
	    Log.e(TAG, "Failed to mount OBB", e);
	}
