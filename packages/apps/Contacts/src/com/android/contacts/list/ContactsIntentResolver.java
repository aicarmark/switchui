...
//<!-- MOTOROLA MOD Start: IKPIM-491 -->
        } else if ("motorola.intent.ACTION_MULTIPLE_PICK".equals(action)) {
            final String resolvedType = intent.resolveType(mContext);
            if (Contacts.CONTENT_TYPE.equals(resolvedType)) {
                request.setActionCode(ContactsRequest.ACTION_PICK_MULTIPLE_CONTACT);
            } else if (People.CONTENT_TYPE.equals(resolvedType)) {
                request.setActionCode(ContactsRequest.ACTION_PICK_MULTIPLE_CONTACT);
                request.setLegacyCompatibilityMode(true);
            } else if (Phone.CONTENT_TYPE.equals(resolvedType)) {
                request.setActionCode(ContactsRequest.ACTION_PICK_MULTIPLE_PHONE);
            } else if (Phones.CONTENT_TYPE.equals(resolvedType)) {
                request.setActionCode(ContactsRequest.ACTION_PICK_MULTIPLE_PHONE);
                request.setLegacyCompatibilityMode(true);
            } else if (StructuredPostal.CONTENT_TYPE.equals(resolvedType)) {
                request.setActionCode(ContactsRequest.ACTION_PICK_MULTIPLE_POSTAL);
            } else if (ContactMethods.CONTENT_POSTAL_TYPE.equals(resolvedType)) {
                request.setActionCode(ContactsRequest.ACTION_PICK_MULTIPLE_POSTAL);
                request.setLegacyCompatibilityMode(true);
            }
//<!-- MOTOROLA MOD End of IKPIM-491 -->
            ...
            request.setActionCode(ContactsRequest.ACTION_INSERT_OR_EDIT_CONTACT);
                ...
                || android.provider.Contacts.Intents.UI.FILTER_CONTACTS_ACTION.equals(action)) { // IKHSS6-12133
            ...
            if ("call".equals(intent.getStringExtra(SearchManager.ACTION_MSG))) {
                ...
                if (!TextUtils.isEmpty(query)) {
                    ...
                    if (ContactsUtils.haveTwoCards(mContext)) {
                        ...
                        newIntent.putExtra("phoneNumber", query);
                    } else {
                        newIntent = new Intent(Intent.ACTION_CALL_PRIVILEGED,
                                Uri.fromParts("tel", query, null));
                    }
                    // BEGIN Motorola, FTR 36344, IKHSS6-8444
                    MEDialer.onDial(mContext, newIntent, MEDialer.DialFrom.CONTACTS);
                    // END IKHSS6-8444
        ...
        // so we need to re-dispatch from here to the intended target.
            ...
            if (contactUri != null) {
                ...
                MEDialer.onDial(mContext, newIntent, MEDialer.DialFrom.FAVORITES);
                request.setRedirectIntent(newIntent);
            }
            // END IKHSS6-8444
