...
public class JoinContactListAdapter extends ContactListAdapter {
    ...
    private static String not_cards_selection = null;
    public JoinContactListAdapter(Context context) {
        ...
        not_cards_selection = filterCardSelection();
    }
    ...
    public void configureLoader(CursorLoader cursorLoader, long directoryId) {
        ...
        // Motorola: Filter out SIM contacts
        loader.setSelection(Contacts._ID + "!=" + mTargetContactId + " AND " + not_cards_selection);
...
//        loader.setSelectionArgs(new String[]{String.valueOf(mTargetContactId)});
    ...
    private String filterCardSelection() {
        StringBuilder filterSelection = new StringBuilder();
        filterSelection.append(Contacts._ID + " NOT IN ( ");

        Cursor simIDCursor=getContext().getContentResolver().query(RawContacts.CONTENT_URI,
                new String []{RawContacts.CONTACT_ID}, RawContacts.ACCOUNT_TYPE+"=?" ,
                new String[]{HardCodedSources.ACCOUNT_TYPE_CARD}, null);
        if (simIDCursor!=null) {
            int rowCount=simIDCursor.getCount();
            int i=1;
            while(simIDCursor.moveToNext()){
                filterSelection.append(simIDCursor.getLong(0));
        ...
        filterSelection.append("0 )");

        return filterSelection.toString();
    }
}
