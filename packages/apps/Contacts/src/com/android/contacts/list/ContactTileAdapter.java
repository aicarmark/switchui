...
import com.motorola.contacts.group.LocalGroupMemberLoader; //MOT MOD - IKPIM-991: Group redesign feature on ics
import com.motorola.contacts.group.LocalGroupMemberLoader.LocalGroupDetailMemberQuery;
...
public class ContactTileAdapter extends BaseAdapter {
    ...
    private int mNameRawContactIdIndex;
    ...
    private void bindColumnIndices() {
        ...
        if (mDisplayType == DisplayType.GROUP_MEMBERS) {
            ...
            */
            mIdIndex = LocalGroupDetailMemberQuery.CONTACT_PHOTO_ID_COLUMN_INDEX;
            mLookupIndex = LocalGroupDetailMemberQuery.CONTACT_LOOKUP_KEY_COLUMN_INDEX;
            mPhotoUriIndex = LocalGroupDetailMemberQuery.CONTACT_PHOTO_URI_COLUMN_INDEX;
            mNameIndex = LocalGroupDetailMemberQuery.CONTACT_DISPLAY_NAME_PRIMARY_COLUMN_INDEX;
            mStarredIndex = LocalGroupDetailMemberQuery.CONTACT_STARRED_COLUMN_INDEX;
            mPresenceIndex = LocalGroupDetailMemberQuery.CONTACT_PRESENCE_STATUS_COLUMN_INDEX;
            mStatusIndex = LocalGroupDetailMemberQuery.CONTACT_STATUS_COLUMN_INDEX;
            ...
            mNameRawContactIdIndex = ContactTileLoaderFactory.NAME_RAW_CONTACT_ID;
    ...
    private int getDividerPosition(Cursor cursor) {
        //MOTO MOD BEGIN, IKHSS6UPGR-8987, Since the functionality of Query can return null, no need to throw exception, we have add the null check everywhere,
        //just set it as a empty cursor is ok.
        switch (mDisplayType) {
            case STREQUENT:
    ...
    private ContactEntry createContactEntryFromCursor(Cursor cursor, int position) {
        ...
        contact.nameRawContactId = cursor.getLong(mNameRawContactIdIndex);
