...
/*package*/ class CallLogQueryHandler extends AsyncQueryHandler {
    ...
    private Cursor createHeaderCursorFor(int section) {
        ...
        matrixCursor.addRow(new Object[]{
                0L, "", 0L, 0L, 0, "", "", "", null, 0, null, null, null, null, 0L, null, 0, null, null,  0,//MOTO Dialer code
                section
        });
        return matrixCursor;
    }
     ...
     */
    public void fetchVoicemailOnly(int networkType) {
        cancelFetch();
        invalidate();
        //MOT Dialer Start
        ...
        fetchCalls(QUERY_NEW_CALLS_TOKEN, true /*isNew*/, CallLogViewTypeHelper.VIEW_OPTION_VOICEMAIL/*viewType*/, networkType);
        fetchCalls(QUERY_OLD_CALLS_TOKEN, false /*isNew*/, CallLogViewTypeHelper.VIEW_OPTION_VOICEMAIL/*viewType*/, networkType);
        //MOT Dialer End
    }
    ...
    private void fetchCalls(int token, boolean isNew, int viewType, int networkType) {
        ...
        //}
        if (viewType > CallLogViewTypeHelper.VIEW_OPTION_ALL) {
            selection = String.format("(%s) AND (%s = ?)", selection, Calls.TYPE);
            selectionArgs.add(Integer.toString(getFilterString(viewType)));
        }
        String needNewCallsSearch = isNewCallsSearchNeeded(viewType) ? null:Integer.toString(viewType);
        if (networkType > CallLogViewTypeHelper.NETWORK_TYPE_ALL) {
            selection = String.format("(%s) AND (%s = ?)", selection, ContactsUtils.CallLog_NETWORK);
            selectionArgs.add(Integer.toString(getNetworkString(networkType)));
        }
        if (DialtactsActivity.DBG) {
            Log.d(TAG, "The selection=== " + selection
                    + "The selectionArgs=== " + selectionArgs.toString()
                    + "The token=== " + token
                    + "The needNewCallsSearch=== " + needNewCallsSearch);
        }
        startQuery(token, needNewCallsSearch, Calls.CONTENT_URI_WITH_VOICEMAIL,
    ...
    protected synchronized void onQueryComplete(int token, Object cookie, Cursor cursor) {
        //MOT Dialer Code Start - IKHSS6UPGR-6267
        if (cursor == null) {
            Log.w(TAG, "somethings wrong when db query, so ignoring: " + token);
            return;
        //MOT Dialer Code End - IKHSS6UPGR-6267
        } else if (token == QUERY_NEW_CALLS_TOKEN) {
            //MOT Dialer Code - IKHSS6UPGR-10741 - Start
            // Firstly release the old cursor to avoid the cursor leak.
            MoreCloseables.closeQuietly(mNewCallsCursor);
            //MOT Dialer Code - IKHSS6UPGR-10741 - End
        ...
        } else if (token == QUERY_OLD_CALLS_TOKEN && cursor != null) {
            // 2012.08.24 to fix IKCBSMMCPPRC-1753 to avoid the cursor leak
            MoreCloseables.closeQuietly(mOldCallsCursor);
            // Store the returned cursor.
            mOldCallsCursor = new ExtendedCursor(
                    cursor, CallLogQuery.SECTION_NAME, CallLogQuery.SECTION_OLD_ITEM);
        } else if (token == QUERY_VOICEMAIL_STATUS_TOKEN && cursor != null) {
    ...
    private Cursor createMergedCursor() {
        try {
            //MOT Dialer Code : we may not query new call in outgoing & receive case
    ...
    private int getFilterString(int viewType) {
        /*
    ...
    private int getNetworkString(int networkType) {
        /*
         * network types in calllogs db
         * TelephonyManager.PHONE_TYPE_CDMA = 1;
         * TelephonyManager.PHONE_TYPE_GSM = 2;
         */
        int callLogType;
        switch (networkType) {
            case CallLogViewTypeHelper.NETWORK_TYPE_CDMA:
                callLogType = TelephonyManager.PHONE_TYPE_CDMA;
                break;
            case CallLogViewTypeHelper.NETWORK_TYPE_GSM:
                callLogType = TelephonyManager.PHONE_TYPE_GSM;
                break;
            default:
                return 0;
        }
    ...
    public void fetchFilterCalls(int callType, int networkType) {
        cancelFetch();
        invalidate();
        fetchCalls(QUERY_NEW_CALLS_TOKEN, true /*isNew*/, callType, networkType);
        fetchCalls(QUERY_OLD_CALLS_TOKEN, false /*isNew*/, callType, networkType);
    }

    private boolean isNewCallsSearchNeeded(int viewType) {
        return (viewType == CallLogViewTypeHelper.VIEW_OPTION_MISSED) &&
                   (viewType == CallLogViewTypeHelper.VIEW_OPTION_VOICEMAIL) &&
                   (viewType == CallLogViewTypeHelper.VIEW_OPTION_ALL);
    }
    //Mot Dialer end
}
