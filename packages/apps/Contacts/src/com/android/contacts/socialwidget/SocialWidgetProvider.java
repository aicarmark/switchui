...
public class SocialWidgetProvider extends AppWidgetProvider {
    ...
    public static void loadWidgetData(final Context context,
        ...
        if (contactUri == null) {
            ...
            //IKTABLETMAIN-4494, MOTO-MOD, reboot after widget's contact deleted, contactUri is null, show not exist, begin.
            ...
            //setPhoto(views, ContactBadgeUtil.loadPlaceholderPhoto(context)); for pass build
            //Moto Mod IKHSS6-4499, Config the dare theme by xml
            setPhoto(views, ContactBadgeUtil.loadDefaultAvatarPhoto(context, false, context.getResources().getBoolean(R.bool.contacts_dark_ui)));
            appWidgetManager.updateAppWidget(widgetId, views);
            //IKTABLETMAIN-4494, MOTO-MOD, End
            return;
        }
    ...
    private static void bindRemoteViews(final Context context, final int widgetId,
        ...
        if (!contactData.isLoaded()) {
            setDisplayNameAndSnippet(context, views,
                    context.getString(R.string.invalidContactMessage), null, null, null);
            //MOT MOD BEGIN - IKHSS7-2299, contact no longer exist, so cancel the pending intent, this is needed to keep behavior consistent before/after phone repower.
            Uri contactUri = SocialWidgetSettings.getInstance().getContactUri(context, widgetId);
            if (contactUri != null) {
                final PendingIntent pendingIntent = PendingIntent.getActivity(context,
                        0, prepareOnClickIntent(contactUri), 0);
                if (pendingIntent != null) pendingIntent.cancel();
            //MOT MOD END - IKHSS7-2299
            //IKTABLETMAIN-4494, MOTO-MOD, contact no longer exist, remove the record, Beging
                int widgetIds[] = new int[1];
                widgetIds[0] = widgetId;
                SocialWidgetSettings.getInstance().remove(context, widgetIds);
            } else {
                Log.e(TAG, "contactUri of social widget: " + widgetId + "is null");
            }
            //IKTABLETMAIN-4494, MOTO-MOD, End
            //Moto Mod IKHSS6-4499, Config the dare theme by xml
            setPhoto(views, ContactBadgeUtil.loadDefaultAvatarPhoto(context, false, context.getResources().getBoolean(R.bool.contacts_dark_ui)));
            ...
            //Moto Mod Beging IKHSS6-4499, Config the dare theme by xml
            setPhoto(views, photo != null
                    ? BitmapFactory.decodeByteArray(photo, 0, photo.length, options)
                            : ContactBadgeUtil.loadDefaultAvatarPhoto(context, false, context.getResources().getBoolean(R.bool.contacts_dark_ui)));
            //Moto Mod END IKHSS6-4499
            // MOT MOD END    IKHSS7-765
            // TODO: Rotate between all the stream items?

            // OnClick launch QuickContact
            final Intent intent = prepareOnClickIntent(contactData.getLookupUri()); //IKTABLETMAIN-4375, MOTO-MOD;
            final PendingIntent pendingIntent = PendingIntent.getActivity(context,
                    0, intent, 0);
    ...
    //IKTABLETMAIN-4375, MOTO-MOD, prepareOnClickIntent is used to prepare the intent by the contactUri, make it a method to be easily reused, Begin
    private static Intent prepareOnClickIntent(Uri contactUri) {
    ...
    //IKTABLETMAIN-4375, MOTO-MOD, End
    ...
    private static void setDisplayNameAndSnippet(Context context, RemoteViews views,
        ...
        //Mot Mod Begin IKTABLETMAIN-5321
        if(name == null) {
            name =  context.getString(android.R.string.unknownName);
        }
        //Mot Mod End IKTABLETMAIN-5321
        sb.append(name);
