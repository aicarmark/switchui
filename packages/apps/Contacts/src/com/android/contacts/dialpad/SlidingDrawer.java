...
public class SlidingDrawer extends ViewGroup {
    ...
    @Override
        protected void onFinishInflate() {
            mHandleClick = findViewById(com.android.contacts.R.id.handle_click);
            mHandle = findViewById(mHandleId);
            if (mHandle == null || mHandleClick == null) {
                throw new IllegalArgumentException("The handle attribute is must refer to an"
                        + " existing child.");
            }
            //mHandle.setOnClickListener(new DrawerToggler());
            mHandleClick.setOnClickListener(new DrawerToggler());
    ...
    /*@Override
        public boolean onInterceptTouchEvent(MotionEvent event) {
            ...
            // to differentiate the show button from the other part of handle
            handle.getHitRect(frame);
            Rect cFrame = new Rect();
            mHandleClick.getHitRect(cFrame);

            Rect nFrame = new Rect();

            nFrame.left   = frame.left + cFrame.left;
            nFrame.right  = frame.left + cFrame.right - cFrame.left;
            nFrame.top    = frame.top + cFrame.top;
            nFrame.bottom = frame.top + cFrame.bottom - cFrame.top;

            if (!mTracking && !nFrame.contains((int) x, (int) y)) {
                return false;
            }
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                // to not change the state of button of 1,2,3 when closing SlidingDrawer
    ...
    private class DrawerToggler implements OnClickListener {
        public void onClick(View v) {
            ...
            // trackball.
            v.performHapticFeedback(HapticFeedbackConstants.VIRTUAL_KEY);//ChinaDev
            if (mAnimateOnClick) {
                animateToggle();
            } else {
                toggle();
