...
public class HanziToPinyin {
    ...
    private static final boolean VDBG = false/*DialtactsActivity.VDBG*/;
    ...
    public static final byte[][] PINYINS = {
            ...
            /*rm this useless pinyin{ 72, 77, 0, 0, 0, 0 },*/ { 72, 79, 78, 71, 0, 0 }, { 72, 79, 85, 0, 0, 0 },
            ...
            { 76, 85, 78, 0, 0, 0 }, { 76, 85, 79, 0, 0, 0 }, /*rm this useless pinyin{ 77, 0, 0, 0, 0, 0 },*/
            ...
            { 78, 85, 65, 78, 0, 0 }, { 78, 85, 69, 0, 0, 0 }, /*@*/{ 78, 85, 78, 0, 0, 0 }, { 78, 85, 79, 0, 0, 0 },
    ...
    // BEGIN Motorola, ODC_001639, 2013-01-31, SWITCHUITWO-601,synchronized from com.android.providers.contacts.HanziToPinyin to ensure a contact can be searched in contacts
    // and can be hightlight in diapad at the same time
    /* 2012-11-05, NWQ834 merged from Ironprime_TD */
    /**
     * Rectify tokens array for correct Pinyin normally used in Chinese names
     */
    public static final Token[] POLYPHONE_RECTIFY = {
        // extendable for all incorrect converted pinyin tokens
        new Token(Token.PINYIN, Character.toString('\u91cd'), "CHONG"),      //  zhong-> chong
        new Token(Token.PINYIN, Character.toString('\u513f'), "ER"),         //  qun -> er
        new Token(Token.PINYIN, Character.toString('\u8d3e'), "JIA"),         //  gu   -> jia        
        new Token(Token.PINYIN, Character.toString('\u533a'), "OU"),         //  qu   -> ou
        new Token(Token.PINYIN, Character.toString('\u6734'), "PIAO"),       //  pu   -> piao
        new Token(Token.PINYIN, Character.toString('\u4ec7'), "QIU"),        //  chou -> qiu
        new Token(Token.PINYIN, Character.toString('\u5355'), "SHAN"),       //  dan  -> shan
        new Token(Token.PINYIN, Character.toString('\u6c88'), "SHEN"),       //  chen -> shen
        new Token(Token.PINYIN, Character.toString('\u89e3'), "XIE"),        //  jie  -> xie
        new Token(Token.PINYIN, Character.toString('\u6bb7'), "YIN"),        //  yan  -> yin
        new Token(Token.PINYIN, Character.toString('\u4fde'), "YU"),         //  shu  -> yu
        new Token(Token.PINYIN, Character.toString('\u4e50'), "YUE"),        //  le   -> yue
        new Token(Token.PINYIN, Character.toString('\u67e5'), "ZHA"),        //  cha  -> zha
        new Token(Token.PINYIN, Character.toString('\u7fdf'), "ZHAI"),       //  dxxx -> zhai
        new Token(Token.PINYIN, Character.toString('\u66fe'), "ZENG"),       //  ceng -> zeng
        new Token(Token.PINYIN, Character.toString('\u4e48'), "ME"),       //  mao -> me
        new Token(Token.PINYIN, Character.toString('\u5475'), "HE"),       //   a -> he
        new Token(Token.PINYIN, Character.toString('\u7ED9'), "GEI"),       //   ge -> gei
        new Token(Token.PINYIN, Character.toString('\u77BF'), "QU")       //   ju -> qu
    };
/* NWQ834 End */
    // END SWITCHUITWO-601
    ...
    private void rectifyToken(Token token) {
        if (token != null) {
            // check if the given token is in the RECTIFY_POLYPHONE table
            int i = 0;
            do{
                String srcChar = POLYPHONE_RECTIFY[i].source;
                //Log.v(TAG, "table["+i+"], srcChar = "+srcChar+", tokenSrc = "+token.source+", len="+POLYPHONE_RECTIFY.length);
                if (token.source.equals(srcChar)) {
                    // found, target pinyin need to rectified
                    token.target = POLYPHONE_RECTIFY[i].target;
                    break;
                }
                i++;
            } while(i<POLYPHONE_RECTIFY.length);
        }
        return;
    }
    ...
    private Token getToken(char character) {
        ...
        if (offset < 0) {
            ...
            //Log.v(TAG, "go through Collator to compare, total loop len= "+end+", character= "+character);

            while (begin <= end) {
                offset = (begin + end) / 2;
                final String unihan = Character.toString(UNIHANS[offset]);
                cmp = COLLATOR.compare(letter, unihan);
                //Log.v(TAG, "comparing: "+letter+" vs ("+unihan+"), cmp = "+cmp+", offset = "+offset);
        ...
        //Log.v(TAG, "--- inital token.source= "+token.source+", type= "+token.type+", token.target  = "+token.target);

        // rectify token if needed
        rectifyToken(token);
        //Log.v(TAG, "+++ rectified token.source= "+token.source+", type= "+token.type+", token.target  = "+token.target+", token="+token);
        return token;
    }
