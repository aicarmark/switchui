...
public class ContactEditorUtils {
    ...
    private List<AccountWithDataSet> getWritableAccounts() {
        return mAccountTypes.getAccountsWithCard(true); // MOT CHINA
    }
     ...
     */
    public boolean shouldShowAccountChangedNotification(Context context) {
        final AccountWithDataSet defaultAccount = getDefaultAccount();
        if (defaultAccount != null && HardCodedSources.ACCOUNT_TYPE_CARD.equals(defaultAccount.type)) {
            final int phoneType = SimUtility.getTypeByAccountName(defaultAccount.name);
            // Return true if default account is card but card is full
            if (SimUtility.getFreeSpace(context.getContentResolver(), phoneType) <= 0) {
                return true;
            }
        }
        return shouldShowAccountChangedNotification();
    }
