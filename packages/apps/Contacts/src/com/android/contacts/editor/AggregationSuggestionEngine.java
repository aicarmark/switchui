...
public class AggregationSuggestionEngine extends HandlerThread {
    ...
    public boolean quit() {
        ...
        mDataCursor = null;
        //MOTO MOD BEGIN
        if (mContentObserver != null && mContext != null && mContext.getContentResolver() != null) {
        //MOTO MOD END
    ...
    public void onNameChange(ValuesDelta values) {
        ...
        if (mSuggestionsUri != null) {
            if (mContentObserver == null) {
                ...
                if (mContext != null && (cr = mContext.getContentResolver()) != null) {
                    cr.registerContentObserver(Contacts.CONTENT_URI, true, mContentObserver);
                }
                //MOTO MOD END
            }
        } else if (mContentObserver != null && mContext != null &&  mContext.getContentResolver() != null) {
            mContext.getContentResolver().unregisterContentObserver(mContentObserver);
            mContentObserver = null;
        }
        scheduleSuggestionLookup();
    ...
    private void loadAggregationSuggestions(Uri uri) {
        // MOT MOD BEGIN -- IKHSS7-8526
        if (mContext == null) {
            return;
        }
        // MOT MOD END -- IKHSS7-8526
