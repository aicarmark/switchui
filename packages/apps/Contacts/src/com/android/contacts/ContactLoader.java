...
public class ContactLoader extends Loader<ContactLoader.Result> {
    ...
    private boolean mExportDataOnly; //MOT MOD - IKHSS7-2028
    // MOTO MOD BEGIN
    private boolean  shouldForceLoad = false;
    private LoadContactTask mLoadContactTask;
    public static int RUN_NEW_TASK = 101;
    private Handler mHandler = new Handler() {

        @Override
        public void handleMessage(Message msg) {
             if (msg.what == RUN_NEW_TASK) {
                 mLoadContactTask = new LoadContactTask();
                 mLoadContactTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, (Void[])null);
    ...
    public static final class Result {
        ...
        private Result(Uri requestedUri, Uri uri, Uri lookupUri, long directoryId, String lookupKey,
                ...
                boolean isUserProfile, String accountType, boolean hasPhoneNumber) {
        ...
        private Result(Result from) {
            ...
            mAccountType = from.mAccountType;
            mHasPhoneNumber = from.mHasPhoneNumber;
        }
        ...
        public boolean singleIsRestricted() {
            if (mEntities!=null) {
                for(Entity entity: mEntities) {
                    ContentValues cv = entity.getEntityValues();
                    Integer isRestricted = cv.getAsInteger("is_restricted");
                    if (isRestricted==null || isRestricted!=1) {
                        return false;
                    }
                }
                //all raw contacts are restrcited contact
                return true;
            }
            return false;
        }
    ...
    private static class ContactQuery {
        final static String[] COLUMNS = new String[] {
                ...
                "is_restricted", //MOT MOD - IKHSS7-2028

                Contacts.HAS_PHONE_NUMBER, // MOT China
        };
        ...
        public final static int IS_RESTRICTED = 65; //MOT MOD - IKHSS7-2028

        public final static int HAS_PHONE_NUMBER = 66; //MOT CHINA
    }
    ...
    private final class LoadContactTask extends AsyncTask<Void, Void, Result> {
        //MOTO MOD BEGIN
        public AsyncPhotoLoader mAsyncPhotoLoader;
        //MOTO MOD END
        @Override
        protected Result doInBackground(Void... args) {
            try {
                final ContentResolver resolver = getContext().getContentResolver();
                Uri uriCurrentFormat = ContactLoaderUtils.ensureIsContactUri( //MOT MOD - IKHSS7-2028
                        resolver, mLookupUri);

                //MOT MOD BEGIN - IKHSS7-2028
                if (mExportDataOnly) {
                    uriCurrentFormat = uriCurrentFormat.buildUpon().
                        appendQueryParameter("for_export_only", "1").build();
                }
                //MOT MOD END - IKHSS7-2028
            ...
            if (photoUri != null) {
                try {
                    ...
                    //MOTO MOD BEGIN
                        ...
                        Log.e(TAG,"OutOfMemoryError happens when load photo " + photoUri);
                    } finally {
                        fis.close();
                        fd.close();
                    }
        ...
        private Result loadContactHeaderData(final Cursor cursor, Uri contactUri) {
            ...
            final String accountType = cursor.getString(ContactQuery.ACCOUNT_TYPE);
            final boolean hasPhoneNumber = cursor.getInt(ContactQuery.HAS_PHONE_NUMBER) == 1;
            ...
            return new Result(mRequestedUri, contactUri, lookupUri, directoryId, lookupKey,
                    ...
                    customRingtone, isUserProfile, accountType, hasPhoneNumber);
        }
        ...
        private ContentValues loadRawContact(Cursor cursor) {
            ...
            cursorColumnToContentValues(cursor, cv, ContactQuery.IS_RESTRICTED); //MOT MOD - IKHSS7-2028
            cursorColumnToContentValues(cursor, cv, ContactQuery.HAS_PHONE_NUMBER); //MOT CHINA
            return cv;
        }
        ...
        protected void onPostExecute(Result result) {
            ...
            if (result.isLoaded()) {
                ...
                //MOTO MOD BEGIN
                if (mContact.getPhotoBinaryData() == null && mContact.getPhotoUri() != null && !shouldForceLoad) {
                    mContact.setLoadingPhoto(true);
                    AsyncPhotoLoader mAsyncPhotoLoader = new AsyncPhotoLoader();
                    mAsyncPhotoLoader.execute(mContact.getPhotoUri());
                }
                //MOTO MOD END
            ...
            //MOTO MOD BEGIN
            if(shouldForceLoad){
               mHandler.removeMessages(RUN_NEW_TASK);
               mHandler.sendEmptyMessage(RUN_NEW_TASK);
            }
            //MOTO MOD END
        }
    }
    ...
    public ContactLoader(Context context, Uri lookupUri) {
        this(context, lookupUri, false, false, false, false); //MOT MOD - IKHSS7-2028
    }

    //MOT MOD BEGIN - IKHSS7-2028
    public ContactLoader(Context context, Uri lookupUri, boolean loadGroupMetaData,
            boolean loadStreamItems, boolean loadInvitableAccountTypes) {
       this(context, lookupUri, loadGroupMetaData, loadStreamItems, loadInvitableAccountTypes, false);
    }
    //MOT MOD END - IKHSS7-2028

    public ContactLoader(Context context, Uri lookupUri, boolean loadGroupMetaData,
            boolean loadStreamItems, boolean loadInvitableAccountTypes, boolean exportOnly) { //MOT MOD - IKHSS7-2028
        super(context);
        mLookupUri = lookupUri;
        ...
        mExportDataOnly = exportOnly; //MOT MOD - IKHSS7-2028
    }
    ...
    protected void onForceLoad() {
        //MOTO MOD BEGIN, add this to cancel the old task and avoid multiple task in running at the same time.
        if((mLoadContactTask != null && mLoadContactTask.getStatus()== AsyncTask.Status.RUNNING) || shouldForceLoad){
    ...
    protected void onReset() {
        //MOTO MOD BEGIN
        if(mLoadContactTask != null && mLoadContactTask.mAsyncPhotoLoader != null && !mLoadContactTask.mAsyncPhotoLoader.isCancelled()){
            mLoadContactTask.mAsyncPhotoLoader.cancel(true);
            if(!mLoadContactTask.isCancelled())mLoadContactTask.cancel(true);
        }
        //MOTO MOD END
        unregisterObserver();
        mContact = null;
