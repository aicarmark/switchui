...
public final class ContactsPreferenceActivity extends PreferenceActivity {
    ...
    public void onBuildHeaders(List<Header> target) {
        ...
        //MOT MOD BEGIN IKHSS7-16432
        if(!isDisplayOptionNeeded(this)) {
            for(int i=0; i<target.size(); i++) {
                Header h = (Header) target.get(i);
                if(h.id == R.id.display_options) {
                    target.remove(i);
                    break;
                }
            }
        }
        //MOT MOD END IKHSS7-16432
    }
...
//                && !context.getResources().getBoolean(R.bool.config_display_order_user_changeable);
        return false;  //at least we will have contact storage options available
//<!-- MOTOROLA MOD End of IKPIM-491 -->
    }

    //MOT MOD BEGIN IKHSS7-16432
    /**
     * Returns true if display option is needed
     */
    public static boolean isDisplayOptionNeeded(Context context) {
        return !(!context.getResources().getBoolean(R.bool.config_sort_order_user_changeable)
                && !context.getResources().getBoolean(R.bool.config_display_order_user_changeable));
    }
    //MOT MOD END
