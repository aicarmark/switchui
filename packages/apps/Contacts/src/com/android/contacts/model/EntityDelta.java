...
public class EntityDelta implements Parcelable {
    ...
    private ArrayList<String>  m189EmailToBeDeleted = new ArrayList<String>();
    private ArrayList<String>  m189EmailToBeInserted = new ArrayList<String>();
    private boolean mAuto189 = true;
    ...
    public ValuesDelta getPrimaryEntry(String mimeType) {
        ...
        for (ValuesDelta entry : mimeEntries) {
            if (entry.isPrimary()) {
               //MOTO MOD BEGIN, as when photo is delete from UI ,it will set it to null instead of delete the recorde.
               if(Photo.CONTENT_ITEM_TYPE.equals(mimeType) && entry !=null && entry.getAsByteArray(Photo.PHOTO) == null){
                   continue;
               }else{
                    return entry;
               }
        ...
        if(Photo.CONTENT_ITEM_TYPE.equals(mimeType)){
            for (ValuesDelta entry : mimeEntries) {
                 if (entry.getAsByteArray(Photo.PHOTO) != null) {
                     return entry;
                 }
            }
        }
    ...
    public void buildDiff(ArrayList<ContentProviderOperation> buildInto) {
        ...
        final Long beforeId = mValues.getId();
        Log.v(TAG, "buildDiff(), mAuto189 = "+mAuto189+", isContactInsert= "+isContactInsert+", isContactDelete= "+isContactDelete+", isContactUpdate= "+isContactUpdate+", beforeId= "+beforeId+", insertList_size="+m189EmailToBeInserted.size()+", delList_size="+m189EmailToBeDeleted.size());

        final String accountType = mValues.getAsString(RawContacts.ACCOUNT_TYPE);
        ...
        builder = mValues.buildDiff(mContactsQueryUri, mAuto189, m189EmailToBeInserted, m189EmailToBeDeleted);
            ...
            for (ValuesDelta child : mimeEntries) {
                ...
                if (mContactsQueryUri.equals(Profile.CONTENT_RAW_CONTACTS_URI)) {
                    builder = child.buildDiff(Uri.withAppendedPath(Profile.CONTENT_URI,
                            RawContacts.Data.CONTENT_DIRECTORY), mAuto189, m189EmailToBeInserted, m189EmailToBeDeleted);
                } else {
                    builder = child.buildDiff(Data.CONTENT_URI, mAuto189, m189EmailToBeInserted, m189EmailToBeDeleted);
                }
    ...
    public void setAuto189(boolean auto189) {
    	mAuto189 = auto189;
    }

    public void clearAuto189Lists() {
    	if (m189EmailToBeInserted != null) {
    		m189EmailToBeInserted.clear();
    	}

    	if (m189EmailToBeDeleted != null) {
    		m189EmailToBeDeleted.clear();
    	}
    }
    ...
    public static class ValuesDelta implements Parcelable {
        ...
        private String normalizePhone(String phone) {
            ...
            if (!TextUtils.isEmpty(phone)) {
                ...
                String convertNumber = new String(phone);

                if (phone.startsWith("0086")) convertNumber = phone.substring(4);
                if (phone.startsWith("+86")) convertNumber = phone.substring(3);
                ...
                if (!TextUtils.isEmpty(convertNumber)&& convertNumber.length() == 11
                    ...
                    retNumber = convertNumber;
                }
            }
            return retNumber;
        }

        private void constructEmailListToBeDeleted(ArrayList<String> emailInsertList, ArrayList<String> emailDeletionList) {
            String bPhoneNumber = null;
            String aPhoneNumber = null;
            if (mBefore != null && mBefore.containsKey(Phone.NUMBER)) {
            	// get original/mBefore's
                bPhoneNumber = normalizePhone(mBefore.getAsString(Phone.NUMBER));
            }

            // get mAfter's
            aPhoneNumber = normalizePhone(getAsString(Phone.NUMBER));
            //Log.v(TAG, "bPhoneNum = "+bPhoneNumber+", aPhoneNum = "+aPhoneNumber);
 
            if (isDelete() && !TextUtils.isEmpty(bPhoneNumber)) {
                // add into deletion list
                String email = bPhoneNumber + "@189.cn";
                if (!emailDeletionList.contains(email)) {
                    emailDeletionList.add(email);
                }
            } else if (isUpdate()) {
                if (TextUtils.isEmpty(bPhoneNumber) && !TextUtils.isEmpty(aPhoneNumber)) {
                    // add into insert list
                    String email = aPhoneNumber + "@189.cn";
                    if (!emailInsertList.contains(email)) {
                        emailInsertList.add(email);
                    }	
                } else if (!TextUtils.isEmpty(bPhoneNumber) && TextUtils.isEmpty(aPhoneNumber)) {
                    // add into deletion list
                    String email = bPhoneNumber + "@189.cn";
                    if (!emailDeletionList.contains(email)) {
                        emailDeletionList.add(email);
                    ...
                    if (!aPhoneNumber.equals(bPhoneNumber)) {
                        // add 'before' into deletion list, add 'after' into insert list
                        String email = bPhoneNumber + "@189.cn";
                        if (!emailDeletionList.contains(email)) {
                            emailDeletionList.add(email);
                        }
                        email = aPhoneNumber + "@189.cn";
                        if (!emailInsertList.contains(email)) {
                            emailInsertList.add(email);
                        }
                    }
        ...
        public ContentProviderOperation.Builder buildDiff(Uri targetUri) {
            return buildDiff(targetUri, false, null, null);
        }

        public ContentProviderOperation.Builder buildDiff(Uri targetUri, boolean add189, ArrayList<String> emailInsertList, ArrayList<String> emailDeletionList) {
            // validate, add type field if empty for SIM/USIM DB mirror
            if (Phone.CONTENT_ITEM_TYPE.equals(getMimetype()) && getAsLong(Phone.TYPE)==null) {
	        //Log.v(TAG, "empty phone_type, default to: MOBILE");
                if( mAfter != null ) mAfter.put(Phone.TYPE, Phone.TYPE_MOBILE);	 
            } else if (Email.CONTENT_ITEM_TYPE.equals(getMimetype()) && getAsLong(Email.TYPE)==null) {
	        //Log.v(TAG, "empty email_type, default to: MOBILE");
                if( mAfter != null ) mAfter.put(Email.TYPE, Email.TYPE_MOBILE);	 
            }
            ...
            if (isInsert()) {
                ...
                // if it's phone && auto-189, construct toBeInsertEmail list
                if (add189 && Phone.CONTENT_ITEM_TYPE.equals(getMimetype())) {
                	String aPhoneNumber = normalizePhone(getAsString(Phone.NUMBER));
                ...
                // if it's phone && auto-189, construct toBeDeletedEmail list
                if (add189 && Phone.CONTENT_ITEM_TYPE.equals(getMimetype())) {
                	constructEmailListToBeDeleted(emailInsertList, emailDeletionList);
                }
            } else if (isUpdate()) {
                ...
                // if it's phone && auto-189, construct toBeDeletedEmail and toBeInsertEmail list
                if (add189 && Phone.CONTENT_ITEM_TYPE.equals(getMimetype())) {
                	constructEmailListToBeDeleted(emailInsertList, emailDeletionList);
                }
            } else {            	
                ...
                // if auto-189 && email, compare with toBeDeletedEmail & toBeInsertEmail list, if needed, delete this email
                if (add189 && Email.CONTENT_ITEM_TYPE.equals(getMimetype()) && emailDeletionList.size() > 0) {      
                    //Log.v(TAG, "No change email item, 189ToBeDel_listsize= "+emailDeletionList.size()+",  "+emailDeletionList+", 189ToBeInsert_listsize= "+emailInsertList.size()+", "+emailInsertList);            	            	           	
                    String email = getAsString(Email.ADDRESS);
                    //Log.v(TAG, "email id: "+getId()+" = "+email);
                    if (emailDeletionList.contains(email) && !emailInsertList.contains(email)) {
                        //Log.v(TAG, "email appears in 189ToBeDeleted list, but not in 189ToBeInsert list, need to be deleted !");  		           		
                        builder = ContentProviderOperation.newDelete(targetUri);
                        builder.withSelection(mIdColumn + "=" + getId(), null);            		
                    } else {
                    	//Log.v(TAG, "keep this email");
                    }
                } else {            	
            	    //Log.v(TAG, "child no change");
                }
            }
            return builder;
        }
