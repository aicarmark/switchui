...
public abstract class AccountTypeManager {
     ...
     * MOT CHINA
     * Returns the list of all accounts (if contactWritableOnly is false) or just the list of
     * contact writable accounts (if contactWritableOnly is true), including Card accounts.
     */
    public abstract List<AccountWithDataSet> getAccountsWithCard(boolean contactWritableOnly);
...
class AccountTypeManagerImpl extends AccountTypeManager
    ...
    protected void loadAccountsInBackground() {
        ...
        try {
            ...
            for (SyncAdapterType sync : syncs) {
                ...
                } else if (CardAccountType.ACCOUNT_TYPE.equals(type)) {
                    accountType = new CardAccountType(mContext, auth.packageName);           
                	Log.v(TAG, "CARD account type: "+accountType);                             
                } else if (LocalAccountType.ACCOUNT_TYPE.equals(type)) {
                    accountType = new LocalAccountType(mContext);           
                	Log.v(TAG, "LOCAL account type: "+accountType);                             
                } else {
                	Log.v(TAG, "external account type");
                ...
                // set other fields
                if (CardAccountType.ACCOUNT_TYPE.equals(type)) {
                    accountType.accountType = type;
                    if (PhoneModeManager.isDmds()) {
                        accountType.titleRes = R.string.account_card;
                    } else {
                        int phoneType =TelephonyManager.getDefault().getPhoneType();
                        if (phoneType == TelephonyManager.PHONE_TYPE_CDMA) {
                            accountType.titleRes = R.string.account_card_single_c;
                        } else {
                            accountType.titleRes = R.string.account_card;
                        }
                    }
                    accountType.iconRes = R.drawable.ic_launcher_card;                    
                 } else if (LocalAccountType.ACCOUNT_TYPE.equals(type)) {
                    accountType.accountType = type;
                    accountType.titleRes = R.string.account_local;
                    accountType.iconRes = R.drawable.ic_contact_picture_local;                    
                 } else {
                ...
                Log.v(TAG, "account addded: "+accountType);
            }
        ...
        for (Account account : accounts) {
            ...
            //if (syncable) {     support local/card types
    ...
    public List<AccountWithDataSet> getAccounts(boolean contactWritableOnly) {
        ...
        if (contactWritableOnly) {
            List<AccountWithDataSet> rtnAccounts = Lists.newArrayList();
            synchronized (this) {
                for (AccountWithDataSet account : mContactWritableAccounts) {
                    if (!HardCodedSources.ACCOUNT_TYPE_CARD.equals(account.type)) {
                        // By default, do not return Card accounts when get writable accounts
                        rtnAccounts.add(account);
                    }
                }
            }
            return rtnAccounts;
    ...
    /**
     * MOT CHINA
     * Return list of all known, contact writable (including card account) {@link AccountWithDataSet}'s.
     */
    @Override
    public List<AccountWithDataSet> getAccountsWithCard(boolean contactWritableOnly) {
        ensureAccountsLoaded();
        if (contactWritableOnly) {
            List<AccountWithDataSet> rtnAccounts = Lists.newArrayList();
            synchronized (this) {
                for (AccountWithDataSet account : mContactWritableAccounts) {
                    if (HardCodedSources.ACCOUNT_TYPE_CARD.equals(account.type)) {
                        int phoneType = SimUtility.getTypeByAccountName(account.name);
                        if (SimUtility.isSimReady(phoneType)) {
                            rtnAccounts.add(account);
                        }
                    } else {
                        rtnAccounts.add(account);
