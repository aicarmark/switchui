...
public class ContactSaveService extends IntentService {
    ...
    private void saveContact(Intent intent) {
        ...
        while (tries++ < PERSIST_TRIES) {
            try {
                ...
                if (isProfile) {
                    ...
                    // MOTO MOD: Request SIM sync
                    Cursor accountCursor = null;
                    String accountName = null;
                    String accountType = null;
                    String displayName = null;
                    try {
                        accountCursor = getContentResolver().query(RawContacts.CONTENT_URI,
                                new String[] {RawContacts.ACCOUNT_NAME, RawContacts.ACCOUNT_TYPE, Contacts.DISPLAY_NAME},
                        ...
                        if (accountCursor != null && accountCursor.moveToFirst()) {
                            ...
                            displayName = accountCursor.getString(2);
                        }
                    } finally {
                    ...
                    if (accountName != null && accountType != null && HardCodedSources.ACCOUNT_TYPE_CARD.equals(accountType)) {
                        Account simAccount = new Account(accountName, accountType);
                        Log.v(TAG, "Save/Update Sim contact, startSimSync, rawContactId = "+rawContactId);
                        SimUtility.requestSync(simAccount);
                    }
                    Cursor dataCursor = null;
                    String whereString = null;
                    try{
                        dataCursor = getContentResolver().query(Data.CONTENT_URI,
                        ...
                        if(dataCursor != null){
                            while(dataCursor.moveToNext()) {
                                ...
                                if(whereString != null){
                                    ...
                                    whereString = whereString + Calls.NUMBER + " = '" + number +"'";
                                }else{
                                    whereString = Calls.NUMBER + " = " + number;
                                }
                            }
                        }
                    }finally{
                    ...
                    if(whereString != null && displayName != null){
                        final ContentValues values = new ContentValues();
                        values.put(Calls.CACHED_NAME, displayName);
                        try{
                            getContentResolver().update(Calls.CONTENT_URI_WITH_VOICEMAIL, values,
                                                    whereString , null);
                        }catch (Exception e){
                        }
                    }
        ...
        // BEGIN Motorola, FTR 36344, IKHSS6-8444
        Uri contentUri = null;
        if (lookupUri != null && (contentUri = Contacts.lookupContact(resolver, lookupUri)) != null) {
            final long addContactId = ContentUris.parseId(contentUri);
            MEContacts.onContactOperation(getApplicationContext(), MEContacts.TAG_ID_CONTACT_ADD, addContactId);
        }
        // END IKHSS6-8444
    ...
    private void createGroup(Intent intent) {
        ...
        //begin add by dcjf34 porting IKCBSMMCPPRC-1100
        if(DEBUG)Log.d(TAG,"createGroup,label=" + label);
        ...
        Cursor cursor = resolver.query(Groups.CONTENT_URI, COLUMNS,
                Groups.ACCOUNT_TYPE + " NOT NULL AND "
                + Groups.ACCOUNT_NAME + " NOT NULL AND " + Groups.DELETED + "=0 AND "
                + Groups.TITLE + "=?", new String[] {label}, null);

        if(DEBUG)Log.d(TAG,"Groups.CONTENT_URI=" + Groups.CONTENT_URI + ",Groups.ACCOUNT_TYPE="
                    + Groups.ACCOUNT_TYPE + ",Groups.ACCOUNT_NAME="
                    + Groups.ACCOUNT_NAME + ",Groups.TITLE=" + Groups.TITLE
                );
        try {
            if (null == cursor ) {
                isGroupNameExist = false;
                if(DEBUG)Log.d(TAG,"isGroupNameExisted,null == cursor,isGroupNameExist = false");
            }

            if ( (null != cursor) && (cursor.getCount() > 0) ) {
                isGroupNameExist = true;
            } else {
                if(DEBUG)Log.d(TAG," (null != cursor) && (cursor.getCount() > 0) ,isGroupNameExist = false");
                isGroupNameExist = false;
            }
        } finally {
            if (null != cursor) {
        ...
        if(DEBUG)Log.d(TAG,"createGroup,isGroupNameExist=" + isGroupNameExist);
        if(isGroupNameExist == true){
            showToast(R.string.group_exist);
            return;
        }
        //end add by dcjf34 porting IKCBSMMCPPRC-1100
    ...
    private void deleteContact(Intent intent) {
        ...
        // MOTO MOD: Request SIM sync
        final long rawContactId = ContactsUtils.queryForNameRawContactId(getContentResolver(), contactUri);
        Cursor accountCursor = null;
        String accountName = null;
        String accountType = null;
        try {
        ...
        MEContacts.onContactOperation(getApplicationContext(), MEContacts.TAG_ID_CONTACT_DEL, contactId);
        // END IKHSS6-8444

        // MOTO MOD: Request SIM sync
        if (accountName != null && accountType != null) {
            if (HardCodedSources.ACCOUNT_TYPE_CARD.equals(accountType) || HardCodedSources.ACCOUNT_TYPE_LOCAL.equals(accountType)) {
                Account account = new Account(accountName, accountType);
                Log.v(TAG, "delete Sim/local contact, requestSync, rawContactId = "+rawContactId + ", accountName = "+accountName);
    ...
    private void joinContacts(Intent intent) {
        ...
        // BEGIN Motorola, FTR 36344, IKHSS6-8444
        MEContacts.onMergeContact(getApplicationContext(), contactId1, contactId2);
        // END IKHSS6-8444
    ...
    protected void deliverCallback(final Intent callbackIntent) {
