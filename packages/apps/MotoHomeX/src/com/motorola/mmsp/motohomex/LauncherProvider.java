...
/*Added by ncqp34 at Jul-24-2012 for flex plugin widget*/
...
public class LauncherProvider extends ContentProvider {
    ...
    static final String AUTHORITY = "com.motorola.mmsp.motohomex.settings";
    
    /*Added by ncqp34 at Jul-24-2012 for plugin widget flex*/
    private static final String WEATHER_PACKAGENAME = "com.motorola.mmsp.weather";
    /*ended by ncqp34*/
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            /* 2012-06-01, Added by e13775 for SWITCHUI-1317 */
            try{
                Intent intent = new Intent("android.intent.action.autoimportsim");
                mContext.sendBroadcast(intent);
            } catch (Exception e) {
                Log.e(TAG,"Send autoimportsim intent exception");
            }
            /* 2012-06-01, Added by e13775 for SWITCHUI-1317 end */
        ...
        private int loadFavorites(SQLiteDatabase db, int workspaceResourceId) {
            ...
            try {
                ...
                try {
                    if (Flex.isFlexValid(mContext)) {
                        /* Added by ncqp34 at Jun-29-2012 for flex id */
                        xmlstr = Flex.getResultFromCDA(
                                "@MOTOFLEX@getHomeApplications", mContext);
                        /* ended by ncqp34 */
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG ||
                    ...
                    if (LOGD) Log.d(TAG, "loadFavorites xml tag name:" + name);
                    String packageName="";
                    String className="";
                    String spanx="";
                    String spany="";
                    String iconResId = "";//a.getResourceId(R.styleable.Favorite_icon, 0);
                    String titleResIdFlex = "";//a.getResourceId(R.styleable.Favorite_title, 0);
                    ...
                    if (needCDA) {
                        for (int j = 0; j < parser.getAttributeCount(); j++) {
                            if (parser.getAttributeName(j).equals(
                                    "launcher:screen")) {
                                screen = parser.getAttributeValue(j);
                                values.put(LauncherSettings.Favorites.SCREEN,
                                        screen);
                            } else if (parser.getAttributeName(j).equals(
                                    "launcher:x"))
                                ...
                                if (LOGD)
                                    Log.d(TAG, "parser.icon(j).toString()="
                                            + parser.getAttributeValue(j)
                                                    .toString());
                            } else if (parser.getAttributeName(j).equals(
                                    "launcher:title")) {
                                titleResIdFlex = parser.getAttributeValue(j);
                                if (LOGD)
                                    Log.d(TAG, "parser.title(j).toString()="
                                            + parser.getAttributeValue(j)
                                                    .toString());
                            } else if (parser.getAttributeName(j).equals(
                                    "launcher:uri"))
                                ...
                                // parser.getAttributeValue(j))- 1;
                                // values.put(Favorites.PROFILE_NUM, profileId);
                            } else if (parser.getAttributeName(j).equals(
                                    "launcher:container")) {
                                if (LOGD)
                                    Log.d(TAG, "loadFavorites container:"
                                            + parser.getAttributeValue(j));
                                container = Integer.parseInt(parser
                                        .getAttributeValue(j));
                                values.put(Favorites.CONTAINER, container);
                            }
                        }
                    } else {// Reading default from default_workspace.xml
                            // long container =
            ...
            /*ended by ncqp34*/        
                    ...
                    if (TAG_FAVORITE.equals(name)) {
                        ...
                        added = id >= 0;*/
                        /*Added by ncqp34 at Jan-10-2012 for workspace and hotseat flex*/
                        if (a == null) {
                            /* Added by ncqp34 at Jun-04-2012 for folder flex */
                            long id = addAppShortcut(db, values, packageName,
                                    className, packageManager, intent);
                            added = id >= 0;
                            /* ended by ncqp34 */
                        } else {
                            /* ended by ncqp34 */
                        ...
                        /*Added by ncqp34 at Jan-10-2012 for workspace and hotseat flex*/
                        if (a == null)
                            added = addAppWidget(db, values, packageName,
                                    className, spanx, spany, packageManager);
                        /* ended by ncqp34 */
                        else {
                            String pkgName = a
                            ...
                            // packageManager);
                            added = addAppWidget(db, values, pkgName, clzName,
                                    spanx, spany, packageManager);
                        } 
                        ...
                        if (a == null) {
                            /* Added by ncqp34 at Jun-04-2012 for folder flex */
                            long id = addUriShortcut(db, values, iconResId,
                                    titleResIdFlex, uri);
                            added = id >= 0;
                            /* ended by ncqp34 */
                        } else {
                        ...
                        if (a == null) {
                            String title;
                            String foderName = titleResIdFlex.toString().trim();
                            if (foderName.length() > 15
                                    && "android:string/".equals(foderName
                                            .substring(0, 15))) {
                                int titleResId = mContext.getResources()
                                        .getIdentifier(titleResIdFlex, null,
                                                null);
                                if (titleResId != 0) {
                            ...
                            //added by amt_wangpeipei 2012/09/03 for SWITCHUI-2615 begin
							String shortcutIconResId = "";
							String shortcutTitleResIdFlex = "";
							String shortcutUri = "";
							//added by amt_wangpeipei 2012/09/03 for SWITCHUI-2615 end
                                final String folder_item_name = parser
                                        .getName();
                                for (int j = 0; j < parser.getAttributeCount(); j++) {
                                    Log.d(TAG,
                                            "folder flex--parser.getAttributeName(j) =="
                                                    + parser.getAttributeName(j));
                                    Log.d(TAG,
                                            "folder flex-- parser.getAttributeValue(j)=="
                                                    + parser.getAttributeValue(j));
                                    ...
                                    if (parser.getAttributeName(j).equals(
                                        ...
                                        packageName = parser
                                                .getAttributeValue(j);
                                    } else if (parser.getAttributeName(j)
                                            .equals("launcher:className")) {
                                        className = parser.getAttributeValue(j);
                                    }
                                    //added by amt_wangpeipei 2012/09/03 for SWITCHUI-2615 begin
					                else if (parser.getAttributeName(j)
									         .equals("launcher:icon")) {
						                shortcutIconResId = parser.getAttributeValue(j);
						                shortcutIconResId = String.valueOf(r.getIdentifier(shortcutIconResId, null, null));
									} else if (parser.getAttributeName(j)
									        .equals("launcher:title")) {
						                shortcutTitleResIdFlex = parser.getAttributeValue(j);
					                } else if (parser.getAttributeName(j)
									        .equals("launcher:uri"))
						                shortcutUri = parser.getAttributeValue(j);
					                    //added by amt_wangpeipei 2012/09/03 for SWITCHUI-2615 end
                                    }
                                values.clear();
                                values.put(
                                ...
                                if (TAG_FAVORITE.equals(folder_item_name)
                                    ...
                                    long id = addAppShortcut(db, values,
                                            ...
                                            packageManager, intent);
                                    Log.d(TAG, "folder flex--id ==" + id);
                                    if (id >= 0) {
                                        folderItems.add(id);
                                    }
                                } else if (TAG_SHORTCUT
                                        .equals(folder_item_name)
                                        && folderId >= 0) {
                            	//modified by amt_wangpeipei 2012/09/03 for SWITCHUI-2615 begin
                                    //long id = addUriShortcut(db, values,
                                    //        iconResId, titleResIdFlex, uri);
								    long id = addUriShortcut(db, values,
										shortcutIconResId, shortcutTitleResIdFlex, shortcutUri);
								//modified by amt_wangpeipei 2012/09/03 for SWITCHUI-2615 end
                            ...
                            // We can only have folders with >= 2 items, so we
                            // need to remove the
                            // folder and clean up if less than 2 items were
                            // included, or some
                            // failed to add, and less than 2 were actually
                            // added

                            if (folderItems.size() < 2 && folderId >= 0) {
                                // We just delete the folder and any items that
                                // made it
                                Log.d(TAG, "folder flex--folderItems.size() =="
                                        + folderItems.size());
					...
					if (added)
						i++;
					/*
					 * Added by ncqp34 at Jan-10-2012 for workspace and hotseat
					 * flex
					 */
					if (a != null)
						/* ended by ncqp34 */
	...
	/*Added by ncqp34 at Jan-10-2012 for workspace and hotseat flex*/
	/*Added by ncqp34 at Jun-04-2012 for folder flex*/
	private long addAppShortcut(SQLiteDatabase db, ContentValues values, String packageName,
				 String className, PackageManager packageManager,Intent intent) {
	/*ended by ncqp34 */
			Log.i("Other", "addAppShortcut.....packageName.." + packageName + "...className..." + className);
			if (LOGD) Log.d(TAG, "addAppShortcut adding:" + packageName + "/" + className);
			ActivityInfo info;
			/*Added by ncqp34 at Jun-04-2012 for folder flex*/
       			long id = -1;
			/*ended by ncqp34 */
	...
	private boolean addAppWidget(SQLiteDatabase db, ContentValues values, String packageName,String className,
					 String spanx, String spany , PackageManager packageManager) {

			Log.d(TAG, "addAppWidget.....className.." + className + "...packageName..." 
				+ packageName + "...spanx..." + spanx + "...spany.."+ spany);

			if (packageName == null || className == null) {
				Log.d(TAG, "addAppWidget0");
				return false;
			}

			//ComponentName cn = new ComponentName(packageName.toString(), className.toString());
			...
			if(hasPackage == false) return false;
			/*Added for flex plugin widget*/
			boolean isPlugin = packageName.equals(WEATHER_PACKAGENAME); 
			if(!hasPackage && !isPlugin) return false;
			
			boolean allocatedAppWidgets = false;
			final AppWidgetManager appWidgetManager = AppWidgetManager.getInstance(mContext);
			Log.d("Other", "addAppWidget....mAppWidgetHost...." + mAppWidgetHost);
			
			try {
				if(isPlugin){
					values.put(Favorites.ITEM_TYPE, Favorites.ITEM_TYPE_PLUGINWIDGET);
					values.put(Favorites.INTENT , className);
					PluginWidgetHostId pluginHost = new PluginWidgetHostId(mContext);
	       			int pluginId = pluginHost.allocatePluginWidgetId(className);
					values.put(Favorites.APPWIDGET_ID, pluginId);
				    } else {
						int appWidgetId = mAppWidgetHost.allocateAppWidgetId();
					...
					values.put(Favorites.SPANX, spanx);
					values.put(Favorites.SPANY, spany);
					/*2012-05-09, added by Chen Yidong, for switchui-1037*/
					values.put(Favorites._ID, generateNewId());
	...
	/*Added by ncqp34 at Jun-04-2012 for folder flex*/
	private long addUriShortcut(SQLiteDatabase db, ContentValues values, String iconResId,
						 String titleResId, String uri) {
	/*ended by ncqp34 */
			Log.d("Other", "addUriShortcut.....iconResId.." + iconResId + "...titleResId..." 
				+ titleResId + "...uri..." + uri);
			...
			if (iconResId.equals("") || titleResId.equals("")) {
				Log.w(TAG, "Shortcut is missing title or icon resource ID");
			/*Added by ncqp34 at Jun-04-2012 for folder flex*/
				return -1;	
			}
			...
			intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
			values.put(Favorites.INTENT, intent.toUri(0));
			values.put(Favorites.TITLE, titleResId);
			values.put(Favorites.ITEM_TYPE, Favorites.ITEM_TYPE_SHORTCUT);
			values.put(Favorites.SPANX, 1);
			values.put(Favorites.SPANY, 1);
			values.put(Favorites.ICON_TYPE, Favorites.ICON_TYPE_RESOURCE);
			values.put(Favorites.ICON_PACKAGE, mContext.getPackageName());
			values.put(Favorites.ICON_RESOURCE, iconResId);
