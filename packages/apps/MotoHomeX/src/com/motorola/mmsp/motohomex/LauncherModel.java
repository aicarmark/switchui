...
import com.motorola.mmsp.motohomex.InstallWidgetReceiver.WidgetMimeTypeHandlerData;
...
public class LauncherModel extends BroadcastReceiver
       /*2012-8-3, add by bvq783 for plugin*/
       implements PluginHost.PluginHostCallback {
    ...
    /*2012-8-3, add by bvq783 for plugin*/
    public PluginWidgetHost mPluginWidgetHost = null;
    static final ArrayList<LauncherPluginWidgetInfo> sBgPluginWidgets = new ArrayList<LauncherPluginWidgetInfo>();
    /*2012-8-3, add end*/
    // e13775 07/11/2012: Uninstalled shortcut remain in workspace start
    int mLastWorkspaceSeq = 0;
    int mWorkspaceSeq = 1;
    // e13775 07/11/2012: Uninstalled shortcut remain in workspace end
    ...
    public interface Callbacks {
        ...
        public void bindPluginRemoved(String[] widgets, boolean permanent);
   ...
   /** Added by amt_wangpeipei 2012/08/24 for switchui-2569
    * Attach a task to be executed in a worker thread after
    * specified delay.
    *
    * @param task
    *
    ...
    *
    */
	public static void attachTaskDelayed(Runnable task, long delayMillis) {
    ...
    private void unbindWorkspaceItemsOnMainThread() {
        ...
        final ArrayList<ItemInfo> tmpPluginWidgets = new ArrayList<ItemInfo>();
        /*2012-10-30, add end*/
        synchronized (sBgLock) {
            tmpWorkspaceItems.addAll(sBgWorkspaceItems);
            tmpAppWidgets.addAll(sBgAppWidgets);
            /*2012-10-30, add by 003033 for switchui-2990*/
            tmpPluginWidgets.addAll(sBgPluginWidgets);
            /*2012-10-30, add end*/
        }
        Runnable r = new Runnable() {
                ...
                public void run() {
                   ...
                   /*2012-10-30, add by 003033 for switchui-2990*/
                   for (ItemInfo item : tmpPluginWidgets) {
                       item.unbind();
                   }
                   /*2012-10-30, add end*/
                }
        ...
        Runnable r = new Runnable() {
            public void run() {
                ...
                synchronized (sBgLock) {
                    ...
                    //chen yidong change, 2012-08-20
                    if(modelItem == null) {
                        //Somehow we reached state where we are trying to update item which does not exist in map
                        //just silently observe it instead of crashing
                        Log.e(TAG, "Not updating item as it is not in the list");
                        return;
                    }
                    if (item != modelItem
                    ...
                    //chen yidong change, 2012-08-20 end.
    ...
    static void moveItemInDatabase(Context context, final ItemInfo item, final long container,
        ...
        // Modified by e13775 at July17 2012 for SWITCHUI-2252 start
        if (null == item)
            return;
        // Modified by e13775 at July17 2012 for SWITCHUI-2252 end
                ...
                synchronized (sBgLock) {
                    ...
                    switch (item.itemType) {
                        ...
                        case LauncherSettings.Favorites.ITEM_TYPE_PLUGINWIDGET:
                            sBgPluginWidgets.add((LauncherPluginWidgetInfo)item);
                            break;
                        /*2012-8-3, add end*/
                     }
                }
            ...
            public void run() {
                ...
                synchronized (sBgLock) {
                    switch (item.itemType) {
                        ...
                        /*2012-8-3, add by bvq783 for plugin*/
                        case LauncherSettings.Favorites.ITEM_TYPE_PLUGINWIDGET:
                            sBgPluginWidgets.remove((LauncherPluginWidgetInfo)item);
                            break;
                        /*2012-8-3, add end*/
                    }
    ...
    public void initialize(Callbacks callbacks) {
        synchronized (mLock) {
           ...
           // if(mPluginWidgetHost==null){
            mPluginWidgetHost = (PluginWidgetHost)PluginFactory.makeHost(
                    PluginHost.PLUGIN_HOST_APP_CATEGORY_HOME,
                    PluginHost.PLUGIN_CATEGORY_WIDGET,
                    (PluginHost.PluginHostCallback)this);
            if (mPluginWidgetHost != null)
                mPluginWidgetHost.scanPlugins(false); //false - synchronism, true - asynchronism
          //  }
            /*2012-8-3, add end*/
        }
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
            ...
            /*2012-8-3, add by bvq783 for plugin*/
            final ArrayList<String> removes = new ArrayList<String>();
            if (mPluginWidgetHost != null) {
            PackageManager pm = context.getPackageManager();
            Intent intentemp = new Intent(
                                PluginHost.PLUGIN_HOST_APP_CATEGORY_HOME);
            intentemp.addCategory(PluginHost.PLUGIN_CATEGORY_WIDGET);
            List<ResolveInfo> list = pm.queryBroadcastReceivers(intentemp,
                                        PackageManager.GET_META_DATA);
            final HashMap<String, String> current = new HashMap<String, String>();
            final HashMap<String, ComponentInfo> cList= new HashMap<String, ComponentInfo>();
            for (int i = 0; list != null && i < list.size(); i++) {
                ResolveInfo ri = list.get(i);
                final ComponentInfo ci = ri.activityInfo != null ? ri.activityInfo
                                           : ri.serviceInfo;
                cList.put(ci.name, ci);
                current.put(ci.packageName, ci.name);
            }
            HashMap<String, PluginWidget> pluginInstalled = mPluginWidgetHost.getPluginWidgets();

            switch (op) {
            case PackageUpdatedTask.OP_ADD:
                if (current.containsKey(packageName)) {
                    String name = current.get(packageName);
                    if (!pluginInstalled.containsKey(name)) {
                        ComponentInfo ci = cList.get(name);
                        mPluginWidgetHost.addPluginWidget(ci);
                    }
                }
                break;
            case PackageUpdatedTask.OP_UPDATE:
                if (current.containsKey(packageName)) {
                    String name = current.get(packageName);
                    if (pluginInstalled.containsKey(name)) {
                        ComponentInfo ci = cList.get(name);
                        mPluginWidgetHost.updatePluginWidget(ci);
                        op = PackageUpdatedTask.OP_UPDATE_PLUGIN;
                    } else {
                        //may it's need to add
                        ComponentInfo ci = cList.get(name);
                        mPluginWidgetHost.addPluginWidget(ci);
                    }
                } else {
                    //may it's need to remove
                    removes.clear();
                    Set set = pluginInstalled.entrySet();
                    Iterator itr = set.iterator();
                    while(itr.hasNext()){
                        Map.Entry map = (Map.Entry)itr.next();
                        String key = (String) map.getKey();
                        if (!current.containsValue(key)) {
                            removes.add(key);
                            continue;
                        }
                    }
                    if (removes.size() > 0) {
                        op = PackageUpdatedTask.OP_REMOVE_PLUGIN;
                    }
                }
                break;
            case PackageUpdatedTask.OP_REMOVE:
                removes.clear();
                Set set = pluginInstalled.entrySet();
                Iterator itr = set.iterator();
                while(itr.hasNext()){
            ...
            /*2012-8-3, add by bvq783 end*/
            if (op != PackageUpdatedTask.OP_NONE && op != PackageUpdatedTask.OP_REMOVE_PLUGIN) {
                enqueuePackageUpdated(new PackageUpdatedTask(op, new String[] { packageName }));
            } else if (op == PackageUpdatedTask.OP_REMOVE_PLUGIN) {
                enqueuePackageUpdated(new PackageUpdatedTask(op, removes));
            }
        	...
        	/*2012-7-12, add by bvq783 for switchui-2054*/
            mApp.getAppsModel().localeChanged();
            /*2012-7-12, add end*/
            forceReload();
    ...
    private class LoaderTask implements Runnable {
                ...
                try {
                    ...
                    while (!mStopped && c.moveToNext()) {
                        try {
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                                ...
                                if (itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
                                    ...
                                    info = getShortcutInfo(c, context, iconTypeIndex,
                                            ...
                                            titleIndex);
                                /*2012-10-11, Modifyed by amt_chenjing for all shortcut title*/
                                /*2012-05-16, Chen Yidong for SWITCHUI-1157*/
                                try{
                                    final ResolveInfo ri = manager.resolveActivity(intent, 0);
                                    /*2012-05-28, Chen Yidong for SWITCHUI-1265*/
                                     /*2012-10-30, modify by 003033 for siwtchui-3006*/
									/*CharSequence title = ri.activityInfo.loadLabel(manager);
                                     if(title != null && !title.toString().contains("Android")){
                                     info.title = title;   
                                    }*/
                                    ...
                                    /*2012-10-30, modify end*/             
                                    /*2012-05-28, end*/
                                } catch (Exception eri){
                                    Log.e(TAG, "Load shortcut label error");
                                }
                                /*2012-05-16, end*/
                                /*2012-10-11, Modify end*/
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_PLUGINWIDGET:
                                // Read all Launcher-specific widget details
                                int widgetId = c.getInt(appWidgetIdIndex);
                                id = c.getLong(idIndex);
                                LauncherPluginWidgetInfo pluginInfo = new LauncherPluginWidgetInfo(widgetId);

                                container = c.getInt(containerIndex);
                                if (container != LauncherSettings.Favorites.CONTAINER_DESKTOP &&
                                      container != LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
                                    Log.e(TAG, "Plugin Widget found where container "
                                        + "!= CONTAINER_DESKTOP nor CONTAINER_HOTSEAT - ignoring!");
                                    continue;
                                }
                                ...
                                pluginInfo.container = c.getInt(containerIndex);

                                // check & update map of what's occupied
                                if (!checkItemPlacement(occupied, pluginInfo)) {
                                    break;
                                }
                                sBgItemsIdMap.put(pluginInfo.id, pluginInfo);
                                sBgPluginWidgets.add(pluginInfo);                      
                                break;
                            /*2012-8-3, add end*/
                            }
                        } catch (Exception e) {
        ...
        /*2012-11-5, add by amt_chenjun for plugin*/
        /** Filters the set of pluginwidgets which are on the specified screen. */
        private void filterCurrentPluginWidgets(int currentScreen,
                ArrayList<LauncherPluginWidgetInfo> pluginWidgets,
                ArrayList<LauncherPluginWidgetInfo> currentScreenWidgets,
                ArrayList<LauncherPluginWidgetInfo> otherScreenWidgets) {
            // If we aren't filtering on a screen, then the set of items to load is the full set of
            // widgets given.
            if (currentScreen < 0) {
                currentScreenWidgets.addAll(pluginWidgets);
            }

            for (LauncherPluginWidgetInfo widget : pluginWidgets) {
                if (widget == null) continue;
                if (widget.container == LauncherSettings.Favorites.CONTAINER_DESKTOP &&
                        widget.screen == currentScreen) {
                    currentScreenWidgets.add(widget);
	...
	/*2012-11-5, add end*/
        ...
        private void bindWorkspaceItems(final Callbacks oldCallbacks,
				...
				/*2012-11-5, add by amt_chenjun for plugin*/
				final ArrayList<LauncherPluginWidgetInfo> pluginWidgets,
				/*2012-11-5, add end*/
            ...
            /*2012-11-5, add by amt_chenjun for plugin*/
            N = pluginWidgets.size();
            for (int i = 0; i < N; i++) {
                final LauncherPluginWidgetInfo widget = pluginWidgets.get(i);
                final Runnable r = new Runnable() {
                    public void run() {
                        Callbacks callbacks = tryGetCallbacks(oldCallbacks);
                        if (callbacks != null) {
                            callbacks.bindPluginWidget(widget);
                        }
                    }
                };
                if (postOnMainThread) {
        ...
        private void bindWorkspace(int synchronizeBindPage) {
			...
			/*2012-11-5, add by amt_chenjun for plugin*/
			ArrayList<LauncherPluginWidgetInfo> plugins = 
			        new ArrayList<LauncherPluginWidgetInfo>();
			/*2012-11-5, add end*/
            synchronized (sBgLock) {
				...
				/*2012-11-5, add by amt_chenjun for plugin*/
				plugins.addAll(sBgPluginWidgets);
				/*2012-11-5, add end*/
            }
			...
			/*2012-11-5, add by amt_chenjun for plugin*/
            ArrayList<LauncherPluginWidgetInfo> currentPluginWidgets =
                    new ArrayList<LauncherPluginWidgetInfo>();
            ArrayList<LauncherPluginWidgetInfo> otherPluginWidgets =
                    new ArrayList<LauncherPluginWidgetInfo>();
			/*2012-11-5, add end*/
            ...
            filterCurrentFolders(currentScreen, itemsIdMap, folders, currentFolders,
                    otherFolders);
			/*2012-11-5, add by amt_chenjun for plugin*/
            filterCurrentPluginWidgets(currentScreen, plugins, currentPluginWidgets,
                    otherPluginWidgets);
			/*2012-11-5, add end*/
			...
			/*2012-11-5, add currentPluginWidgets by amt_chenjun for plugin*/
            bindWorkspaceItems(oldCallbacks, currentWorkspaceItems, currentAppWidgets,
                    currentFolders, currentPluginWidgets, null);
			/*2012-11-5, add end*/
            ...
            bindWorkspaceItems(oldCallbacks, otherWorkspaceItems, otherAppWidgets, otherFolders,
                    otherPluginWidgets, (isLoadingSynchronously ? mDeferredBindRunnables : null));
			/*2012-11-5, add end*/
        ...
        private void loadAllAppsByBatch() {
			...
			mApp.getFakeModel().getFakeAppList(mApp);
			/*ended by ncqp34*/
			/*ended by ncqp34*/
    ...
    private class PackageUpdatedTask implements Runnable {
        ...
        /*2012-8-3, added by bvq783 for plugin widget*/
        public static final int OP_UPDATE_PLUGIN = 5;
        public static final int OP_REMOVE_PLUGIN = 6;
        /*2012-8-3, add by bvq783 end*/
        ...
        /* 2012-8-3, added by bvq783 for plugin widget*/
        public PackageUpdatedTask(int op, ArrayList<String> packages) {
            mOp = op;
            if (packages.size() > 0)
                mPackages = new String[packages.size()];
            for (int i=0; packages != null && i<packages.size(); i++) {
                mPackages[i] = packages.get(i);
            }
        }
        /* 2012-8-3, add by bvq783 end*/
        ...
        public void run() {
            ...
            /*2012-8-3, added by bvq783 for plugin widget*/
            if (bRemovePlugin) {
            	  final String[] widgets = mPackages;
            	  mHandler.post(new Runnable() {
                    public void run() {
                        Callbacks cb = mCallbacks != null ? mCallbacks.get() : null;
                        if (callbacks == cb && cb != null) {
                            callbacks.bindPluginRemoved(widgets, true);
                        }
                    }
                });
            }
    ...
    public static class WidgetAndShortcutNameComparator implements Comparator<Object> {
        ...
        public final int compare(Object a, Object b) {
            ...
            if (mLabelCache.containsKey(a)) {
                ...
                /*2012-8-3, add by bvq783 for plugin*/
                if (!(a instanceof PluginWidgetProviderInfo)) {
		      if (a instanceof AppWidgetProviderInfo){
			   labelA = ((AppWidgetProviderInfo) a).label;
		      } else{
		// 2012-09-05 Modifyed by amt_chenjing for SWITCHUI-2696	   
				if (a != null) {
					CharSequence cs = ((ResolveInfo) a).loadLabel(mPackageManager);
					if (cs != null) {
						labelA = cs.toString();
					} else {
						Log.d(TAG, "unable to get label of obeject a ="+ a);
						labelA = "";
						}
					} else {
						labelA = "";
		...
		//2012-09-05 Modify end
                    ...
                    labelA = ((PluginWidgetProviderInfo)a).label;
                }
                /*2012-8-3, add end*/
            ...
            if (mLabelCache.containsKey(b)) {
                ...
                /*2012-8-3, add by bvq783 for plugin*/
                if (!(b instanceof PluginWidgetProviderInfo)) {
		    if(b instanceof AppWidgetProviderInfo){
			   labelB = ((AppWidgetProviderInfo) b).label;
		      } else{
			   CharSequence cs = ((ResolveInfo) b).loadLabel(mPackageManager);
			   if (cs !=null){
			       labelB = cs.toString();
			   }else{
			       Log.d(TAG,"unable to get label of obeject b =" + b);
			       labelB="";
			   }	
		      }
                } else {
                    labelB = ((PluginWidgetProviderInfo)b).label;
                }
                /*2012-8-3, add end*/
	...
	// Added by e13775 at 11 July 2012 for organize apps' group start
    static void updateMultItemFromDatabase(Context context, final Intent data) {
        ...
        Runnable r = new Runnable() {
            public void run() {
                ...
                try {
                    Intent intent = data.getParcelableExtra(Intent.EXTRA_SHORTCUT_INTENT);
                    // Modified by e13775 at July23 2012 for SWITCHUI-2252 start
                    String title = data.getStringExtra(Intent.EXTRA_SHORTCUT_NAME);
                    Parcelable extra = data.getParcelableExtra(Intent.EXTRA_SHORTCUT_ICON_RESOURCE);
                    // Search in sItemsIdMap instead of sWorkspaceItems. sItemsIdMap has
                    // the folder items and sWorkspaceItems does not.
                    Iterator<Map.Entry<Long, ItemInfo>> iterator = sBgItemsIdMap.entrySet().iterator();
                    while(iterator.hasNext()){
                        ItemInfo item = iterator.next().getValue();
                        if (item instanceof ShortcutInfo &&
                                ((ShortcutInfo)item).intent.toString().equals(intent.toString())){
                            ...
                            Uri uriToUpdate = LauncherSettings.Favorites.getContentUri(item.id, false);
                            // Modified by e13775 at July23 2012 for SWITCHUI-2252 start
                            cr.update(uriToUpdate, values, null, null);
                                }
                    }
                } catch (Exception ex){
                    Log.e(TAG, "unable to update the shortcut");
                }
	...
	// Added by e13775 at 11 July 2012 for organize apps' group end

    /*2012-8-3, add by bvq783 for plugin framework*/
    ...
    public Context getViewContext() {
        Callbacks callback = mCallbacks!=null ? mCallbacks.get() : null;
        if (callback != null && callback instanceof Launcher) {
            return (Launcher)callback;
        } else
            return null;
    }

    public PluginWidgetHost getPluginHost() {
        return mPluginWidgetHost;
    }
    ...
    public boolean isCurrentPanel(int widgetId) {
        ...
        Callbacks callback = mCallbacks!=null ? mCallbacks.get() : null;
        if (callback != null && callback instanceof Launcher) {
            result = ((Launcher)callback).checkCurrentPage(widgetId);
        }
        return result;
    }
    /*2012-8-3, add end*/
    ...
    public static void deleteItemsByScreenID(final Context context, int scrID) {

        String path = context.getDatabasePath(LauncherProvider.DATABASE_NAME).getAbsolutePath();
        SQLiteDatabase db = null;

        try {
             db = SQLiteDatabase.openDatabase(path, null, 0);
             //2012-02-28 updated by rjg678 for IKSWITCHUI-67
             db.execSQL("DELETE FROM favorites  WHERE screen=" + scrID + " AND container =" + "-100");
        /*2012-7-12, add by bvq783 for switchui-2134*/
        } catch (Exception e) {
            Log.e(TAG, "deleteItemsByScreenID occupy exception:"+e);
        /*2012-7-12, add end*/
        } finally {
            if (db != null) {
                db.close();
    ...
    public static void updateScreenID(final Context context, int fromId, int toId) {
        ...
        try {
            db = SQLiteDatabase.openDatabase(path, null, 0);
            //2012-02-28 updated by rjg678 for IKSWITCHUI-67
            db.execSQL("UPDATE  favorites SET SCREEN= " + "-1" + 
                    " WHERE screen=" + fromId + " AND container =" + "-100");

            if (fromId > toId) {
                for (int i = fromId - 1; i > toId - 1; i--) {
                    int k = i + 1;
                    //2012-02-28 updated by rjg678 for IKSWITCHUI-67
                    db.execSQL("UPDATE  favorites SET SCREEN= " + k + 
                            " WHERE screen=" + i + " AND container =" + "-100");
                }
            ...
            //2012-02-28 updated by rjg678 for IKSWITCHUI-67
            db.execSQL("UPDATE  favorites SET SCREEN= " + toId + 
                    " WHERE screen=" + "-1" + " AND container =" + "-100");
        /*2012-7-12, add by bvq783 for switchui-2134*/
        } catch (Exception e) {
            Log.e(TAG, "update screen id occupy exception:"+e);
        /*2012-7-12, add end*/
        } finally {
            if (db != null) {
                db.close();
