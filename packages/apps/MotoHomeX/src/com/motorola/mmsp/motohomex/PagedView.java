...
/*2012-7-18, add by bvq783 for switchui-2310*/
...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    // Scrolling indicator
    /*2012-7-18, modify by bvq783 for switchui-2310*/
    private /*ValueAnimator*/ AnimatorSet mScrollIndicatorAnimator;
    ...
    /*Added by ncqp34 at May-25-2012 for switchui-1249*/
    private ImageView mDockDivider;
    private boolean bNeedHideScrollIndic = false;
    /*ended by ncqp34*/
    //hnd734 Home Transition start
    public PagedTransition pt;
    //hnd734 Home Transition end
    /*2012-05-15, added by Bai Jian for SWITCHUI-1049*/
    protected boolean mWithEffect = true;
    /*2012-05-15, added end*/
...
// Modified by e13775 at 19 June 2012 for organize apps' group
	...
	//RJG678 Pinch Panel
	// Modified by e13775 at 19 June 2012 for organize apps' group
	protected void updateCurrentPage(int currentPage) {
    ...
    protected void onPageBeginMoving() {
        ...
        if ( this instanceof Workspace) {
            Workspace w = ((Workspace) this);
            /*add by bvq783 to avoid the animation conflict*/
            /*add by bvq783 end*/
            if (!w.isSmall() && !w.isSwitchingState()) {
                w.getLauncher().stopUnlockAnimation();
                pt.pageBeginMoving(this);
            }
        } else {
            pt.pageBeginMoving(this);
        }
    ...
    protected void onPageEndMoving() {
        ...
        /*2012-05-15, added end*/
        //hnd734 Home Transition start
        /*2012-05-24, Chen Yidong, for SWITCHUI-1245*/
        if ( this instanceof Workspace) {
            Workspace w = ((Workspace) this);
            if (!w.isSmall() && !w.isSwitchingState()) {
                pt.pageEndMoving(this);
            }
        } else {
            pt.pageEndMoving(this);
        }
    ...
    protected void screenScrolled(int screenCenter) {
        //Log.d("July", "screenScrolled, scroll enable:"+isScrollingIndicatorEnabled());
    ...
    /*2012-06-14, Chen Yidong for SWITCHUI-1502*/
    private PaintFlagsDrawFilter mAntiAliasDrawFilter = new 
    ...
    protected void dispatchDraw(Canvas canvas) {
        /*2012-06-14, Chen Yidong for SWITCHUI-1502*/
    	 /*2012-06-29, Hu ShuAn for SWITCHUI-1610*/
        if(canvas != null && mIsPageMoving){
        	/*2012-06-29, Hu Shuan end*/
        ...
        if (screenCenter != mLastScreenCenter || mForceScreenScrolled) {
			...
			//hnd734 Home Transition start
            if (mIsPageMoving) screenScrolled(screenCenter);
			//hnd734 Home Transition end
    ...
    protected boolean hitsPreviousPage(float x, float y) {
    	//added by amt_wangpeipei 2013/01/22 for SWITCHUITWO-571 begin
    	if(mCurrentPage < 0 || mCurrentPage > getChildCount()-1)
    	{
    		mCurrentPage = Math.max(0, Math.min(mCurrentPage, getChildCount()-1));
    	}
    	//added by amt_wangpeipei 2013/01/22 for SWITCHUITWO-571 end
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_MOVE:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                /*2012-07-03, added by ChenYidong for SWITCHUI-1730*/
                if(!mWithEffect){
                    mWithEffect = true;
                }
                /*2012-07-03, added end*/
    ...
    protected View getScrollingIndicator() {
        ...
        // found
        // janet.sun 05.30.2012: don't get it if only 1 child
        if (getChildCount() <= 1) {
            return null; 
        }
    ...
    Runnable hideScrollingIndicatorRunnable = new Runnable() {
        @Override
        public void run() {
            Log.d("July", "Pagedview hidescrolling indicator runnable");
            hideScrollingIndicator(false);
        }
    };
    protected void flashScrollingIndicator(boolean animated) {
        //janet.sun 03.12.2011: add check for child count. Or else the scroll
        // indicator will stay visible after deleting the next to last panel
        if (getChildCount() <= 1) {
            return;
        }
        Log.d("July", "flash scrolling indicator, animated:"+animated);
        removeCallbacks(hideScrollingIndicatorRunnable);
        showScrollingIndicator(!animated);
        postDelayed(hideScrollingIndicatorRunnable, sScrollIndicatorFlashDuration);
    }

    /*Added by ncqp34 at May-25-2012 for switchui-1249 dock divider*/
    protected void getDockDivider() {
        ViewGroup parent = (ViewGroup) getParent();
        mDockDivider = (ImageView) (parent.findViewById(R.id.dock_divider));
    }

    protected void hideDockDivider() {
        if (getChildCount() <= 1) return;
 	getDockDivider();
	if ( mDockDivider != null ) {
            //2012-02-08 Added by RJG678
            //In Port state, there is only DockDivider, 
            //and it should be hided in DragPanel status.
            mDockDivider.setVisibility(View.INVISIBLE);
        }
    }

    protected void showDockDivider() {
        if (getChildCount() <= 1) return;
 	getDockDivider();
	if ( mDockDivider != null ){
            //2012-02-08 Added by RJG678
	...
	/*ended by ncqp34*/
        ...
        if (mScrollIndicator != null) {
            ...
            if (immediately) {
                ...
                mScrollIndicatorAnimator = new AnimatorSet();
                if (mDockDivider != null) {
                    mScrollIndicatorAnimator.playTogether(ObjectAnimator.ofFloat(mScrollIndicator, "alpha", 1f),
                        ObjectAnimator.ofFloat(mDockDivider, "alpha", 1f));
                } else {
                     mScrollIndicatorAnimator.play(ObjectAnimator.ofFloat(mScrollIndicator, "alpha", 1f));
                }
                /*2012-7-18, modify by bvq783 end*/
    ...
    protected void cancelScrollingIndicatorAnimations() {
        if (mScrollIndicatorAnimator != null) {
            /*2012-11-7, add by 003033 for hide scrolling indicator 
            when long press one item quickly after showing scrolling indicator
            false -- not be cancelled in the wrong status
            true -- canceled in the wrong status, need to hide indicator
            */
            if (this instanceof Workspace) {
                bNeedHideScrollIndic = ((Workspace)this).isSwitchingState();
            } else {
                bNeedHideScrollIndic = false;
            }
            Log.d("July", "cancelScrollingIndicatorAnimations, this:"+this+", need hide:"+bNeedHideScrollIndic);
	...
	/*ended by ncqp34*/
        if (mScrollIndicator != null) {
            ...
            if (immediately) {
                ...
                mScrollIndicatorAnimator = new AnimatorSet();
                if (mDockDivider != null) {
                    mScrollIndicatorAnimator.playTogether(ObjectAnimator.ofFloat(mScrollIndicator, "alpha", 0f),
                        ObjectAnimator.ofFloat(mDockDivider, "alpha", 0f));
                } else {
                    mScrollIndicatorAnimator.play(ObjectAnimator.ofFloat(mScrollIndicator, "alpha", 0f));
                }
                /*2012-7-18, modify by bvq783 end*/
                ...
                mScrollIndicatorAnimator.addListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        if (!cancelled || (cancelled && bNeedHideScrollIndic)) {
                            Log.d("July", "hide indicator when end");
                            mScrollIndicator.setVisibility(View.INVISIBLE);
                            /*2012-7-18, add by bvq783 for switchui-2310*/
                            if (mDockDivider != null) {
                                mDockDivider.setVisibility(View.INVISIBLE);
    ...
    private void updateScrollingIndicatorPosition() {
        ...
        // wag044 04/20/2012: IKHSS7-25721 Avoid division by zero
    ...
    //handle it the same as action_cancel in ontouchevent
    public void resetPageWhenResume(){
        snapToDestination();
        mTouchState = TOUCH_STATE_REST;
        mActivePointerId = INVALID_POINTER;
        releaseVelocityTracker();
