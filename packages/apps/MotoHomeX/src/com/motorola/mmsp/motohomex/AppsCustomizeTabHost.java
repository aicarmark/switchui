...
//Add by e13775 at 19 June 2012 for organize apps' group start
...
//Add by e13775 at 19 June 2012 for organize apps' group end
...
public class AppsCustomizeTabHost extends TabHost implements LauncherTransitionable,
    ...
    //added by amt_wangpeipei 2012/05/09 for apps_customize_searchbar layout begin.
    private AppsCustomizeSearchBar mAppsCustomizeSearchBar;
...
//Add by e13775 at 19 June 2012 for organize apps' group
    AllAppsPage mAllAppsPage;
...
// Modified by e13775 at 19 June 2012 for organize apps' group
    ...
    protected void onFinishInflate() {
        ...
        //added by amt_wangpeipei 2012/05/09 begin
        //get apps_customize_searchbar layout and pass this tabhost object to it.
        mAppsCustomizeSearchBar = (AppsCustomizeSearchBar)findViewById(R.id.apps_customize_searchbar);
        mAppsCustomizeSearchBar.setAppsCustomizeTabHost(this);
        //added by amt_wangpeipei 2012/05/09 end.
        ...
        View tabViewAllApps = mLayoutInflater.inflate(R.layout.tab_widget_indicator_all_apps, tabs, false);
        tabViewAllApps.setContentDescription(label);
	/*Added by ncqp34 at Jul-17-2012 for group switch*/
	if(LauncherApplication.mGroupEnable){
            addTab(newTabSpec(APPS_TAB_TAG).setIndicator(tabViewAllApps).setContent(contentFactory));
	}else{
        label = getContext().getString(R.string.all_apps_button_label);
        TextView tabView = (TextView) mLayoutInflater.inflate(R.layout.tab_widget_indicator, tabs, false);
	...
	/*ended by ncqp34*/
        ...
        // AllAppsPage listener the tab apps to open the group picker and the long
        // click to edit the group
        final Spinner tabAllAppsButton = (Spinner) tabViewAllApps
                .findViewById(R.id.tab_widget_all_apps_button);
        // AllAppsPage keeps a reference to the tab apps to change
        // the title when a group is selected by the user
        mAllAppsPage.setTab(tabAllAppsButton);

        // Adding key listener to redirect enter and dpad center to spinner
        View tabViewAllAppsLayout = tabViewAllApps.findViewById(R.id.tab_widget_all_apps);
        tabViewAllAppsLayout.setOnKeyListener(new OnKeyListener() {

            @Override
            public boolean onKey(View arg0, int keycode, KeyEvent ev) {
                // Only redirect the event if the all apps tab is the current
                if (getCurrentTabTag().equals(APPS_TAB_TAG)) {
                    if (keycode == KeyEvent.KEYCODE_ENTER
                            || keycode == KeyEvent.KEYCODE_DPAD_CENTER) {
                        tabAllAppsButton.performClick();
                        return true;
                    } else {
                        // don't eat so all apps can be set as current
                        return false;
                    }
                }
                return false;
        ...
        if (app.hasNavigationBar() || app.isScreenLarge()){
            View moreOptionsButton = findViewById(R.id.moreoverflow_button);      
            moreOptionsButton.setOnKeyListener(keyListener);
        }
   	...
   	//added by amt_wangpeipei 2012/05/11 return AppsCustomizeSearchBar object.
    public AppsCustomizeSearchBar getAppsCustomizeSearchBar() {
		return mAppsCustomizeSearchBar;
	}
    
	
	/*added by amt_wangpeipei 2012/05/09 begin*/
	//change top view between AppsCustomizeSearchBar and TabsContainer after search or back button click.
    public void setAppsCustomizeSearchBarVisibility(int visibility){
    	mAppsCustomizeSearchBar.setVisibility(visibility);
    }
    public void setTabsContainerVisibility(int visibility){
    	mTabsContainer.setVisibility(visibility);
    }
    /*added by amt_wangpeipei 2012/05/09 end*/
    
    /**
     * added by amt_wangpeipei 2012/05/10 
     * @return AppsCustomizePagedView object to be used in AppsCustomizeSearchBar to refresh data after filter.
     */
    public AppsCustomizePagedView getAppsCustomizePane(){
...
// Modified by e13775 at 19 June 2012 for organize apps' group start 
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    	/*2012-11-19, Modifyed by amt_chenjing for switchuitwo-44*/
        boolean remeasureTabWidthContainer = (mTabs.getLayoutParams().width <= 0);
        super.onMeasure(widthMeasureSpec, heightMeasureSpec);

        // Set the width of the tab list to the content width
        if (remeasureTabWidthContainer) {
	...
	/*Added by ncqp34 at Jun-13-2012 for switchui-2186*/
    ...
    public void setCurrentTabFromContent(AppsCustomizePagedView.ContentType type) {
        setOnTabChangedListener(null);
        setCurrentTabByTag(getTabTagForContentType(type));
        mAllAppsPage.setAppsPageItems(type == AppsCustomizePagedView.ContentType.Applications);
        setOnTabChangedListener(this);
    }
    ...
    void reset() {
        if (mInTransition) {
            ...
            /*2012-6-13, add by bvq783 for 1469*/
            mAppsCustomizeSearchBar.hideInputMethod();
            /*2012-6-13, add end*/
        }
    }
    ...
    public void onResume() {
		  //added by amt_wangpeipei 2012/08/02 for SWITCHUI-2515 begin
		  mAppsCustomizeSearchBar.showVoiceButton();
		  //added by amt_wangpeipei 2012/08/02 for SWITCHUI-2515 end.
        ...
        if (!isChildrenDrawingOrderEnabled()) {
            for (int i = 0; i < count; i++) {
                ...
                if (child == this) {
					...
					// modify by rjg678 for pinch panel
                    if (child.getVisibility() == GONE || child instanceof DragPanel) {
                        continue;
                    }
					// modify end
    ...
    /**
     * Tells AllAppsPage anyway about the tab changed and set the
     * clickable property as appropriate
     */
    void tabChanged(AppsCustomizePagedView.ContentType type){
        mAllAppsPage.onTabChanged(type);
    }
}
