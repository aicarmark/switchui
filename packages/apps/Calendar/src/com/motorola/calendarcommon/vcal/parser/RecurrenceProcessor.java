...
// IS A COPY OF com.android.providers.calendar.RecurrenceProcessor
// WHICH IS UNDER CALENDAR APPLICAION.
// *************************
...
byday:
    ...
    /**
     * Filter the dates by BYSETPOS in recurrence rule. This method returns a
     * date list after apply BYSETPOS rule.
     *
     * @param r The recurrence rule
     * @param list The list of dates to be processed.
     * @return The list of dates after apply the BYSETPOS.
     */
    private static List<Time> filterDatesBYSETPOS(EventRecurrence r, List<Time> list)
    {
        final int listSize = list.size();
        int[] positions = new int[r.bysetposCount];
        for (int i = 0; i < r.bysetposCount; i++) {
            int pos = r.bysetpos[i];
            if (pos > 0) {
                pos--;
            } else if (pos <= 0) {
                pos = pos + listSize;
        ...
        if (DUMP) {
            ...
            Log.v(TAG, "BYSETPOS, positions: [" + b.toString() + "]");
        }

        List<Time> dateList = new ArrayList<Time>();
        for (int p : positions) {
            if (p >= 0 && p < listSize) {
                dateList.add(list.get(p));
            }
        }
    ...
    /**
     * Add the <code>genDate</code> to the <code>out</code>.
     *
     * @param r The recurrence rule
     * @param genDate The generated date
     * @param untilDateValue The until date value after normalizing
     * @param rangeStartDateValue The range start date value after normalizing
     * @param rangeEndDateValue The range end date value after normalizing
     * @param count Count of current expanded dates
     * @param add True to add, false to remove.
     * @param out The out put set.
     * @return 0 if it need to continue expanding; >0 if need to stop expanding.
     */
    private int processGeneratedDate(EventRecurrence r, Time genDate, long untilDateValue,
            long rangeStartDateValue, long rangeEndDateValue, int count, boolean add, TreeSet<Long> out) {
        long genDateValue = normDateTimeComparisonValue(genDate);
        // one reason we can stop is that
        // we're past the until date
        if (genDateValue > untilDateValue) {
            if (DUMP) {
                Log.v(TAG, "stopping b/c until="
                        + untilDateValue
                        + " generated="
                        + genDate.format2445());
            }
        ...
        if (genDateValue >= rangeEndDateValue) {
            if (DUMP) {
                Log.v(TAG, "stopping b/c rangeEnd="
                        + rangeEndDateValue
                        + " generated=" + genDate.format2445());
            }
            return 2;
        }
        ...
        if (genDateValue >= rangeStartDateValue) {
            if (DUMP) {
                Log.v(TAG, "adding date = " + genDate.format2445());
            }
            if (add) {
        ...
        if (r.count > 0 && r.count == count) {
            if (DUMP) {
                Log.v(TAG, "stopping b/c count=" + count);
            }
            return 3;
        }

        // if we got to here, we need to continue expand
        return 0;
    }
    ...
    public static class DaySet
    {
        ...
        boolean get(Time iterator, int day)
        {
            ...
            if (DEVELOPMENT) {
                Log.v(TAG, "get called with iterator=" + iterator.format2445()
                        + " " + (iterator.month+1)
                        + "/" + iterator.monthDay
            ...
            if (day < 1 || day > 28) {
                ...
                day = t.monthDay;
                if (DEVELOPMENT) {
                    Log.v(TAG, "normalized t=" + t.format2445() + " " + (t.month+1)
                            + "/" + t.monthDay
                            + "/" + t.year);
                }
            }
            ...
            if (DEVELOPMENT) {
                Log.v(TAG, "set t=" + t + " " + (realMonth+1) + "/" + day + "/" + realYear);
            }
            if (realYear != mYear || realMonth != mMonth) {
                if (t == null) {
                    ...
                    unsafeNormalize(t);
                    if (DEVELOPMENT) {
                        Log.v(TAG, "set t=" + t.format2445() + " " + (t.month+1)
                                + "/" + t.monthDay
                                + "/" + t.year
    ...
    public long[] expand(Time dtstart,
        ...
        for (Long val: dtSet) {
            ...
            //MOTO Begin this is a workaround, for CR IKOLYMP-11302
            if ((mIterator.allDay == true) && (mIterator.hour != 0 || mIterator.minute != 0 || mIterator.second != 0))
            {
               Log.i(TAG, "recurrent exdates allday is true, but hour =" + mIterator.hour
                        + " minute =" + mIterator.minute
                        + " second =" + mIterator.second);
               mIterator.hour = 0;
               mIterator.minute = 0;
               mIterator.second = 0;
            }
    ...
    public void expand(Time dtstart,
        ...
        try {
            ...
            // Added to support BYSETPOS
            boolean usebysetpos = r.bysetposCount > 0;
            ...
            events: {
                while (true) {
                    ...
                    // clear the BYSETPOS date list
                    bysetposDateList.clear();

                    if (DEVELOPMENT) {
                        Log.v(TAG, "year=" + generated.year);
                    }
                        ...
                        do { // day
                            ...
                            do {
                                int hour = usebyhour
                                                ...
                                                : iteratorHour;
                                if (DEVELOPMENT) {
                                    Log.v(TAG, "      hour=" + hour + " usebyhour=" + usebyhour);
                                }
                                ...
                                do {
                                    ...
                                    do {
                                        ...
                                        if (0 == filtered) {
                                            // For BYSETPOS, we need to calculate all recurrence
                                            // instances in the interval of FREQ, then
                                            // post-processing/filter them by position number.
                                            if (usebysetpos) {
                                                bysetposDateList.add(new Time(generated));
                                                break filter;
                                            }
                                            ...
                                            if (0 != processGeneratedDate(r, generated, untilDateValue,
                                                    rangeStartDateValue, rangeEndDateValue, count, add, out)) {
                                                break events;
                                            }
                                        }
                                        } // filter label
                    ...
                    // If the recurrence rule has a SETBYPOS, we saved all the instances in a list,
                    // and here's the postprocessing
                    if (usebysetpos) {
                        if (DUMP) {
                            StringBuilder b = new StringBuilder();
                            for (Time e : bysetposDateList) {
                                if (b.length() > 0) b.append(", ");
                                b.append(e.format2445());
                            }
                            Log.v(TAG, "BYSETPOS, all before filter: [" + b.toString() + "]");
                        }
                        List<Time> dates = filterDatesBYSETPOS(r, bysetposDateList);
                        for (Time d : dates) {
                            long genDateValue = normDateTimeComparisonValue(d);
                            if (genDateValue >= dtstartDateValue) {
                                // This condition must be the complement of the
                                // condition for incrementing count at the
                                // beginning of the method, so if we don't
                                ...
                                if (0 != processGeneratedDate(r, d, untilDateValue,
                                        rangeStartDateValue, rangeEndDateValue, count, add, out)) {
                                    break events;
                                }
                            }
