...
public class AlertService extends Service {
    ...
    static boolean updateAlertNotification(Context context) {
        ...
        if (alertCursor == null || alertCursor.getCount() == 0) {
            ...
            // Begin motorola for lockscreen
            updateLockScreenNotification(cr, "", 0);
            // End motorola
    ...
    public static boolean generateAlerts(Context context, NotificationMgr nm,
        ...
        // Begin motorola for Lockscreen notification
        NotificationInfo mNotificationInfo = null;
        if (!highPriorityEvents.isEmpty()) {
            /*Modified by songshun.zhang for SWITCHUITWO-549 at 2013-1-21 */
            //mNotificationInfo = highPriorityEvents.get(highPriorityEvents.size() - 1);
            mNotificationInfo = highPriorityEvents.get(0);
            /*Modified end*/
        } else if (!mediumPriorityEvents.isEmpty()) {
            /*Modified by songshun.zhang for SWITCHUITWO-549 at 2013-1-21 */
            //mNotificationInfo = mediumPriorityEvents.get(mediumPriorityEvents.size() - 1);
            mNotificationInfo = mediumPriorityEvents.get(0);
            /*Modified end*/
        } /*Modified by songshun.zhang for SWITCHUITWO-549 at 2013-1-21 */
        else if (!lowPriorityEvents.isEmpty()) {
            mNotificationInfo = lowPriorityEvents.get(0);
        }
        /*Modified end*/
        if (mNotificationInfo != null) {
             String latestEventName = mNotificationInfo.eventName;
             if (TextUtils.isEmpty(latestEventName)) {
                latestEventName = context.getString(R.string.no_title_label);
             }
             /*Modified by songshun.zhang for SWITCHUITWO-549 at 2013-1-21 */
             //updateLockScreenNotification(context.getContentResolver(), latestEventName, numFired);
             int num = highPriorityEvents.size() + mediumPriorityEvents.size() + lowPriorityEvents.size();
             updateLockScreenNotification(context.getContentResolver(), latestEventName, num);
             /*Modified end*/
        }
        // End motorola
        return true;
    ...
    /* package */ static class NotificationPrefs {
        ...
        // added by amt_sunli 2012-12-18 SWITCHUITWO-311 begin
        private static final String USR_CHOOSE_RINGTONE = "content://media/external/audio/media";
        private static final String DEFAULT_RINGTONE = "content://settings/system/notification_sound";
        // added by amt_sunli 2012-12-18 SWITCHUITWO-311 end
        ...
        private String getRingtoneAndSilence() {
            if (ringtone == null) {
                if (quietUpdate) {
                    ...
                    // added by amt_sunli 2012-12-18 SWITCHUITWO-311 begin
                    boolean isAvailedRingtone = true;
                    if (!TextUtils.isEmpty(ringtone) && ringtone.contains(USR_CHOOSE_RINGTONE)) {
                        if (context != null) {
                            Cursor mCursor = null;
                            try {
                                mCursor = context.getContentResolver().query(Uri.parse(ringtone), null, null, null, null);
                                if (mCursor == null || mCursor.getCount() == 0) {
                                    isAvailedRingtone = false;
                                }
                            } catch(Exception e) {
                                e.printStackTrace();
                            } finally {
                    ...
                    if (isAvailedRingtone == false) {
                        Editor edit = prefs.edit();
                        edit.putString(GeneralPreferences.KEY_ALERTS_RINGTONE, DEFAULT_RINGTONE);
                        edit.commit();
    ...
    // Begin motorola for lockscreen feature
    private static final String CALENDAR_INTRADAY_COUNT = "calendar_intraday_count";
    private static final String LATEST_AGENDA_NAME  = "latest_agenda_name";

    public static void updateLockScreenNotification(ContentResolver cr, String latestEventName, int eventCount){
        Settings.System.putString(cr, LATEST_AGENDA_NAME, latestEventName);
        Settings.System.putInt(cr, CALENDAR_INTRADAY_COUNT, eventCount);
    }
    // End motorola
}
