...
public class AlertUtils {
    ...
    static String formatTimeLocation(Context context, long startMillis, boolean allDay,
        ...
        if (!allDay && tz != Time.getCurrentTimezone()) {
            ...
            //modify by amt_xulei for SWITCHUITWO-514 / EG808T-JB_P000903/SWITCHUITWO-584 at 2013-1-24
            //reason:display timezone for "08:00" instead of "0800" when Locale is Chinese
            String timeZone = TimeZone.getTimeZone(tz).getDisplayName(
                    isDST, TimeZone.SHORT, Locale.getDefault());
            if (Utils.isDefaultLocaleChinese()){
            	int split = timeZone.length()-2;
            	timeZone = timeZone.substring(0, split) + ":" +  timeZone.substring(split, timeZone.length());
            }
