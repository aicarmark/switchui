...
import com.android.calendar.Utils.LunarInfo;
import com.android.calendar.GeneralPreferences;

public class MonthWeekEventsView extends SimpleWeekView implements OnSharedPreferenceChangeListener{
    ...
    //Begin Motorla - lunar calendar
    private static int SIDE_PADDING_MONTH_LUNAR = 25;
    private static int BOTTOM_PADDING_MONTH_LUNAR = 16;
    private static int SIDE_PADDING_TABLET_MONTH_LUNAR = 25;
    private static int BOTTOM_PADDING_TABLET_MONTH_LUNAR = 20;
    private static int SIDE_PADDING_MONTH_LUNAR_TEXT = 15;
    private static int TEXT_LENGTH_LUNAR_HOLIDAY = 2;
    ...
    protected int mMonthViewHolidayColor;
    protected int mMonthViewTermColor;
    ...
    protected void loadColors(Context context) {
        ...
        mMonthViewHolidayColor = res.getColor(R.color.monthlyview_holiday);
        mMonthViewTermColor = res.getColor(R.color.monthlyview_term);  
    }
    ...
    protected void initView() {
        ...
        //Begin Motorola
        // for lunar information
        mIsTabletConfig = Utils.getConfigBool(getContext(), R.bool.tablet_config);
        ...
        mShowHoliday = prefs.getBoolean(GeneralPreferences.KEY_SHOW_HOLIDAY, //fwr687 IKSF-3317
    ...
    protected void drawWeekNums(Canvas canvas) {
        ...
        Time baseLunarDate = new Time(mTimeZone);
        baseLunarDate.setJulianDay(mFirstJulianDay);
	if(baseLunarDate.year < 1970){
        	baseLunarDate.set(baseLunarDate.monthDay + 1, baseLunarDate.month, baseLunarDate.year);
        } 
        ...
        for (; i < numCount; i++) {
            ...
            if (Utils.isDefaultLocaleChinese() && (mShowLunar || mShowHoliday)) {
                String lunarinfo = "";
                if (mShowLunar) {
                    LunarInfo lunarData = Utils.monthlyViewLunarDate(
                            getContext(), baseLunarDate, mShowHoliday);
                    if (lunarData != null && (lunarData.lunarType == LunarInfo.LUNAR_TYPE_FESTIVAL
                            || lunarData.lunarType == LunarInfo.SOLAR_TYPE_HOLIDAY)) {
                        mLunarNumPaint.setColor(mMonthViewHolidayColor);
                    } else if ((lunarData != null && lunarData.lunarType == LunarInfo.LUNAR_TYPE_SOLAR_TEAM) 
                    		|| (mHasToday && todayIndex == i)) {
                        mLunarNumPaint.setColor(mMonthViewTermColor);
                    } else {
                        mLunarNumPaint.setColor(isFocusMonth ? mMonthNumColor
                                : mMonthNumOtherColor);
                    }
                    if (lunarData != null && !TextUtils.isEmpty(lunarData.lunarStr)) {
                        lunarinfo = lunarData.lunarStr;
                    }
                } else {
                    if (mShowHoliday) {
                        lunarinfo = Utils.getSolarHolidayByDate(getContext(),
                                        baseLunarDate.month, baseLunarDate.monthDay);
                ...
                if(!TextUtils.isEmpty(lunarinfo)){
                    int textX1 = x;
                    int textY1 = mHeight - BOTTOM_PADDING_MONTH_LUNAR;
                    if (mIsTabletConfig) {
                        if (lunarinfo.length() > TEXT_LENGTH_LUNAR_HOLIDAY) {
                            textX1 = x - SIDE_PADDING_TABLET_MONTH_LUNAR
                                    - SIDE_PADDING_MONTH_LUNAR_TEXT;
                        } else {
                            textX1 = x - SIDE_PADDING_TABLET_MONTH_LUNAR;
                        }
                    } else {
                        if (lunarinfo.length() > TEXT_LENGTH_LUNAR_HOLIDAY) {
                            textX1 = x - SIDE_PADDING_MONTH_LUNAR
                                    - SIDE_PADDING_MONTH_LUNAR_TEXT;
                        } else {
                            textX1 = x - SIDE_PADDING_MONTH_LUNAR;
                        }
                    }
                    canvas.drawText(lunarinfo, textX1, textY1+12, mLunarNumPaint);
                }
                baseLunarDate.monthDay += 1; // next day
                baseLunarDate.normalize(false);
            }
            //End Motorola
    ...
    public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
        ...
        if (key.equals(GeneralPreferences.KEY_SHOW_HOLIDAY)) {
            boolean newSetting = sharedPreferences.getBoolean(key,
                    GeneralPreferences.DEFAULT_VALUE_OF_SHOW_HOLIDAY);
