...
public class CalendarAppWidgetProvider extends AppWidgetProvider {
    ...
    boolean mShowLunar = true;
    boolean mShowHoliday = true;
    // added by amt_sunli 2012-11-28 SWITCHUITWO-148 begin
    private static final String LUNAR_CHANGED_ACTION = "com.motorola.lunarchanged";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        // modified by amt_sunli 2012-11-28 SWITCHUITWO-148 begin
        if (Utils.getWidgetUpdateAction(context).equals(action) || LUNAR_CHANGED_ACTION.equals(action)) {
        // modified by amt_sunli 2012-11-28 SWITCHUITWO-148 end
    ...
    private void performUpdate(Context context,
        ...
        for (int appWidgetId : appWidgetIds) {
            ...
            final String year = Integer.toString(time.year) + context.getResources().getString(R.string.year);
            views.setTextViewText(R.id.day_of_week, year);
            views.setTextViewText(R.id.date, date);
            // Begin Motorola
            final String dayOfMonth = DateUtils.getMonthString(time.month, DateUtils.LENGTH_MEDIUM);
            if (Utils.isDefaultLocaleChinese() && mShowLunar) {
		String lunarStr = Utils.lunarMonthAndDay(context, time, true);
            	if (lunarStr != null) {
            	    int ind = lunarStr.indexOf(" ");
            	    if(ind > 0) {
                        lunarStr = lunarStr.substring(ind + 1);
                    }
            	}            	
            	views.setTextViewText(R.id.icon_month, lunarStr);
            } else {
            	views.setTextViewText(R.id.icon_month, dayOfMonth);
            }
            views.setTextViewText(R.id.icon_day_of_month, Integer.toString(time.monthDay));
            // End Motorola
