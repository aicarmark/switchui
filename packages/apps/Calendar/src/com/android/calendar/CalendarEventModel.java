...
public class CalendarEventModel implements Serializable {
    ...
    public void addAttendees(String attendees, Rfc822Validator validator, int type) {
    ...
    // BEGIN MOTOROLA - IKPIM-803: Support optional attendee type on event edit
    public static boolean equalAttendeeList(LinkedHashMap<String, Attendee> AttendeeList1,
            LinkedHashMap<String, Attendee> AttendeeList2) {
        if (AttendeeList1 == null) {
            if (AttendeeList2 == null) {
                return true;
            } else {
                return false;
            }
            ...
            if (AttendeeList1.size() != AttendeeList2.size()) {
                return false;
            }
            for (String key : AttendeeList1.keySet()) {
                if (AttendeeList2.containsKey(key)) {
                    String email1 = AttendeeList1.get(key).mEmail;
                    int type1 = AttendeeList1.get(key).mType;
                    String email2 = AttendeeList2.get(key).mEmail;
                    int type2 = AttendeeList2.get(key).mType;
                    if (!TextUtils.equals(email1, email2) || (type1 != type2)) {
                        return false;
                    }
                } else {
                    return false;
    ...
    protected boolean checkOriginalModelFields(CalendarEventModel originalModel) {
        ...
        // BEGIN MOTOROLA - IKPIM-803: Support optional attendee type on event edit
        if (!equalAttendeeList(mAttendeesList, originalModel.mAttendeesList)) {
            return false;
        }
        // END MOTOROLA - IKPIM-803
