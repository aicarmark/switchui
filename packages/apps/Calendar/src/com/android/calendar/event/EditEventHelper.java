...
public class EditEventHelper {
    ...
    public static final String[] EVENT_PROJECTION = new String[] {
            ...
            Events.STATUS, // 21
            Events.ACCOUNT_NAME, // 22
            Events.ACCOUNT_TYPE, // 23
    };
    ...
    protected static final int EVENT_INDEX_ACCOUNT_NAME = 22;
    protected static final int EVENT_INDEX_ACCOUNT_TYPE = 23;
    ...
    // Begin Motorola
    static final int CALENDARS_INDEX_ACCOUNT_TYPE = 11;
    // End Motorola
    ...
    static final String CALENDARS_WHERE_WRITEABLE_VISIBLE_NOLOCALCALENDAR = CALENDARS_WHERE_WRITEABLE_VISIBLE
    ...
    public boolean saveEvent(CalendarEventModel model, CalendarEventModel originalModel,
        ...
        if (uri == null) {
            ...
            if (TextUtils.isEmpty(model.mRrule)) {
                ...
                checkTimeDependentFields(originalModel, model, values, modifyWhich);
                // IKHSS7-1798 - Start
                // Need to update the orginalInstanceTime field if the DTSTART changes
                if (model.mStart != -1 && originalModel.mStart != -1 && model.mStart != originalModel.mStart
                        && !TextUtils.isEmpty(model.mSyncId)) {
                    long delta = model.mStart - originalModel.mStart; // delta mills in UTC

                    String[] projection = new String[] {
                            Events._ID, // 0
                            Events.ORIGINAL_INSTANCE_TIME, // 1
                            Events.ORIGINAL_SYNC_ID, // 2
                    };
                    ContentResolver cr = mContext.getContentResolver();
                    Cursor exCursor = cr.query(Events.CONTENT_URI, projection,
                            Events.ORIGINAL_SYNC_ID + "=?", new String[] { model.mSyncId }, null);
                    try {
                        if (exCursor != null && exCursor.getCount() > 0) {
                            exCursor.moveToFirst();
                            ContentValues exValues = new ContentValues();
                            do {
                                Uri exUri = ContentUris.withAppendedId(Events.CONTENT_URI,
                                        exCursor.getLong(0));
                                exValues.clear();
                                exValues.put(Events.ORIGINAL_INSTANCE_TIME,
                                        Long.valueOf(exCursor.getLong(1) + delta));
                                ops.add(ContentProviderOperation.newUpdate(exUri)
                                        .withValues(exValues).build());
        ...
        String originalOrgnizer = null;
        if (originalModel != null) {
            originalReminders = originalModel.mReminders;
            originalOrgnizer = originalModel.mOrganizer;
        } else {
        ...
        if (!CalendarContract.ACCOUNT_TYPE_LOCAL.equals(model.mSyncAccountType) &&
            ...
            if (newEvent || !CalendarEventModel.equalAttendeeList(originalModel.mAttendeesList,
                    model.mAttendeesList)) {
                HashMap<String, Attendee> updateAttendees = new HashMap<String, Attendee>();
            // END MOTOROLA - IKPIM-803
                ...
                if (!newEvent) {
                    ...
                    for (String originalEmail : originalAttendees.keySet()) {
                        if (newAttendees.containsKey(originalEmail)) {
                            // BEGIN MOTOROLA - IKPIM-803: Support optional attendee on event edit
                            // Check whether attendee type got changed
                            int originalType = originalAttendees.get(originalEmail).mType;
                            int newType = newAttendees.get(originalEmail).mType;
                            if (originalType != newType) {
                                updateAttendees.put(originalEmail, newAttendees.get(originalEmail));
                            }
                            // END MOTOROLA - IKPIM-803
                ...
                if (updateAttendees.size() > 0) {
                    // Update the changed attendees
                    for (Attendee attendee : updateAttendees.values()) {
                        if (DEBUG) {
                            Log.d(TAG, "updateAttendees item: name=" + attendee.mName + ", email="
                                    + attendee.mEmail + ", type=" + attendee.mType);
                        }
                        values.clear();
                        values.put(Attendees.ATTENDEE_NAME, attendee.mName);
                        values.put(Attendees.ATTENDEE_EMAIL, attendee.mEmail);
                        ...
                        b = ContentProviderOperation.newUpdate(Attendees.CONTENT_URI)
                                .withValues(values);
                        String selection = Attendees.EVENT_ID + "=? AND "
                                + Attendees.ATTENDEE_EMAIL + "=?";
                        b.withSelection(selection,
                                new String[] { Long.toString(eventId), attendee.mEmail });
                        ops.add(b.build());
                    }
                }
                // END MOTOROLA - IKPIM-803
    ...
    public String updatePastEvents(ArrayList<ContentProviderOperation> ops,
        ...
        if (origRecurrence.count > 0) {
            ...
            if ("com.android.exchange".equals(originalModel.mSyncAccountType)) {
                untilTime.set(endTimeMillis - MILLIS_PER_DAY);
            }
            // End Motorola
            if (origAllDay) {
                untilTime.hour = 0;
        ...
        ops.add(b.build());
        // Begin Motorola
        // We need to delete all exceptions after the *endTimeMillis*, these
        // exceptions doesn't mean anything because they have been out of the
        // original event's time frame.
        List<ContentProviderOperation> deleteOps = buildDeleteExceptionsAfter(mContext,
                originalModel.mId, originalModel.mSyncId, endTimeMillis);
        if (deleteOps.size() > 0) {
    ...
    void addRecurrenceRule(ContentValues values, CalendarEventModel model) {
        ...
        // modified by amt_sunli 2012-12-11 SWITCHUITWO-224 begin
        if (end >= start) {
        // modified by amt_sunli 2012-12-11 SWITCHUITWO-224 end
            if (isAllDay) {
    ...
    public static void setModelFromCursor(CalendarEventModel model, Cursor cursor) {
        ...
        // Begin Motorola
        model.mSyncAccount = cursor.getString(EVENT_INDEX_ACCOUNT_NAME);
    ...
    // Begin Motorola
    /**
     * Build an operation list to remove all exception events that are after a given time.
     *
     * @param context The Context
     * @param eventId The id of parent event
     * @param eventSyncId The sync id of parent event
     * @param timeAfter The given time after which all exceptions will be deleted.
     * @return The list of operations to delete exceptions.
     */
    public static List<ContentProviderOperation> buildDeleteExceptionsAfter(Context context,
            long eventId, String eventSyncId, long timeAfter) {
        if (TextUtils.isEmpty(eventSyncId)) {
            // Exceptions cannot belongs to an event that isn't sync'd
            return Collections.emptyList();
        }

        final String[] projection = new String[] { Events._ID };
        final String selection = Events.ORIGINAL_ID + "=? AND " + Events.ORIGINAL_SYNC_ID
                + "=? AND " + Events.ORIGINAL_INSTANCE_TIME + ">=?";

        List<ContentProviderOperation> deleteOps = new ArrayList<ContentProviderOperation>();
        ContentResolver cr = context.getContentResolver();
        Cursor c = cr.query(Events.CONTENT_URI, projection, selection,
                new String[] { String.valueOf(eventId), eventSyncId, String.valueOf(timeAfter) },
                null);
        if (c != null) {
            try {
                while (c.moveToNext()) {
                    ...
                    long exEventId = c.getLong(0);
                    b.withSelection(Events._ID + "=?", new String[] { String.valueOf(exEventId)} );
                    deleteOps.add(b.build());
                }
            } finally {
                c.close();
            }
