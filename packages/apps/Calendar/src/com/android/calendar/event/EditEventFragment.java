...
public class EditEventFragment extends Fragment implements EventHandler {
    ...
    public static boolean mIsAttendeeEmpty = true;
                ...
                case TOKEN_ATTENDEES:
                    try {
                        while (cursor.moveToNext()) {
                            ...
                            attendee.mType = cursor.getInt(EditEventHelper.ATTENDEES_INDEX_TYPE);
    ...
    private void startQuery() {
        ...
        if (!newEvent) {
            ...
            if (mModel.mCalendarId == -1 && mModel.mAttendeesList.size() > 0) {
                mIsAttendeeEmpty = false;
            } else {
                mIsAttendeeEmpty = true;
            }
            ...
            if (mIsAttendeeEmpty) {
                ...
                mHandler.startQuery(
                        ...
                        Calendars.CONTENT_URI,
                        EditEventHelper.CALENDARS_PROJECTION,
                        EditEventHelper.CALENDARS_WHERE_WRITEABLE_VISIBLE_NOLOCALCALENDAR,
                        null /* selection args */, null /* sort order */);
            }
...
//        mContext.requestWindowFeature(Window.FEATURE_INDETERMINATE_PROGRESS);
    ...
    protected void displayEditWhichDialog() {
        if (mModification == Utils.MODIFY_UNINITIALIZED) {
            ...
            //add by amt_xulei for SWITCHUITWO-586 at 2013-1-29 save ics-vcal
            final boolean isLocalEvent = mModel.mSyncAccountType.equals("LOCAL");
            final boolean allowChangeSingle = isLocalEvent || (!notSynced);
            //end add for SWITCHUITWO-586
            boolean isFirstEventInSeries = mModel.mIsFirstEventInSeries;
            int itemIndex = 0;
            CharSequence[] items;

            if (!allowChangeSingle) {
                    ...
                    .setItems(items, new OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            if (which == 0) {
                                ...
                                mModification = (!allowChangeSingle) ? Utils.MODIFY_ALL
                                        : Utils.MODIFY_SELECTED;
                                if (mModification == Utils.MODIFY_SELECTED) {
                                    mModel.mOriginalSyncId = (!allowChangeSingle) ? null : mModel.mSyncId;
                                    mModel.mOriginalId = mModel.mId;
                                }
                            } else if (which == 1) {
                                mModification = (!allowChangeSingle) ? Utils.MODIFY_ALL_FOLLOWING
                                        : Utils.MODIFY_ALL;
                            } else if (which == 2) {
    ...
    public void onPause() {
       ...
       // BEGIN MOTOROLA - IKPIM-803: Support meeting attendee list screen
        if (mSaveOnDetach && act != null && !mIsReadOnly && !act.isChangingConfigurations()
                && mView.prepareForSave() && !mView.isMultiPicking()) {
        // END MOTOROLA - IKPIM-803
