...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    // added by amt_sunli 2012-12-17 SWITCHUITWO-290 begin
    private Time mMaxDate;
    private Time mMinDate;
    // added by amt_sunli 2012-12-17 SWITCHUITWO-290 end
    ...
    */
    ...
    private void init(Context context) {
        ...
        // added by amt_sunli 2012-12-17 SWITCHUITWO-290 begin
        mMaxDate = new Time(Utils.getTimeZone(context, mTZUpdater));
        mMinDate = new Time(Utils.getTimeZone(context, mTZUpdater));
        int viewType = mController.getViewType();
        if(ViewType.WEEK  == viewType){
            mMaxDate.set(59, 59, 23, 27, 11, 2036);
            mMinDate.set(59, 59, 23, 3, 0, 1970);
        }else{
            mMaxDate.set(59, 59, 23, 30, 11, 2036);
            mMinDate.set(59, 59, 23, 1, 0, 1970);
        }
        // added by amt_sunli 2012-12-17 SWITCHUITWO-290 end
...
//        mContext.startProgressSpinner();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if ((mTouchMode & TOUCH_MODE_HSCROLL) != 0) {
            ...
            /*2012-9-20, add by amt_jiayuanyuan for SWITCHUITWOV-17 */
            Log.d("archermind","nextView.mViewStartX = " +nextView.mViewStartX);
            if(nextView.mViewStartX != 0){
    ...
    private void drawAfterScroll(Canvas canvas) {
        ...
        // added by amt_sunli 2012-12-25 SWITCHUITWO-378 begin
        if (mSelectionAllday && mSelectionMode != SELECTION_HIDDEN) {
            ...
            if (mNumDays > 1) {
                ...
                canvas.drawLine(mRect.left + horizontalPadding, midY, mRect.right - horizontalPadding,
                        midY, p);
                canvas.drawLine(midX, mRect.top + verticalPadding, midX, mRect.bottom - verticalPadding, p);
            } else {
        ...
        // added by amt_sunli 2012-12-25 SWITCHUITWO-378 end
    ...
    private void drawAmPm(Canvas canvas, Paint p) {
        ...
        // modified by amt_sunli 2012-12-05 SWITCHUITWO-197 begin
//        canvas.drawText(text, HOURS_LEFT_MARGIN, y, p);
        int width = (int) (p.measureText(text) + 0.5);
        canvas.drawText(text, width < HOURS_LEFT_MARGIN ? HOURS_LEFT_MARGIN
                : (int) ((mHoursWidth + width) / 2), y, p);
        // modified by amt_sunli 2012-12-05 SWITCHUITWO-197 end
        if (mFirstHour < 12 && mFirstHour + mNumHours > 12) {
            ...
            // modified by amt_sunli 2012-12-05 SWITCHUITWO-197 begin
            width = (int) (p.measureText(text) + 0.5);
            canvas.drawText(text, width < HOURS_LEFT_MARGIN ? HOURS_LEFT_MARGIN
                    : (int) ((mHoursWidth + width) / 2), y, p);
//            canvas.drawText(text, HOURS_LEFT_MARGIN, y, p);
            // modified by amt_sunli 2012-12-05 SWITCHUITWO-197 end
    ...
    private void doDraw(Canvas canvas) {
        ...
        //Begin Motorola - SWITCHUITWO-215 
...
//    }
    ...
    private boolean setSelectionFromPosition(int x, final int y, boolean keepOldSelection) {
        ...
        // modified by amt_sunli SWITCHUITWO-416 2012-12-26 begin
        boolean isClickUpperLeft = false;
        if (x < mHoursWidth) {
            x = mHoursWidth;
            isClickUpperLeft = true;
        }
        // modified by amt_sunli SWITCHUITWO-416 2012-12-26 end
...
//        findSelectedEvent(x, y);
          findSelectedEvent(x, y, isClickUpperLeft);
          // modified by amt_sunli SWITCHUITWO-416 2012-12-26 end
...
//        }
     ...
     // modified by amt_sunli SWITCHUITWO-416 2012-12-26 begin
    private void findSelectedEvent(int x, int y, boolean isClickUpperLeft) {
     // modified by amt_sunli SWITCHUITWO-416 2012-12-26 end
        ...
        mSelectedEvents.clear();
        // modified by amt_sunli SWITCHUITWO-416 2012-12-26 begin
        if (mSelectionAllday && isClickUpperLeft) {
            return;
        }
        // modified by amt_sunli SWITCHUITWO-416 2012-12-26 end
        if (mSelectionAllday) {
    ...
    public boolean onLongClick(View v) {
        // modified by amt_sunli 2012-11-29 SWITCHUITWO-169 begin
        int flags = DateUtils.FORMAT_SHOW_WEEKDAY | DateUtils.FORMAT_ABBREV_WEEKDAY;
        // modified by amt_sunli 2012-11-29 SWITCHUITWO-169 end
    ...
    // added by amt_sunli 2012-12-17 SWITCHUITWO-290 begin
    private boolean isInDateBounds(float distanceX){
        Time pointTime = new Time(mBaseDate.timezone);
        pointTime.set(mController.getTime());
        if(pointTime.after(mMaxDate)&& distanceX > 0){
            return false;
        }else if (pointTime.before(mMinDate)&& distanceX < 0){
            return false;
        }else
            return true;
    }
