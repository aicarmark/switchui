...
public class CalendarController {
    ...
    /*2012-10-18, add by amt_jiayuanyuan for SWITCHUITWOV-193 */
    private EventHandler nowEventHandler;
    /*2012-10-18, add end*/
    ...
    public void sendEvent(Object sender, final EventInfo event) {
                ...
                if (mToBeAddedEventHandlers.size() > 0) {
                    for (Entry<Integer, EventHandler> food : mToBeAddedEventHandlers.entrySet()) {
                   	...
                   	if (food.getValue() != null) {
							if (food.getValue().equals(nowEventHandler)) {
								eventHandlers.put(food.getKey(),food.getValue());
							}
						}
                   	  /*2012-10-18, add end*/
        ...
        if (!handled) {
            ...
            //Begin Motorola - Launch Lunar Picker
            else if(event.eventType == EventType.LUNAR_PICKER) {
                launchLunarPicker();
                return;
            }
            //End Motorola
...
//    }
    ...
    private void launchSearch(long eventId, String query, ComponentName componentName) {
  	...
  	//Begin Motorola
    private void launchLunarPicker() {
        Log.d(TAG,"launchLunarPicker");
        Intent intent = new Intent(Intent.ACTION_VIEW);
        intent.setClass(mContext, LunarPickerActivity.class);
        mContext.startActivity(intent);
    }
    //End Motorola
